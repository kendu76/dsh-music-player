...
### 3.1 第一层：定时状态行 + 信源池状态行 + 期次列表

期次列表上方新增**常驻「定时」状态行**（定时规则的入口与状态摘要）与**「信源池」状态行**
（RFC docs/news-rss-pool-rfc.md §9.2，RSS 信源池的入口与状态摘要）：

```
┌──────────────────────────────────────────────┐
│  ⏰ 每日定时  08:00 播报 · 12:30 静默 · 18:00 播报 › │   ← 定时状态行（常驻）
│  📡 信源池  10 源 · 池内 23 条 · 30 分钟前更新  ⟳   │   ← 信源池状态行（常驻）
│     （⚠ 2 源拉取失败（IT之家 · 超时）· ⚠ 已暂停 源A） │   ← 失败/暂停提示同行动态追加
│ ────────────────────────────────────────────  │
...
- **信源池状态行**：左「📡 信源池」，中间摘要（启用源数 / 池内条目数 / 最近拉取时间；
  有失败源或自动暂停源时同行动态追加 `⚠ N 源拉取失败（源名 · 原因）` 与 `⚠ 已暂停 源名`），
  右「⟳」**立即拉取**按钮（池未启用时置灰）。点状态行任意位置进入**信源池配置子视图**（§4.3）。
- **空态**：池未启用时摘要显示「信源池未启用」；点击仍可进入配置开启。
│  05-28 · 09:00                               │
│    …                              [🗑]       │   ← 行尾悬停出现删除（二次确认）
└──────────────────────────────────────────────┘
```
...
- **班次行**：时间为 `<input type="time">`（步进 1 分钟）；**「收集后立即播放」勾选框，
  ...
  周末调休补班照常，见下方「仅工作日（班次级）」）。行尾 [▶] **立即执行**、[🗑] 删除
  （至少保留一个班次才可启用）。
...
- **添加班次**：弹窗新增，默认 `08:00 · 新闻条数 8 · 勾选立即播放 · 不勾选仅工作日 · 不选任何类别`；上限 6 个
...
- **新闻条数（班次级，编辑弹窗内设置）**：单期收集条数上限，数字输入 1–20，**默认 8**。
  **多类别尽量平均分配**：若班次选择了多个类别（预设类别 chips + 自定义主题合计），
  ...
  配额兜底收敛（见功能设计 §7.2）。
- **仅工作日（班次级，编辑弹窗内设置）**：勾选后该班次**只在「工作日」**到点定时执行——
  周一至周五扣除**法定节假日**（放假不跑），**周末调休补班视为工作日照常执行**；判断依据
  按需自动联网获取的节假日日历（timor.tech——**仅在到点判断该班次、且日历缺失/过期时才
...
### 4.3 信源池配置子视图（第一层「📡 信源池」进入，push 子视图）

与定时规则编辑器同一导航模式（左上「← 返回」回列表）：

```
┌──────────────────────────────────────────────┐
│ ← 返回        信源池配置                       │
│  [✓] 启用信源池（关 = 完全退回 web_search 补盲） │   ← 总开关
│  拉取节奏 [30] 分钟（15-180，默认 30）           │
│  ⚠ IT之家 拉取失败：HTTP 502                     │   ← 失败源列表（透传错误码）
│  RSS 源（2 / 30）                    [＋ 添加源] │
│  [新华社] [https://rss.news.cn/x.xml] [官方源][国内][✓启用]   [🗑]  │
│  [IT之家] [https://www.ithome.com/rss/] [权威媒体][科技][✓启用] [🗑] │
│         （暂停中的源行尾显示「恢复」按钮）         │
│  [保存]  [取消]                                 │
└──────────────────────────────────────────────┘
```

- **总开关**：关闭 = 完全停止拉取，收集退回纯 `web_search`（能力不降级，RFC §6.2）；
- **拉取节奏**：15–180 分钟，默认 30；保存即重建 Host 拉取定时器；
- **源列表**：每行 = 名称 + RSS URL + 分级（官方源/权威媒体/优质二手/KOL）+ 主投类别 +
  启用开关 + 删除；「＋ 添加源」追加一行（上限 30）；**自动停用**（连续 3 次失败）的源行尾
  显示「恢复」按钮（清 `suspendedUntil` 立即恢复拉取）；
- **失败源列表**：透传工具错误码与消息原文（不做诊断），与收集失败提示同一哲学；
- 默认池 10 源（official 7 + major 3，2026-09 实测今日新鲜）开箱即用，用户无需手工配置即可受益；
  增删改是给进阶用户的自由项。
...
| store | `editions`、`newsView`（'list'\|editionId\|'read'\|'schedule'）、`newsSchedulePrefs`（`{ enabled, model:{provider,model}?, shifts:[{id,time,autoplay,workdaysOnly,itemCount,scope:{categories[],topics[]}}], prefVersion, syncedVersion }`，班次 `scope` 必填（至少一个类别或主题；全局 `defaultScope` 已退役，旧数据空范围由 Host 规整为全选六类），`itemCount` 为「新闻条数」（1-20，默认 8，多类别时各类别尽量平均分配），`workdaysOnly` 为「仅工作日执行」（默认 false：周一至周五扣除法定节假日、周末调休补班照常，见功能设计 §7.6），`model` 为「新闻采集模型」选择器）、`newsSyncState`、`currentItem` 高亮映射 |
