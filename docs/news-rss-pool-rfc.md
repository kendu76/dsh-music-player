...
| # | 软肋 | 现状 | 后果 |
|---|---|---|---|
| 1 | 无订阅概念 | 每次触发 = 全新 `web_search`（提示词 §8.2 的方法论），无信源池 | 索引覆盖不全（设计文档 §8.3「诚实边界」已自认）；token 成本按定时任务数线性放大；无法做增量 |
| 2 | 时效锚不可靠 | 只认 `web_search` 返回的 `publishedAt`，缺失即弃 | 核心源带 pubDate 却不利用；「无 publishedAt 即弃」造成漏报 |
| 3 | 去重靠 LLM 自觉 | 提示词「同一事件跨源去重只留最权威」；工具层只有冷却窗（防同定时任务重复提交），无内容级去重 | 同一事件不同措辞可在不同轮次/不同定时任务重复进期；跨定时任务去重被设计文档 §7.4 自己承认「不成立」 |
...
- **只保留增量**：`pool.items` 只存「进入池后尚未进入任何期次」的条目（`usedIn` 为空）。
  ...
  条目在下一定时任务仍可用（如高频静默定时任务与低频播报定时任务共享池）。
...
- **注入上限 60 条**：池条目摘要 ≤80 字截断，60 条约 5~6k 字符，控制在执行会话的
  followup 消息体合理长度内。
- **只注入与定时任务范围相关的池条目**：`runCollection` 已知道 `shift.scope`（类别 + 主题），
...
- 自定义主题（定时任务 `scope.topics`）默认不在池内（池的默认源是预设类别向）。
- 若用户给某个主题配置了专属 feed（面板信源池增删时选择「归属主题」），则该 feed 的条目
  在该主题定时任务时正常注入——KOL 源只有在用户**显式添加**该 KOL 的 RSS 时才进池（不进默认池），
  这是「KOL 兜底」的唯一入口。
...
- **增量/跨定时任务**：池内「已用条目」不再注入（`usedIn` 非空即摘除），天然减少跨定时任务重复；
...
- 对每条提交条目的 `normalizeTitle`，与以下两组比对：
  ...
  - **组 B · 当日已有期次**：`news.editions` 中 `date === 本期 date` 的条目
    （跨定时任务/跨手动触发的当日去重——补上设计文档 §7.4 承认的缺口）。
- 判定（无需向量库，规则可测）：
  - 相等 → 重复；
  - 较长者包含较短者且长度比 ≥ 0.6 → 重复；
...
同类任务（同一定时任务）当天多次执行时，若「当日已报」条目被硬性剔除，后跑的任务会收不满
配置的新闻条数（如目标 8 条、实际只报 4 条）。**去重不放松**，而是「按需冗余 + 信息注入 +
收敛后置」三管齐下：

1. **按需候选冗余**：仅当**当天该定时任务已执行过**（去重会真的剔除条目）时，定时任务指令才要求
   每类按目标条数 1.5~2 倍提交候选（`news_broadcast` 提交上限相应放宽到 2× 定时任务条数）；
   当天首次执行无去重风险，提交上限保持 = 定时任务条数，**不做无谓冗余、不浪费 token**；
...
3. **收敛后置**：`capCategoriesToQuota` 从「去重前」移到「去重后」执行——候选先经
   工具层去重（剔除与今日已报重复的），剩下的都是新条目，再按定时任务条数/类别配额收敛
...
阶段二 · 深抓（仅候选，配额内）：
  ...
  配额：每类 ≤ 2 条、全期 ≤ 6 条（默认；可随定时任务 itemCount 线性放宽，
...
- 池材料注入：`runCollection` 收集前懒拉取后，按定时任务范围 `filterPoolForScope` 预筛并注入
...
| M-R2 收集接入 | `runCollection` 收集前懒拉取、按定时任务范围 `filterPoolForScope` 预筛池条目、注入【信源池材料】；系统提示词新增「信源池优先 + 两阶段收集」小节 | 池是「可选加强」非「必需前置」——无池/池空/拉取失败时指令不注入，agent 走纯 web_search |
