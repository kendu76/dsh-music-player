...
export function apply(ctx) {
  ...
  // 登录态动作的统一包装：请求前先做主动续命（TTL 24h 到期才刷），随后原样请求。
  // 不再做「撞 20017/20018 后静默刷新重试」的被动补救——日常若真碰到「设备不匹配」
  // （token 已被酷狗判定失效/与设备解绑），直接清空本地会话并带 kgLoginDead 标记，
  // 让客户端回到扫码登录页重新扫码（酷狗无其他续命手段，只能重扫）。
  ...
  const kgWithFreshToken = async (fn) => {
    ...
    try {
      ...
      // 主动续命已尽力，业务仍报「设备不匹配」→ 会话确实死了：清掉 cookie 并打
      // 标记，客户端据此跳回扫码登录页（不重试、不补救）。
  ...
  // v5/login_by_token 换新 token（社区标准 24h 一刷），让 token 在过期前被续上，
  // 避免「好端端突然要重扫」。刷新失败且报设备不匹配 → token 已死，自动登出并带
  // kgLoginDead 标记（客户端跳回扫码页）；其他失败（网络等）忽略，原请求照常。
  ...
  // 【2026-08-30 观察期结论】token 实测可自然存活 68h+ 且业务接口正常工作——
  // 真实过期线远宽于社区流传的 24h。观察期（12h 主动续命屏蔽 + 被动补救兜底）
  // 结束，正式放开主动续命并直接对齐社区节奏（24h 一刷），同时移除被动补救
  // （业务撞 20017/20018 不再现场刷新重试，直接登出跳登录页——见 kgWithFreshToken）。
