...
export function apply(ctx) {
  ...
  const tool = {
    name: 'music_play',
    description: '控制 DSH 本地音乐库与小说库的播放。播放时可按歌曲名/歌手/小说名关键词搜索并播放（不传 query 则播放第一首音乐，没有音乐则播放第一本小说），或按歌单名播放自建歌单（playlist 参数）；也可用 action 执行暂停/继续/停止/下一首·下一章/上一首·上一章。传 source=web 时会改走「在线 QQ 音乐」（需已在播放面板登录，登录后可播 VIP/高音质）；传 source=netease 会改走「在线网易云音乐」（匿名可播免费曲，登录后解锁更多）。',
    parameters: {
      type: 'object',
      ...
      properties: {
        ...
        source: { type: 'string', enum: ['local', 'web', 'netease'], description: '播放来源：local 本地库/小说（默认）；web 在线 QQ 音乐（需登录解锁 VIP/高音质）；netease 在线网易云音乐（匿名可播免费曲，登录后解锁更多）。仅当 action 为 play 时使用' },
    ...
    async execute(args) {
      ...
      const source = args && typeof args.source === 'string' && ['local', 'web', 'netease'].includes(args.source) ? args.source : 'local'
