...
export function apply(ctx) {
  ...
  //   rss             信源池配置（enabled/pollMinutes/feeds[]，RFC docs/news-rss-pool-rfc.md）
  ...
  const runCollection = async (shiftId) => {
    ...
    // 池新鲜度保障：收集前若池无新鲜数据（从未拉取 / 距上次成功拉取 ≥ 拉取节奏），
    // 先同步拉一轮——否则用户刚点「立即执行」时池可能还是空的（首拉 60s 延迟未到），
    // 注入不了池材料，RSS 池形同虚设。拉取失败静默降级：池空 → 不注入，agent 走 web_search。
    if (news.rss && news.rss.enabled) {
      const pollInterval = Math.max(LIMITS.rssPollMin, Math.min(LIMITS.rssPollMax,
        Number(news.rss.pollMinutes) || LIMITS.rssPollDefault)) * 60 * 1000
      const lastPull = Math.max(poolLastPull || 0, Number(news.pool && news.pool.fetchedAt) || 0)
      if (Date.now() - lastPull >= pollInterval) {
    ...
    if (shift) {
      ...
      const quotaText = units > 1
        ...
        : ''
      // 冗余提示只在「当天该班次已执行过」时附加（去重会剔除已报条目，需要多交候选
      ...
      const redundancyRule = sameShiftToday
        ? `今天本班次已执行过，去重会剔除与已报重复的条目——每个类别按目标条数的 1.5~2 倍提交候选（如每类目标 3 条就交 5~6 条），工具层先去重、再收敛回 ${itemCount} 条。`
        : ''
    ...
    // 信源池注入（RFC §5.2）：按班次范围预筛池条目，作为事实材料附在指令里。
    ...
    if (news.rss && news.rss.enabled && Array.isArray(news.pool && news.pool.items) && news.pool.items.length > 0) {
      // 给池条目补 feed 元信息（filterPoolForScope 用 feedTitle/feedCategory 匹配）。
      const feedById = new Map((news.rss.feeds || []).map((f) => [f.id, f]))
      const enriched = news.pool.items.map((it) => {
        const f = feedById.get(it.feedId)
        return { ...it, feedTitle: f ? f.title : (it.feedTitle || it.feedId), feedCategory: f ? f.category : (it.feedCategory || '') }
      })
    ...
    // 【已报条目】注入：当日已有期次（含本班次历史期次）的条目标题清单——让执行会话的
    ...
    const tick = async () => {
      ...
      for (const s of p.shifts) {
        ...
        if (s.workdaysOnly === true) {
          // 「仅工作日」班次：做判断前若当前年份的工作日历缺失/过期，**此刻才懒拉取**
  ...
  // ---- RSS 信源池：Host 拉取器（RFC docs/news-rss-pool-rfc.md §4）----
  // 与班次定时器解耦：按 rss.pollMinutes（默认 30 分钟）拉取，启动即拉一轮。
  // 拉取是纯 Host 侧 Node fetch（不走 DSH web_fetch，天然免疫 fake-ip 防 SSRF 限制）；
  // 不进 agent 会话、不占 token。池只是「喂料」，整理仍由 agent 在环。
  let poolTimer = null
  let poolInitialTimer = null // 启动后延迟 60s 的首轮拉取
  let poolStartAt = 0 // 本进程最近一次重建定时器的时刻（启动锚点门控）
  let poolLastPull = 0 // 内存水位：最近一次成功拉取时间（重启后由 pool.fetchedAt 兜底）
  ...
  // （路由 /rss/pull、rebuildPoolTimer）负责先 loadNews。
  ...
  const rebuildPoolTimer = async () => {
    await loadNews()
    if (poolTimer !== null) { clearInterval(poolTimer); poolTimer = null }
    if (poolInitialTimer !== null) { clearTimeout(poolInitialTimer); poolInitialTimer = null }
    poolStartAt = Date.now()
    const tick = () => {
      const rss = news.rss
      if (!rss || !rss.enabled || !Array.isArray(rss.feeds) || rss.feeds.length === 0) return
      const interval = Math.max(LIMITS.rssPollMin, Math.min(LIMITS.rssPollMax, Number(rss.pollMinutes) || LIMITS.rssPollDefault)) * 60 * 1000
      // 启动锚点门控：首个完整拉取间隔内不拉（避免测试环境与刚启动时立刻触发）。
      const last = Math.max(poolLastPull || 0, Number(news.pool && news.pool.fetchedAt) || 0, poolStartAt || 0)
      if (Date.now() - last < interval) return
      void pullPoolOnce().catch(() => {})
    }
    poolTimer = setInterval(tick, 30 * 1000) // 与班次定时器同节奏
    // 初始拉取延迟 60s（fresh pool）：不随 apply 同步触发——测试环境会 stub 全局 fetch
    // 并断言调用次数（如 /lyric/online 的本地歌词用例），同步拉 10 个源会污染计数；
    // 60s 远小于任何测试文件时长，产品侧 1 分钟即可用上首轮池数据（面板另有「立即拉取」）。
    poolInitialTimer = setTimeout(() => { void pullPoolOnce().catch(() => {}) }, 60 * 1000)
  }
  ctx.effect(() => () => {
    if (poolTimer !== null) { clearInterval(poolTimer); poolTimer = null }
    if (poolInitialTimer !== null) { clearTimeout(poolInitialTimer); poolInitialTimer = null }
  }, 'music-player: news rss pool timer')

  // ---- 法定节假日 / 调休日历：懒拉取（「仅工作日」班次到点判断时才会在线查询）----
  ...
  // **懒加载原则**：仅当定时器到点判断「仅工作日」班次、且该年份日历缺失/过期时才拉取
  ...
  // 池状态摘要（面板信源池状态行 / 收集指令注入共用）。
  const poolStatus = () => {
    const rss = news.rss
    const feeds = (rss && rss.feeds || []).filter((f) => f && f.enabled)
    const now = Date.now()
    const suspended = feeds.filter((f) => f.suspendedUntil && Number(f.suspendedUntil) > now).map((f) => f.title)
    const fails = []
    for (const [id, list] of poolFeedFailures) {
      const f = (rss && rss.feeds || []).find((x) => x.id === id)
      if (f && list.length > 0) fails.push({ feedId: id, title: f.title, errors: list.slice(-3).map((x) => x.error) })
    }
    return {
      enabled: !!(rss && rss.enabled),
      pollMinutes: (rss && Number(rss.pollMinutes)) || LIMITS.rssPollDefault,
      feedCount: feeds.length,
      poolSize: (news.pool && news.pool.items && news.pool.items.length) || 0,
      fetchedAt: Number(news.pool && news.pool.fetchedAt) || null,
      suspended,
      failures: fails,
    }
  }
  ...
  const serve = async (req, res) => {
    try {
      ...
      // ---- RSS 信源池路由（RFC docs/news-rss-pool-rfc.md §9）----
      // GET  /dsh-music/news/rss        -> 池配置 + 池状态摘要
      // POST /dsh-music/news/rss        -> 保存池配置（增删 feed/改节奏/开关）→ rebuildPoolTimer
      // POST /dsh-music/news/rss/pull   -> 手动立即拉取一轮
      // POST /dsh-music/news/rss/resume -> 手动恢复被自动停用的源（清 suspendedUntil）
      if (pathname === '/dsh-music/news/rss' && req.method === 'GET') {
        await loadNews()
        writeJson(res, { ok: true, rss: news.rss, status: poolStatus() })
        return
      }
      if (pathname === '/dsh-music/news/rss' && req.method === 'POST') {
        await loadNews()
        const body = await readBody(req)
        const prev = news.rss
        const next = sanitizeRssPrefs(body && typeof body === 'object' ? body : {}, prev)
        news.rss = next
        await saveNews()
        void rebuildPoolTimer() // 保存即生效：按新偏好重建拉取定时器
        writeJson(res, { ok: true, rss: next, status: poolStatus() })
        return
      }
      if (pathname === '/dsh-music/news/rss/pull' && req.method === 'POST') {
        await loadNews()
        const r = await pullPoolOnce()
        if (!r.ok) { writeJson(res, { ok: false, error: r.error }, 409); return }
        writeJson(res, { ok: true, ...r, status: poolStatus() })
        return
      }
      if (pathname === '/dsh-music/news/rss/resume' && req.method === 'POST') {
        await loadNews()
        const body = await readBody(req)
        const id = body && typeof body.feedId === 'string' ? body.feedId : ''
        const f = (news.rss && news.rss.feeds || []).find((x) => x.id === id)
        if (!f) { writeJson(res, { ok: false, error: 'feed 不存在' }, 404); return }
        delete f.suspendedUntil
        poolFeedFailures.delete(id)
        await saveNews()
        writeJson(res, { ok: true, rss: news.rss })
        return
      }
  ...
  const newsBroadcastTool = {
    ...
    async execute(args) {
      ...
      // 候选冗余是否开启：仅当「当天已有同班次期次」时才需要（去重会真的剔除条目，
      // 冗余缓冲才能凑满条数）；当天首次执行时去重参照为空、不会剔除任何条目，按原
      // 提交上限（= 班次条数）即可——**不做无谓的 2 倍冗余，避免 token 浪费**。
      const redundant = (() => {
        if (shiftItemCount === null) return false // 对话直接播报：无班次条数约束，不去重冗余
      ...
      // 提交容量上限：开启冗余时 = 班次条数 + 去重冗余缓冲（默认 8 → 16，1.5~2 倍候选），
      // 否则保持班次条数（单类别可超过默认 8 条/类；不冗余时按条数收敛）。
      ...
      // 冷却窗先于去重：同班次 10 分钟内重复提交直接跳过（省去无谓的去重计算；
  ...
  const newsScheduleTool = {
    ...
    async execute(args) {
      ...
      const desc = {
        ...
        calendar: { // 工作日历状态：在线拉取缓存 + 内置表，供「仅工作日」班次判断
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '0) 信源池优先：**每次收集（含对话直接播报）先调 news_schedule {action:"pool"} 获取【信源池材料】**——若返回了池条目，直接采信其 pubDate（时效无需再核），池内与本期范围（类别/主题）匹配的条目优先采用；池内不足或缺少的类别/突发主题，再用 web_search 补盲（补盲查询词一律带当天日期锚定）。定时班次触发时指令里已附【信源池材料】，可不必再调，但重复调用无害。同一事件池内与搜索命中重复时，取分级更高者（官方源 > 权威媒体 > 优质二手 > KOL），同级取池内条目。池条目摘要不足时用 web_fetch 抓原文补细节（受两阶段配额约束）。若 pool 返回「未启用或无条目」，直接走 web_search 即可（池永不成为瓶颈）。',
      '2) 两阶段收集（先快筛再深抓）：阶段一只用 web_search 摘要（与池内 summary）判断「相关 + 值得报」，产生候选清单，此阶段不动 web_fetch；阶段二只对候选清单里「信息量不足 / 最可能缺细节」的条目抓原文——配额每类 ≤2 条、全期 ≤6 条（班次条数较多时可放宽到每类 ≤3、全期 ≤8）。web_fetch 单次报错（如 WEB_BLOCKED_URL「resolves to a non-public IP address」——常见于本机代理 fake-ip DNS 模式）时：确认性重试 1 次仍失败即放弃抓取，该条以搜索摘要为准继续整理，本会话后续条目不再尝试 web_fetch；web_fetch 失败不影响期次生成、不要走失败上报。',
      ...
      '4) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条（工具层另有确定性去重兜底，会剔除与本期/当日重复的条目）；**候选冗余按需**：仅当班次指令明确要求「按目标条数 1.5~2 倍提交候选」时（当天该班次已执行过、需要缓冲去重）才多交候选；指令没提时按目标条数收集即可，不要超量；指令附了【已报条目】清单时，**优先收集未在清单中的新条目**（已报事件勿重复收集），清单不足再补池外/搜索新事件；每类 ≤8 条、全期 ≤20 条；班次若配置了「新闻条数」（1-20，默认 8，触发班次时指令会给出本期条数与多类别平均配额），以该条数为全期上限并按配额在各类别间尽量平均分配（单类别则全部归该类）；每条摘要**建议控制在 200 字以内**（这只是建议而非硬性限制：代码不做任何截断，生成多长就播报多长——请在自然完整的句子处收尾，既保证信息完整又不拖慢单条节奏；过短则信息量不足）；摘要为中性口播内容，讲清事件要点、背景与影响，必标 source，不做猜测性推断；**条目标题不参与播报**（只在面板列表展示），因此每条摘要必须**自含主语**：开头点明「谁/什么事」，再讲进展与影响，让听众不看面板也能听懂每条在讲什么；提交的 categories 必须与班次范围一致（触发班次时指令会给允许的类别名），严禁自行新增范围外的类别（如科技班次里另立「AI」类——AI 内容应归入科技类，除非班次自定义主题声明了 AI）；无班次的对话播报按内容合理分类即可；',
      '5) 调 news_broadcast 提交（用户没说「先别播」时 autoplay 默认 true；静默收集传 false）；同一班次 10 分钟内重复提交会被冷却窗跳过；',
      ...
      '7) 定时由 Host 端自维护：用户在面板「⏰ 每日定时」配置班次（时刻/范围/新闻条数/是否立即播放/是否仅工作日（含法定节假日与周末调休补班）/新闻采集模型）即生效，到点 Host 自动新建一个执行会话去收集并绑定结果，无需 agent 创建/同步 DSH 定时任务；用户要求查看/修改/取消定时时，引导其到面板操作即可。',
    ].join('\n'),
  })
  ...
  void rebuildPoolTimer() // 启动时重建 RSS 信源池拉取定时器（并按节奏补拉一轮）
  // 注：节假日日历不在此拉取——仅在定时器到点判断「仅工作日」班次时懒拉取（见 refreshCalendar）
}
