...
function splitOversize(sentence) {
  ...
  while (start < n) {
    ...
    // Tier 1: last natural clause break inside [start, end) that is outside
    ...
    let pieceEnd
    if (brk > start && brk + 1 - start >= MIN_PIECE_CHARS) {
      pieceEnd = brk + 1
    } else {
      ...
      if (any > start && any + 1 - start >= MIN_PIECE_CHARS) {
        pieceEnd = any + 1
      } else {
    ...
    if (pieceEnd < n && n - pieceEnd < MIN_PIECE_CHARS) {
      ...
      const fits = (j) => j + 1 - start >= MIN_PIECE_CHARS && n - (j + 1) >= MIN_PIECE_CHARS
      let alt = -1
      for (let j = end - 1; j > start; j--) { // prefer a pause outside quotes
        if (fits(j) && !inQuote[j] && isPause(j)) { alt = j; break }
      }
      if (alt === -1) {
        for (let j = end - 1; j > start; j--) { // else any pause, quotes included
          if (fits(j) && isPause(j)) { alt = j; break }
        }
      }
      if (alt > start) pieceEnd = alt + 1
    }
...
export function splitBookChunks(text, breaks = null) {
  ...
  // jump still lands on the chunk that opens the section.
  ...
  const addSentence = (rawSentence) => {
    ...
    // cur + sentence would overflow. If the buffer has grown to a meaningful
    // size, close it and start the sentence in a fresh chunk. But a micro
    // buffer (a short clause left behind by a preceding split) must NOT be
    // flushed on its own — that orphaned 9-19 char chunks like
    // `没有丝毫被打动的样子。` between two ~145-char blocks. Fold it into the
    // sentence and let splitOversize cut the merged text at pauses that keep
    // both sides meaningful instead.
    if (cur.length > 0 && cur.length < MIN_PIECE_CHARS) {
      const merged = cur + sentence
      cur = ''
      pushPieces(splitOversize(merged))
      return
    }
    push()
    if (sentence.length > MAX_TTS_CHARS) pushPieces(splitOversize(sentence))
...
export function apply(ctx) {
  ...
  const disposeExecutionSession = async (sessionId) => {
    if (!sessionId) return
  ...
  const runCollection = async (shiftId) => {
    ...
    const text = (shiftId
      ? `立即执行我的 ${shift.time} 新闻班次（班次 id ${shift.id}）：${nowText}。请收集${scopeText}相关的今日头条——搜索查询词一律用上面的日期锚定当天（如「${nowD.getMonth() + 1}月${nowD.getDate()}日 热搜榜」），整理后提交播报，${playText}。${scopeRule}请按新闻收集流程执行。`
  ...
  const purgeStaleNews = (now = new Date()) => {
    ...
    return (async () => {
      ...
      for (const sid of sessionIds) {
        try { await disposeExecutionSession(sid) } catch { /* best-effort：归档失败不影响清理 */ }
      }
  ...
  const serve = async (req, res) => {
    try {
      ...
      if (pathname === '/dsh-music/lyric' && req.method === 'GET') {
        ...
        if (lrcPath === null) { writeJson(res, { ok: false, hasLrc: false }); return }
        try {
          const lrc = parseLrc(readBufToString(readFileSync(lrcPath)))
          writeJson(res, { ok: true, hasLrc: true, name: basename(lrcPath), lrc })
        } catch (err) {
  ...
  const newsBroadcastTool = {
    ...
    async execute(args) {
      await loadNews()
      const check = sanitizeEditionInput(args, { today: new Date().toISOString().slice(0, 10) })
      if (!check.ok) {
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '6) 定时由 Host 端自维护：用户在面板「⏰ 每日定时」配置班次（时刻/范围/是否立即播放/新闻采集模型）即生效，到点 Host 自动新建一个执行会话去收集并绑定结果，无需 agent 创建/同步 DSH 定时任务；用户要求查看/修改/取消定时时，引导其到面板操作即可。',
    ].join('\n'),
  })
