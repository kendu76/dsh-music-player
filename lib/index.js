...
export function apply(ctx) {
  ...
  const runCollection = async (shiftId) => {
    ...
    // 池材料附在指令末尾：agent 先看池、池不足再 web_search 补盲（提示词 §5.3 有对应规则）。
    const fullText = poolText === '' ? text : text + '\n\n' + poolText
  ...
  // 由 editions 反查「已进入期次的条目 hash」（防同一 url 再次入池）。
  ...
  const newsBroadcastTool = {
    ...
    async execute(args) {
      ...
      // 参照组 = 当日已有期次的全部条目（跨班次/跨手动触发的当日去重）。
      ...
      for (const e of news.editions) {
        ...
        for (const c of (e.categories || [])) {
          for (const it of (c.items || [])) todayRefs.push(it)
        }
      }
      let dedupNote = ''
      if (todayRefs.length > 0 || input.categories.some((c) => c.items.length > 1)) {
        ...
        for (const cat of input.categories) {
          const r = dedupeItemsAgainst(cat.items, todayRefs, { sourceRank })
          cat.items = r.kept
          allKept.push(...r.kept)
        ...
        // official 升级替换：把当日旧期次里被升级掉的参照条目移除（数据层，音频不重生成）。
        if (upgradedRefsAll.length > 0) {
          const upTitles = new Set(upgradedRefsAll.map((r) => r.title))
          let removed = 0
          for (const e of news.editions) {
            if (e.date !== input.date) continue
            for (const c of (e.categories || [])) {
              const before = c.items.length
              c.items = c.items.filter((it) => !upTitles.has(it.title))
