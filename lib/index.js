...
export function apply(ctx) {
  ...
  const runCollection = async (shiftId) => {
    ...
    let countRule = ''
    if (shift) {
      // 班次范围白名单（与 news_broadcast 工具层范围过滤一致：scope.categories ∪
      // scope.topics，自定义主题用同名类别）。⚠ 类别为空 = 纯主题班次，白名单只含主题——
      // 不能把「类别为空」误扩成全部预设类别：否则指令允许的预设类别会在工具层被当
      // 范围外整体过滤，agent 白收集大半、落库只剩主题那几条（曾致 11:00 AI 班次
  ...
  const newsBroadcastTool = {
    ...
    async execute(args) {
      ...
      // 不做每班次期数裁剪：当日期次全保留（跨天由每日 03:00/启动清理 purgeStaleNews 移除）。
