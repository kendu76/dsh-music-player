...
export function apply(ctx) {
  ...
  // ---- 每日新闻播报（news editions / 定时偏好 / 执行态 / 失败日志 / RSS 信源池）----
  ...
  let news = { editions: [], schedulePrefs: sanitizeSchedulePrefs({}), runState: null, failures: [], rss: sanitizeRssPrefs({}), pool: { enabled: false, items: [] } }
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '7) 定时由 Host 端自维护：用户在面板「⏰ 每日定时」配置班次（时刻/范围/新闻条数/是否立即播放/新闻采集模型）即生效，到点 Host 自动新建一个执行会话去收集并绑定结果，无需 agent 创建/同步 DSH 定时任务；用户要求查看/修改/取消定时时，引导其到面板操作即可。',
    ].join('\n'),
  })
