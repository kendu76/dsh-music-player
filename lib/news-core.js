...
import { isCalendarWorkday } from './calendar.js'
...
export const LIMITS = {
  ...
  topicsPerShift: 5,
  shifts: 6,
  // 定时任务新闻条数（单期全期上限，多类别时尽量平均分配）：取值 1-20，默认 8。
  shiftItemCountMin: 1,
  shiftItemCountMax: 20,
  shiftItemCountDefault: 8,
  cooldownMs: 10 * 60 * 1000, // 冷却窗：同定时任务 10 分钟内重复提交跳过
...
/** 规整定时任务「新闻条数」：1-20 的整数，非法/缺失回退默认 8。 */
...
/**
 * 定时任务是否应在给定时刻触发（Host 定时器到点判断）。「仅工作日」定时任务（workdaysOnly）
 * 按「工作日」语义判断：周一至周五为工作日，**扣除法定节假日**（落在工作日也放假不跑），
 * **周末调休补班视为工作日照常跑**；其余定时任务每天到点都触发。
 * calendars 为多年份工作日历（buildCalendar 的产物：{ 'YYYY': { 'MM-DD': type } }），
 * 缺省时仅按「周一至周五」判断（旧行为）。
 * 返回 false 表示本次到点不应触发；true 表示到点正常触发。
 */
export function shiftFiresAt(shift, date, calendars) {
  if (!shift || typeof shift !== 'object') return false
  if (shift.workdaysOnly === true) {
    const yearMap = calendars && calendars[String(date.getFullYear())]
    return isCalendarWorkday(date, yearMap)
  }
  return true
}
...
/**
 * 按定时任务条数预算收敛各类别条目：每类截到其平均配额（多类别尽量平均分配）。
 ...
 *   （定时任务配置了 itemCount 时传入：单类别可超过默认 8 条/类，全期以定时任务条数为上限）。
...
export function renderScript({ title, date, categories, opening, closing }) {
  ...
  // 类别与条目。**单类别期次不播类别引导**——单主题定时任务的开场（如「以下是今日
...
/** 空范围兜底：定时任务收集范围必填（至少一个类别或主题），null/空一律规整为全部预设类别。 */
...
/** 规整定时任务范围：null/空范围兜底为全预设类别（历史数据兼容），其余走白名单过滤。 */
 ...
 * 注：defaultScope（全局默认类别）已退役——定时任务范围必填，不再有「继承默认」语义。
 */
export function sanitizeSchedulePrefs(input, prev) {
  ...
  for (const s of rawShifts) {
    ...
    out.shifts.push({
      ...
      itemCount: normalizeShiftItemCount(s.itemCount), // 定时任务新闻条数：1-20，默认 8
    })
  }
  // 归一化：定时任务按触发时刻升序（HH:MM 定长格式，字典序即时间序），列表展示/工具
...
// Host 侧拉取器（lib/index.js pullPoolOnce）在每次收集执行前懒拉取。
...
/**
 * 按定时任务范围过滤池条目（注入前预筛）：命中规则（RFC §5.2）：
 *   1. feed 主投类别 ∈ scope.categories → 命中；
 *   2. feed 标题 / 条目标题命中任一主题关键词 → 命中；
 *   3. official 级源（一手官方）无条件命中（重大事件通常跨类别）。
 * 已进入某期次的条目（usedIn 非空）直接排除——当天已报过的不再作为候选注入，
 * 减少同类任务多次执行的重复（usedIn 由 markPoolUsed 在 news_broadcast 入库时标记）。
...
export function filterPoolForScope(poolItems, scope, { limit = LIMITS.poolInjectMax } = {}) {
  ...
  const hit = (it) => {
    if (!it) return false
    if (Array.isArray(it.usedIn) && it.usedIn.length > 0) return false // 已报过 → 排除
    // 一手官方源（official 级）无条件命中（RFC §5.2 规则 3）：重大事件通常跨类别，
    // 标题未必带主题关键词。tier 由调用方在 enriched 时从 feed 表补到 feedTier。
    if (String(it.feedTier || '') === 'official') return true
...
/**
 * 工具层去重：把提交的条目与「参照组」（本期次内已通过者 ∪ 参照期次条目）比对，
 * 剔除重复。返回 { kept, dropped }（dropped 为被剔除的原始条目）。
 * @param {object[]} items 提交条目（含 title/summary/source）
 * @param {object[]} refs 参照组条目（参照期次的 items；默认当日已有期次）
 * @param {{ sourceRank?: (s:string)=>number, keepUnseen?: number, maxKeep?: number,
 *           includeRefs?: boolean }} opts
 *   sourceRank：来源分级打分，越高越权威（用于 official 源升级替换：提交条目与参照
 *     重复但更权威时，保留提交条目并标记升级）；
 *   keepUnseen：优先保留与参照组不重复的条目（按原序取前 keepUnseen 条）；与参照重复
 *     但更权威（upgrade）的条目也计为「新条目」保留——多期收集去重后，仍能提交足量
 *     新条目，而不被「当日已报」整体压掉（默认 0 = 关闭，与旧行为一致）；
 *   maxKeep：保留上限（keepUnseen 过滤后若仍超出，按原序截断到 maxKeep；默认不截断）；
 *   includeRefs：是否把已通过去重的参照条目一并写入 refHashes 输出（默认 true——当日
 *     去重需要；关掉时只输出本轮新条目，用于「同一参照组内补足新条目」的递推调用）。
 * @returns {{ kept: object[], dropped: object[], upgrades: number, upgradedRefs: object[],
 *             refHashes: string[] }}
 *   upgradedRefs：被升级替换掉的参照条目（调用方需在旧期次数据层移除）；
 *   refHashes：已通过去重/被参照占用的归一化标题（供多次提交累加参照、把「已报」排除在
 *     条数目标之外——当日多期收集时先提交的批次占住这些标题，后续批次只补新条目）。
 */
export function dedupeItemsAgainst(items, refs, {
  sourceRank = () => 0, keepUnseen = 0, maxKeep = 0, includeRefs = true,
} = {}) {
  if (!Array.isArray(items) || items.length === 0) return { kept: [], dropped: [], upgrades: 0, upgradedRefs: [], refHashes: [] }
  // 参照组按归一化标题去重（同一事件多来源只留一条作为占位）。
  const refNorms = []
  const refNormSet = new Set()
  for (const r of (refs || [])) {
    const n = normalizeTitle(r && r.title)
    if (n === '') continue
    if (!refNormSet.has(n)) {
      refNormSet.add(n)
      refNorms.push({ norm: n, source: r && r.source, item: r })
    }
  }
  const keepNew = Math.max(0, Math.floor(keepUnseen))
  const cap = Math.max(0, Math.floor(maxKeep))
  const seenNorm = new Set() // 本期次内已保留条目的归一化标题
  const consumedRefs = new Set() // 已被升级替换的参照标题（避免同类后续条目重复计升级）
  const kept = []
  const dropped = []
  const upgradedRefs = []
  let upgrades = 0
  for (const it of items) {
    if (!it || typeof it !== 'object') continue
    if (keepNew > 0 && kept.length >= keepNew) { dropped.push(it); continue } // 新条目已凑够 → 其余整批丢弃
    ...
    for (const r of refNorms) {
      ...
      dupRef = r
      // 该参照已被升级替换 → 同类后续条目不再重复升级（但事件仍算已报，照常去重）。
      if (!consumedRefs.has(r.norm)) {
    ...
    if (dupRef && upgradeRef) {
      ...
      consumedRefs.add(dupRef.norm)
    }
    seenNorm.add(norm)
    kept.push(it)
  }
  if (cap > 0 && kept.length > cap) {
    const excess = kept.splice(cap)
    dropped.push(...excess)
  }
  // refHashes：参照组（含被升级占位的）∪ 本轮新条目——多次提交累加参照时，先提交的
  // 批次占住这些标题，后续批次据此只补新条目（把「已报」排除在条数目标之外）。
  const refHashes = includeRefs ? [...refNormSet, ...seenNorm] : [...seenNorm]
  return { kept, dropped, upgrades, upgradedRefs, refHashes }
}
