  ...
  factory: (require) => {
    ...
    function NewsPane({ panelRef }) {
      ...
      const runShiftNowManual = (shift) => {
        ...
        const workdayNote = shift.workdaysOnly ? '（该班次为仅工作日执行：法定节假日除外、周末调休补班照常；手动触发不受限）' : '';
        const text = `立即执行我的 ${shift.time} 新闻班次：收集${scopeText}相关的头条（共 ${itemCount} 条，多类别时尽量平均分配），整理后用 news_broadcast 提交，收集完${shift.autoplay === false ? '先不播放（静默收集）' : '立即播放'}。${workdayNote}`;
