...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function NowPlayingBar() {
      ...
      // 网易云收藏写操作未实现 → nc 曲目不显示爱心钮（toggleFav 亦有兜底）。
      ...
      const heartBtn = hasTrack && !isBook && !ncCur && !isRadio ? React.createElement('button', {
    ...
    // 行点击播放（直播流，无进度），收藏用独立 ♥（存 Host radio.json，不依赖远端账号）。
      ...
      const listEl = (() => {
        ...
        const renderRows = (list) => list.map((st) => {
          ...
          return React.createElement('div', { key: uidOf(st) || st.url, className: cls },
            React.createElement('button', {
              className: 'dsh-music-qq-station-play', title: active ? (s.playing ? '暂停' : '继续播放') : '播放',
              onClick: () => playStation(st),
            }, React.createElement('span', null, active && s.playing ? '❚❚' : '▶')),
            React.createElement('div', { className: 'dsh-music-qq-station-info', onClick: () => playStation(st), title: (st.url || '') },
              React.createElement('div', { className: 'dsh-music-qq-station-name' },
                active ? (s.playing ? '▶ ' : '') : '', st.name || '未命名电台',
              ...
              meta !== '' ? React.createElement('div', { className: 'dsh-music-qq-station-meta' }, meta) : null),
            React.createElement('button', {
              className: 'dsh-music-qq-station-fav' + (faved ? ' on' : ''), title: faved ? '取消收藏' : '收藏',
              onClick: () => toggleFavStation(st),
            }, faved ? '♥' : '♡'),
          );
        });
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-qq-station { display: flex; align-items: center; gap: 6px; width: 100%; box-sizing: border-box; padding: 6px 8px; border-radius: 8px; border: 1px solid transparent; }\n' +
      '.dsh-music-qq-station:hover { background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); }\n' +
      '.dsh-music-qq-station.active { background: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 12%, transparent); border-color: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 35%, transparent); }\n' +
      '.dsh-music-qq-station-play { flex: none; width: 26px; height: 26px; border-radius: 50%; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.3)); background: transparent; color: var(--dsw-alias-label-primary, #e6e6e6); cursor: pointer; font-size: 11px; display: inline-flex; align-items: center; justify-content: center; }\n' +
      '.dsh-music-qq-station.active .dsh-music-qq-station-play { border-color: var(--dsh-music-accent, #2f9e6e); color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-qq-station-info { flex: 1; min-width: 0; cursor: pointer; text-align: left; }\n' +
      '.dsh-music-qq-station-name { font-size: 13px; color: var(--dsw-alias-label-primary, #e6e6e6); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }\n' +
      '.dsh-music-qq-station.active .dsh-music-qq-station-name { color: var(--dsh-music-accent, #2f9e6e); font-weight: 600; }\n' +
      '.dsh-music-qq-station-meta { font-size: 11px; color: var(--dsw-alias-label-secondary, #8a8f98); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 1px; }\n' +
      ...
      '.dsh-music-qq-station-fav { flex: none; border: none; background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; font-size: 15px; line-height: 1; padding: 2px 4px; }\n' +
