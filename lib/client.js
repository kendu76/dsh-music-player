      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (pf === null) {
          ...
          const feedRows = (pf.feeds || []).map((f, i) => {
            ...
            return React.createElement('div', { key: f.id || i, className: 'dsh-music-news-shift-card', style: { alignItems: 'center', flexWrap: 'wrap', gap: 6 } },
              React.createElement('input', {
                value: f.title, placeholder: '源名称', style: { width: 90, padding: '2px 6px' },
                onChange: (e) => setFeed(i, { title: e.target.value }),
              }),
              React.createElement('input', {
                value: f.url, placeholder: 'RSS URL', style: { flex: '1', minWidth: 140, padding: '2px 6px' },
                onChange: (e) => setFeed(i, { url: e.target.value }),
              }),
              React.createElement('select', {
                value: f.tier, style: { padding: '2px 4px' },
                onChange: (e) => setFeed(i, { tier: e.target.value }),
              },
                Object.keys(tierNames).map((t) => React.createElement('option', { key: t, value: t }, tierNames[t]))),
              React.createElement('select', {
                value: f.category, style: { padding: '2px 4px' },
                onChange: (e) => setFeed(i, { category: e.target.value }),
              },
              ...
              suspended ? React.createElement('button', {
                className: 'dsh-music-settings-btn ghost', title: '该源连续失败已自动停用',
                onClick: () => resumeFeed(f.id),
              }, '恢复') : null,
          ...
          body = React.createElement('div', { className: 'dsh-music-news-pane-inner' },
            ...
            React.createElement('div', { className: 'dsh-music-news-body' },
              ...
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, margin: '2px 0 10px' } },
                ...
                React.createElement('input', {
                  ...
                  value: pf.pollMinutes, style: { width: 64, padding: '2px 6px' },
                  onChange: (e) => {
                    const raw = e.target.value;
              ...
              failRows.length > 0 ? React.createElement('div', { style: { marginBottom: 8 } }, failRows) : null,
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '2px 0 6px' } },
                React.createElement('div', { style: { fontWeight: 600 } }, 'RSS 源（' + (pf.feeds || []).length + ' / ' + LIMITS_NEWS.rssFeeds + '）'),
