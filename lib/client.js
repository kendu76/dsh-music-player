...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function bindAudio() {
      ...
      const onPlay = () => { set({ playing: true, error: null }); acquireWakeLock(); if (!String(store.currentId || '').startsWith('book:')) setupLiveViz(); resumeVizCtx(); };
      ...
      const onPlaying = () => { qqErrorSkipCount = 0; if (!String(store.currentId || '').startsWith('book:')) setupLiveViz(); resumeVizCtx(); };
