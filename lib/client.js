    ...
    async function loadTracks() {
      ...
      try {
        ...
        set({
          ...
          kgLoggedIn: !!result.kgLoggedIn, kgNickname: result.kgNickname || '',
          ncLoggedIn: !!result.ncLoggedIn, ncNickname: result.ncNickname || '',
    ...
    function KGOnlinePanel({ panelRef }) {
      ...
      useEffect(() => {
        jsonGet('/dsh-music/kg/status').then((d) => {
          if (d) { setLoggedIn(!!d.loggedIn); setUserid(d.userid || ''); setNickname(d.nickname || ''); }
        }).catch(() => {});
    ...
    function NCOnePanel({ panelRef }) {
      ...
      useEffect(() => {
        jsonGet('/dsh-music/nc/status').then((d) => {
          if (d) { setLoggedIn(!!d.loggedIn); setNickname(d.nickname || ''); }
      ...
      function doLogout() {
        ncPost('/dsh-music/nc/login/logout', {}).then(() => {
          ...
          set({ ncQueue: [], ncSource: '', ncQueueFrom: null, ncQueueRev: 0 });
        }).catch(() => {});
      }
      // ---- 两层 UI：main / playlist ----
      const [layer, setLayer] = useState('main');
