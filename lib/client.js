...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const store = {
      ...
      lyricScan: null,
      // 当前歌词来源（诊断用）：'local'=本地同名.lrc | 'qq-qrc'=QQ逐字 |
      // 'qq'=QQ普通LRC | 'lrclib'=LRCLIB。空串=无歌词/未知。随行下发到
      ...
      fetch('/dsh-music/lyric?path=' + encodeURIComponent(track.path), { cache: 'no-store' })
        ...
        .then((d) => {
          ...
          if (d && d.ok && Array.isArray(d.lrc) && d.lrc.length > 0) {
            musicLyric = d.lrc;
            syncLyricPanelData();
            noteLyricSource('local', d.lrc.length + ' 行（本地同名 .lrc）');
            updateLyric();
            return;
          }
    ...
    // 与 Host 端 news-core.js 对齐的常量（客户端只需要 UI 用到的两个上限与预设类别）。
    const PRESET_CATEGORIES = ['热点', '国内', '国际', '科技', '财经', '体育', '娱乐'];
    const LIMITS_NEWS = { topicsPerShift: 5, shifts: 6 };
      ...
      return React.createElement('span',
        {
          ...
          // data-src：歌词来源（local/qq-qrc/qq/lrclib），DevTools 可直接确认 QRC 是否生效。
          'data-mask': '1',
    ...
    function NewsPane({ panelRef }) {
      ...
      const runShiftNowManual = (shift) => {
        ...
        const text = `立即执行我的 ${shift.time} 新闻班次：收集${scopeText}相关的头条，整理后用 news_broadcast 提交，收集完${shift.autoplay === false ? '先不播放（静默收集）' : '立即播放'}。`;
      ...
      const openShiftEditor = (index) => {
        ...
        setShiftEditor(existing === undefined || existing === null
          ? { index: null, draft: { id: 's' + Math.random().toString(36).slice(2, 8), time: '08:00', autoplay: true, scope: { categories: [], topics: [] } } }
          : { index, draft: { ...existing, scope: existing.scope ? JSON.parse(JSON.stringify(existing.scope)) : { categories: PRESET_CATEGORIES.slice(), topics: [] } } });
      };
      ...
      const failureLine = lastFailure !== null
        ? React.createElement('div', { className: 'dsh-music-error' },
      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (pRaw === null) {
          ...
          // 班次卡片列表：时间 / 类别摘要 / 立即播放开关 / 操作按钮（立即执行·编辑·删除）。
          const shiftCards = (p.shifts || []).map((sh, i) => {
            ...
            return React.createElement('div', { key: sh.id, className: 'dsh-music-news-shift-card' },
              ...
              React.createElement('span', { className: 'dsh-music-news-shift-scope', title: shiftScopeLabel(sh) }, shiftScopeLabel(sh)),
          ...
          const shiftModal = shiftEditor !== null ? (() => {
            ...
            return portalToBody(React.createElement('div', {
              ...
              React.createElement('div', { className: 'dsh-music-picker dsh-music-news-shift-modal', style: panelCenterStyle(panelRef, true, 190, 480), onKeyDown },
    ...
    function PlayerPanel() {
      ...
      // （格式说明排首位 + xiaomi 语音）；新闻播报=单条 xiaomi 语音。多于一条才加序号。
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-news-shift-modal { width: 360px; max-width: 90vw; }\n' +
