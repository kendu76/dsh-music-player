...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const LIMITS_NEWS = {
      ...
      itemCountMin: 1, itemCountMax: 20, itemCountDefault: 8, // 班次新闻条数
      rssFeeds: 30, rssPollMin: 15, rssPollMax: 180, rssPollDefault: 30, // RSS 信源池（RFC）
    };
    ...
    // ---- 新闻播完自动切回原内容 ----
    // 新闻意图到达（班次到点自动播报/面板整期播放）且当前有内容在播时，先快照被打断的
    ...
    function NewsPane({ panelRef }) {
      ...
      const [shiftEditor, setShiftEditor] = useState(null); // 班次编辑弹窗：null=关闭；{index:number|null, draft} 打开（index null=新增）
      const [pool, setPool] = useState(null);            // RSS 信源池：{ rss, status }（GET /news/rss）
      const [poolForm, setPoolForm] = useState(null);    // 信源池配置子视图工作副本
      ...
      useEffect(() => {
        ...
        const tick = () => {
          ...
          fetch('/dsh-music/news/rss', { cache: 'no-store' }).then((r) => r.json()).then((d) => {
            if (!stop && d && d.ok) setPool({ rss: d.rss, status: d.status });
          }).catch(() => {});
        };
        tick();
      ...
      // 进入信源池配置时拉取最新池配置 + 状态（并重置工作副本）。
      useEffect(() => {
        if (view.kind !== 'pool') return;
        setPoolForm(null); // 先清空 → 显示「加载中…」，避免展示旧配置
        loadPool(true);
      }, [view.kind]);
      ...
      const loadSchedule = () => fetch('/dsh-music/news/schedule', { cache: 'no-store' })
        ...
        .catch(() => {});
      // ---- RSS 信源池辅助（RFC docs/news-rss-pool-rfc.md）----
      const loadPool = (alsoForm) => fetch('/dsh-music/news/rss', { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
          if (d && d.ok) {
            setPool({ rss: d.rss, status: d.status });
            if (alsoForm) setPoolForm(JSON.parse(JSON.stringify(d.rss)));
          }
        })
        .catch(() => {});
      // 立即拉取一轮（状态行按钮 / 配置子视图共用）。
      const pullPoolNow = () => {
        fetch('/dsh-music/news/rss/pull', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}),
        }).then((r) => r.json()).then((d) => {
          if (d && d.ok) {
            showToast('已拉取信源池（新增 ' + (d.added || 0) + ' 条）', true);
            setPool((prev) => ({ rss: prev ? prev.rss : d.rss, status: d.status }));
          } else {
            showToast((d && d.error) || '拉取失败', false);
          }
        }).catch(() => showToast('拉取失败', false));
      };
      // 恢复被自动停用的源（配置子视图）。
      const resumeFeed = (feedId) => {
        fetch('/dsh-music/news/rss/resume', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ feedId }),
        }).then((r) => r.json()).then((d) => {
          if (d && d.ok) { showToast('已恢复该源', true); setPool((prev) => (prev ? { ...prev, rss: d.rss } : prev)); }
          else showToast('恢复失败', false);
        }).catch(() => showToast('恢复失败', false));
      };
      // 保存信源池配置（配置子视图）。
      const savePool = (next) => {
        fetch('/dsh-music/news/rss', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(next),
        }).then((r) => r.json()).then((d) => {
          if (d && d.ok) { setPool({ rss: d.rss, status: d.status }); showToast('已保存信源池配置', true); }
          else showToast('保存失败', false);
        }).catch(() => showToast('保存失败', false));
      };
      // 清除收集失败记录（失败提示行「✕」）：后端清空持久化，前端同步清掉本地展示，
      // 提示立即消失（不等 5s 轮询）。与「收集成功自动清空」互补——手动点掉即可。
      const clearFailures = () => {
      ...
      // 离开定时视图时冲刷防抖窗口内未提交的改动（如改完班次立刻点「返回」）。
      useEffect(() => {
      ...
      const runShiftNow = (shift) => {
        // 立即执行（自动）：注入执行指令，agent 立刻按该班次配置跑一轮；失败回退复制。
      ...
      const runShiftNowManual = (shift) => {
        ...
        const text = `立即执行我的 ${shift.time} 新闻班次：收集${scopeText}相关的头条（共 ${itemCount} 条，多类别时尽量平均分配），整理后用 news_broadcast 提交，收集完${shift.autoplay === false ? '先不播放（静默收集）' : '立即播放'}。`;
        copyText(text, '执行指令已复制，发送给 agent 即执行');
      };
      // 班次卡片/弹窗：范围摘要（scope 必填；旧 null/空范围兜底按全部预设类别展示）。
      ...
      // 打开班次编辑弹窗：index=null 表示新增，否则编辑第 index 个班次（深拷贝工作副本）。
      const openShiftEditor = (index) => {
        ...
        setShiftEditor(existing === undefined || existing === null
          ? { index: null, draft: { id: 's' + Math.random().toString(36).slice(2, 8), time: '08:00', autoplay: true, itemCount: LIMITS_NEWS.itemCountDefault, scope: { categories: [], topics: [] } } }
      ...
      // 执行中标签：显示班次触发时刻（如「08:00 班次」）而非内部随机 id——对用户那是乱码
      const runningLabel = (() => {
        if (!running) return '';
      ...
      const failureLine = lastFailure !== null
        ...
        : null;
      // ---- RSS 信源池状态行（RFC §9.2）：常驻工具栏第二行 ----
      const poolStatusLine = (() => {
        if (pool === null || !pool.status) return null;
        const st = pool.status;
        const since = st.fetchedAt ? fmtTs(st.fetchedAt) : '未拉取';
        const susp = (st.suspended && st.suspended.length > 0) ? ' · ⚠ 已暂停 ' + st.suspended.join('、') : '';
        const fail = (st.failures && st.failures.length > 0)
          ? ' · ⚠ ' + st.failures.length + ' 源拉取失败（' + st.failures[0].title + '）' : '';
        const title = st.enabled
          ? ('信源池 ' + st.feedCount + ' 源 · 池内 ' + st.poolSize + ' 条 · ' + since + susp + fail)
          : '信源池未启用';
        return React.createElement('div', { className: 'dsh-music-subtabs', style: { marginBottom: 4, flexWrap: 'nowrap' } },
          React.createElement('button', {
            className: 'dsh-music-subtab',
            title: '信源池配置（RSS 源 / 拉取节奏 / 失败源）',
            style: { textAlign: 'left', flex: '1', minWidth: 0, maxWidth: 'none', overflow: 'hidden', textOverflow: 'ellipsis' },
            onClick: () => setView({ kind: 'pool' }),
          }, '📡 信源池　' + title),
          React.createElement('button', {
            className: 'dsh-music-subtab', title: '立即拉取一轮',
            disabled: !st.enabled,
            onClick: (ev) => { ev.stopPropagation(); pullPoolNow(); },
          }, '⟳'),
        );
      })();
      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (pRaw === null) {
          ...
          // 展示与编辑共用一份「按触发时刻升序」的班次副本（HH:MM 字典序即时间序）：
          ...
          // 班次卡片列表：时间 / 类别摘要 / 新闻条数 / 立即播放开关 / 操作按钮（立即执行·编辑·删除）。
          const shiftCards = (p.shifts || []).map((sh, i) => {
            ...
            return React.createElement('div', { key: sh.id, className: 'dsh-music-news-shift-card' },
              ...
              React.createElement('span', { className: 'dsh-music-news-shift-scope', title: shiftScopeLabel(sh) },
              ...
              React.createElement('div', { className: 'dsh-music-news-shift-actions' },
                ...
                React.createElement('button', {
                  ...
                  title: running ? '收集进行中，请稍候' : '立即执行该班次',
                  disabled: Boolean(running),
                  onClick: () => runShiftNow(sh),
                }, running ? '⟳' : '▶'),
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', title: '编辑班次', onClick: () => openShiftEditor(i) }, '✎'),
                React.createElement('button', { className: 'dsh-music-icon-btn', title: '删除班次', onClick: () => updateForm({ ...p, shifts: p.shifts.filter((x) => x.id !== sh.id) }) }, '🗑'),
              ),
            );
          });
          const shiftList = shiftCount > 0
            ...
            : React.createElement('div', { className: 'dsh-music-news-shift-empty', style: { marginBottom: 8 } }, '还没有班次，点击上方「＋ 添加班次」设置每日定时。');
          ...
          // 班次编辑弹窗（新增/编辑都走这里；portal 到 body，以面板中心为基准居中）。
            ...
            return portalToBody(React.createElement('div', {
              ...
              React.createElement('div', { className: 'dsh-music-picker dsh-music-news-shift-modal', style: panelCenterStyle(panelRef, true, 190, 560), onKeyDown },
                React.createElement('div', { className: 'dsh-music-picker-head' },
                  React.createElement('span', { className: 'dsh-music-picker-title' }, isNew ? '添加班次' : '编辑班次'),
          ...
          body = React.createElement('div', { className: 'dsh-music-news-pane-inner' },
            ...
            React.createElement('div', { className: 'dsh-music-news-body' },
              ...
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '2px 0 6px' } },
                React.createElement('div', { style: { fontWeight: 600 } }, '班次（每天按以下时刻触发）'),
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: () => openShiftEditor(null), disabled: shiftCount >= LIMITS_NEWS.shifts }, '＋ 添加班次'),
              ),
              shiftList,
              shiftModal,
            ),
        ...
        // ---- RSS 信源池配置子视图（RFC §9.2）----
        // 总开关 / 拉取节奏 / feed 列表（增删启停、分级、归属类别）/ 失败与恢复。
        const pf = poolForm || (pool && pool.rss) || null;
        if (pf === null) {
          body = React.createElement('div', { className: 'dsh-music-news-pane-inner' },
            React.createElement('div', { className: 'dsh-music-news-head' }, headBar('信源池配置', { kind: 'list' })),
            React.createElement('div', { className: 'dsh-music-news-body' }, React.createElement('div', { className: 'dsh-music-empty' }, '加载中…')),
          );
        } else {
          const setPf = (patch) => setPoolForm({ ...pf, ...patch });
          const setFeed = (i, patch) => {
            const feeds = pf.feeds.slice();
            feeds[i] = { ...feeds[i], ...patch };
            setPf({ ...pf, feeds });
          };
          const addFeed = () => {
            if (pf.feeds.length >= LIMITS_NEWS.rssFeeds) { showToast('源数量已达上限', false); return; }
            setPf({ ...pf, feeds: pf.feeds.concat([{ id: '', title: '', url: '', tier: 'major', category: '科技', enabled: true }]) });
          };
          const tierNames = { official: '官方源', major: '权威媒体', secondary: '优质二手', kol: 'KOL' };
          const feedRows = (pf.feeds || []).map((f, i) => {
            const suspended = f.suspendedUntil && f.suspendedUntil > Date.now();
            return React.createElement('div', { key: f.id || i, className: 'dsh-music-news-feed-card' },
              // 第一行：源名称 + RSS URL（占满宽度）
              React.createElement('div', { className: 'dsh-music-news-feed-row' },
                React.createElement('input', {
                  className: 'dsh-music-news-feed-input', value: f.title, placeholder: '源名称',
                  style: { width: 120, flex: 'none' },
                  onChange: (e) => setFeed(i, { title: e.target.value }),
                }),
                React.createElement('input', {
                  className: 'dsh-music-news-feed-input', value: f.url, placeholder: 'RSS URL', style: { flex: '1', minWidth: 0 },
                  onChange: (e) => setFeed(i, { url: e.target.value }),
                }),
              ),
              // 第二行：分级 / 类别 / 启用 / 恢复 / 删除
              React.createElement('div', { className: 'dsh-music-news-feed-row', style: { justifyContent: 'space-between' } },
                React.createElement('div', { className: 'dsh-music-news-feed-ctrl' },
                  React.createElement('select', { className: 'dsh-music-news-feed-select', value: f.tier, onChange: (e) => setFeed(i, { tier: e.target.value }) },
                    Object.keys(tierNames).map((t) => React.createElement('option', { key: t, value: t }, tierNames[t]))),
                  React.createElement('select', { className: 'dsh-music-news-feed-select', value: f.category, onChange: (e) => setFeed(i, { category: e.target.value }) },
                    PRESET_CATEGORIES.map((c) => React.createElement('option', { key: c, value: c }, c))),
                  React.createElement('label', { className: 'dsh-music-news-feed-toggle' },
                    React.createElement('input', { type: 'checkbox', checked: f.enabled !== false, onChange: (e) => setFeed(i, { enabled: e.target.checked }) }),
                    '启用'),
                  suspended ? React.createElement('button', { className: 'dsh-music-settings-btn ghost dsh-music-news-feed-resume', title: '该源连续失败已自动停用', onClick: () => resumeFeed(f.id) }, '恢复') : null,
                ),
                React.createElement('button', { className: 'dsh-music-icon-btn', title: '删除源', onClick: () => setPf({ ...pf, feeds: pf.feeds.filter((x) => x !== f) }) }, '🗑'),
              ),
            );
          });
          const st = (pool && pool.status) || {};
          const failRows = (st.failures || []).map((x) => React.createElement('div', { key: x.feedId, className: 'dsh-music-news-failure', style: { marginBottom: 4 } },
            React.createElement('span', { className: 'dsh-music-news-failure-text' }, '⚠ ' + x.title + ' 拉取失败：' + ((x.errors || []).slice(-1)[0] || '')),
          ));
          body = React.createElement('div', { className: 'dsh-music-news-pane-inner' },
            React.createElement('div', { className: 'dsh-music-news-head' }, headBar('信源池配置', { kind: 'list' })),
            React.createElement('div', { className: 'dsh-music-news-body' },
              React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 7, margin: '8px 0 14px', cursor: 'pointer', fontSize: 13 } },
                React.createElement('input', { type: 'checkbox', checked: pf.enabled !== false, onChange: (e) => setPf({ ...pf, enabled: e.target.checked }) }),
                '启用信源池（关 = 完全退回 web_search 补盲）',
              ),
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, margin: '0 0 14px' } },
                React.createElement('div', { style: { fontWeight: 600 } }, '拉取节奏'),
                React.createElement('input', {
                  type: 'number', min: LIMITS_NEWS.rssPollMin, max: LIMITS_NEWS.rssPollMax, step: 1,
                  value: pf.pollMinutes, style: { width: 70, padding: '5px 8px' },
                  onChange: (e) => {
                    const raw = e.target.value;
                    let v = raw === '' ? LIMITS_NEWS.rssPollDefault : Number(raw);
                    if (!Number.isFinite(v)) v = LIMITS_NEWS.rssPollDefault;
                    v = Math.max(LIMITS_NEWS.rssPollMin, Math.min(LIMITS_NEWS.rssPollMax, Math.round(v)));
                    setPf({ ...pf, pollMinutes: v });
                  },
                }),
                React.createElement('span', { style: { fontSize: 11, color: 'var(--dsw-alias-label-secondary, #8a8f98)' } },
                  '分钟（' + LIMITS_NEWS.rssPollMin + '-' + LIMITS_NEWS.rssPollMax + '，默认 ' + LIMITS_NEWS.rssPollDefault + '）'),
              ),
              failRows.length > 0 ? React.createElement('div', { style: { marginBottom: 12 } }, failRows) : null,
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '4px 0 6px' } },
                React.createElement('div', { style: { fontWeight: 600, fontSize: 13 } }, 'RSS 源（' + (pf.feeds || []).length + ' / ' + LIMITS_NEWS.rssFeeds + '）'),
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: addFeed }, '＋ 添加源'),
              ),
              React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 } },
                (pf.feeds || []).length > 0 ? feedRows
                  : React.createElement('div', { className: 'dsh-music-news-shift-empty' }, '还没有源，点击「＋ 添加源」或恢复默认池。')),
              React.createElement('div', { style: { display: 'flex', gap: 8 } },
                React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => { savePool(pf); setPoolForm(null); } }, '保存'),
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: () => setPoolForm(null) }, '取消'),
              ),
            ),
          );
        }
        ...
        body = React.createElement('div', { className: 'dsh-music-news-pane-inner' },
          React.createElement('div', { className: 'dsh-music-news-toolbar' }, statusLine, poolStatusLine, failureLine),
    ...
    function KGOnlinePanel({ panelRef }) {
      ...
      async function loadTopSongs(top) {
        ...
        const d = await json('/dsh-music/kg/top-songs?rankId=' + encodeURIComponent(top.id) + '&offset=0&num=30');
        if (d && d.ok) {
          const t = d.toplist || {};
      ...
      async function loadMoreTopSongs() {
        ...
        const d = await json('/dsh-music/kg/top-songs?rankId=' + encodeURIComponent(topDetail.id) + '&offset=' + topOffset + '&num=30');
        if (d && d.ok) {
      ...
      async function doSearch(kwOverride) {
        ...
        if (sg && sg.ok) { setResults(sg.results || []); setSearchLastLen((sg.results || []).length); setSearchPage(sg.page || 1); }
        else setQError((sg && sg.error) || '歌曲搜索失败');
        if (pg && pg.ok) { setPlResults(pg.playlists || []); setPlSearchLastLen((pg.playlists || []).length); setPlSearchPage(pg.page || 1); }
      ...
      if (browseTab === 'search') {
        ...
        const songMoreBtn = (searchLastLen >= 20)
          ...
          : null;
        const plMoreBtn = (plSearchLastLen >= 20)
          ? React.createElement('div', { className: 'dsh-music-qq-loadmore' },
    ...
    function PlayerPanel() {
      ...
      const ttsHintLines = s.tab === 'book'
        ...
        : (s.tab === 'news'
          ? ['AI语音目前仅支持xiaomi提供方（限时免费），请在设置中配置好再使用此功能。',
             '新闻收集需要 DeepSeek 搜索服务（web_search 使用 DeepSeek 官方 API），请在 DSH 设置中配置好再使用。']
          : (s.tab === 'music' ? ['支持 mp3 / m4a / flac / wav / ogg / opus / aac / webm 等格式，自动递归扫描子目录。'] : null));
        ...
        React.createElement('div', { className: 'dsh-music-whatsnew', role: 'dialog', 'aria-modal': 'true', 'aria-label': title },
          ...
          React.createElement('div', { className: 'dsh-music-whatsnew-body' },
            ...
            showHist && history.length > 0
              ? history.map((h, i) => React.createElement('div', { key: h.version || i, className: 'dsh-music-whatsnew-hist' },
                  React.createElement('div', { className: 'dsh-music-whatsnew-hist-head' },
                    'v' + (h.version || '?') + (h.date ? ' · ' + h.date : '')),
                  (Array.isArray(h.sections) ? h.sections : []).map((sec, j) => React.createElement(WhatsNewSection, { key: j, sec }))))
              : null),
    ...
    function AboutHead() {
      ...
      return React.createElement('div', { className: 'dsh-music-about-top' },
        ...
        React.createElement('div', { className: 'dsh-music-about-desc' },
          s.description || 'DeepSeek Harness 本地音乐 + AI 讲书插件：扫描本地音乐目录流式播放、解析 .txt/.epub 小说经 AI 合成朗读，内置在线 QQ / 酷狗音乐，并注册 music_play 模型工具——对话框里说一句即可点歌 / 听书。'));
    }
    // 关于页内容（卡片组）：运行状态 / 关于 / 免责声明。只渲染卡片本身，
    // 放在 .dsh-music-list 滚动列表内——超高时滚动条出现在卡片区，头部不动。
    function About() {
      const s = useStore();
      ...
      return React.createElement('div', { className: 'dsh-music-about' },
        ...
        React.createElement('div', { className: 'dsh-music-about-note' },
          '在线音乐功能通过非官方接口访问，内容版权归版权方及平台所有，仅供个人学习、技术研究与日常试听，严禁商业用途与二次分发；账号风控与法律风险由使用者自行承担。'));
    }
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-about-note { font-size: 11px; color: var(--dsw-alias-label-tertiary, rgba(255,255,255,0.45)); line-height: 1.7; }\n' +
      ...
      // RSS 信源池 feed 卡片：两行布局（源名称/URL 一行、分级/类别/启用/删除一行），
      // 卡片内留足间距、控件跨行不换行，避免之前单行堆叠导致的拥挤与 URL 截断。
      '.dsh-music-news-feed-card { box-sizing: border-box; width: 100%; display: flex; flex-direction: column; gap: 8px; padding: 10px 12px; background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 10px; }\n' +
      '.dsh-music-news-feed-card:hover { border-color: var(--dsw-alias-border-l2, rgba(128,128,128,0.35)); }\n' +
      '.dsh-music-news-feed-row { display: flex; align-items: center; gap: 8px; min-width: 0; }\n' +
      '.dsh-music-news-feed-ctrl { display: flex; align-items: center; gap: 8px; min-width: 0; flex: 1; }\n' +
      '.dsh-music-news-feed-input, .dsh-music-news-feed-select { box-sizing: border-box; padding: 5px 8px; font-size: 12px; color: var(--dsw-alias-label-primary, #e6e6e6); background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.06)); border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.35)); border-radius: 8px; outline: none; }\n' +
      '.dsh-music-news-feed-input:focus, .dsh-music-news-feed-select:focus { border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-news-feed-select { flex: none; }\n' +
      '.dsh-music-news-feed-toggle { flex: none; display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; white-space: nowrap; padding: 0 2px; }\n' +
      '.dsh-music-news-feed-resume { flex: none; font-size: 12px; padding: 3px 10px; }\n' +
