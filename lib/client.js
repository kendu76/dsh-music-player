...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // estH 为弹层预估高度：上方放不下时（锚点离视口顶只有几像素）自动翻到锚点**下方**，
    // 否则弹层会顶出视口被裁掉（例如播放条被移到很靠上的位置时）。
    // 无目标元素时回退到视口底部中央。
    const anchorAbove = (el, maxW = 380, estH = 0) => {
    ...
    const anchorPopAbove = (el, maxW = 380) => {
      ...
      const availAbove = r.top - 6 - topGap;
      // 上方空间不够（锚点几乎贴着视口顶）→ 翻到锚点下方：
      ...
      const avail = Math.max(120, availAbove);
      return {
        ...base,
    ...
    const PREF_PLAYBACK_NC = 'dsh-music-nc-playback'; // 在线网易云播放进度+队列（独立于其它在线源）
    const PREF_PLAYBACK_RADIO = 'dsh-music-radio-playback'; // 网络电台「当前台」持久化（直播流无进度）
    const PREF_RADIO_HISTORY = 'dsh-music-radio-history'; // 电台搜索历史（最近在前，最多 10 条）
    const RADIO_RETRY_MAX = 3; // 电台加载失败自动退避重试上限（应对 Chromium 对裸 AAC(aacp) 无限流的概率性解码失败等）
    ...
    // 播放条显示位置（系统配置「显示位置」）：'dock'=输入框上方（默认，保持既有行为）
    // | 'bottom'=页面底部 | 'float'=悬浮条（可整条拖动）。悬浮条坐标另存一档（拖到哪记到哪）。
    const PREF_BAR_POS = 'dsh-music-bar-pos';
    const PREF_BAR_FLOAT_POS = 'dsh-music-bar-float-pos';
    ...
    const PREF_NC_HISTORY = 'dsh-music-nc-history'; // 网易云搜索历史（最近在前，最多 10 条）
    ...
    const PREF_KEYS = new Set([
      ...
      PREF_PLAYBACK, PREF_PLAYBACK_QQ, PREF_PLAYBACK_KG, PREF_PLAYBACK_NC, PREF_PLAYBACK_RADIO, PREF_BOOKS_PLAYBACK, PREF_QQ_FAV, PREF_QQ_HISTORY, PREF_QQ_UI, PREF_KG_HISTORY, PREF_NC_HISTORY, PREF_RADIO_HISTORY,
      PREF_SHOW_LYRIC, PREF_SHOW_VIZ, PREF_VIZ_MODE, PREF_SHOW_PROGRESS, PREF_SHOW_QUALITY, PREF_SHOW_BAR_BG, PREF_IMMERSE,
      PREF_LYRIC_FX, PREF_LYRIC_PANEL_GHOST, PREF_SEEN_VERSION, PREF_BAR_POS, PREF_BAR_FLOAT_POS,
    ]);
    ...
    function applyStoredPrefs() {
      try {
        ...
        // 播放条显示位置：白名单校验（脏数据回退 dock=现状）；悬浮坐标校验成对数值。
        const barPos = loadPref(PREF_BAR_POS);
        if (BAR_POS_VALUES.indexOf(barPos) !== -1) store.barPos = barPos;
        const barFloatPos = parseBarFloatPos(loadPref(PREF_BAR_FLOAT_POS));
        if (barFloatPos !== null) store.barFloatPos = barFloatPos;
      } catch (e) {}
    }
    ...
    // 播放条显示位置（系统配置「显示位置」）：
    //   dock   —— 输入框上方（conversation.input.dock，默认，完全保持既有行为）
    //   bottom —— 页面底部（贴窗口底边、对齐会话列宽度；见 useColumnMetrics）
    //   float  —— 悬浮条：三行紧凑卡片（名称 / 歌词独占一行 / 控件常显），整条可拖动，
    //              位置记进 PREF_BAR_FLOAT_POS
    const BAR_POS_VALUES = ['dock', 'bottom', 'float'];
    const BAR_FLOAT_W = 420;   // 悬浮条宽度：三行（名称 / 歌词独占一行 / 控件常显）。
                               // 宽度下限由第三行决定：时长 ~75px + 8 个 24px 圆钮 + 7×8px
                               // 间距 + 内边距 ≈ 351px，420px 留了约 70px 余量（字体/主题差异也够）；
                               // 歌词独占整行后有 400px 可用，中文歌词一行三十来字仍能看全。
    const BAR_FLOAT_EDGE = 16; // 悬浮条未拖动过时的贴边留白（右下角）
    // 页面底部模式：给 DSH 的输入区底座补的下内边距（见 CSS 里 data-dsh-music-bar-bottom
    // 那条规则）。取 44px ≈ 播放条自身高度（~36px）+ 8px 间隙：输入卡片整体上移这么多，
    // 贴底的条因此既压不住输入框、也不遮住任何对话内容。
    const BAR_BOTTOM_PAD = 44;
    // 悬浮条坐标（{x,y}，整数像素）；脏数据回退 null = 用 CSS 贴右下角。
    function parseBarFloatPos(raw) {
    ...
    const store = {
      ...
      kgLoggedIn: false, kgNickname: '',
      ncLoggedIn: false, ncNickname: '',
      ...
      kgAuthDeadMsg: '',
      // QQ 音乐登录态已过期（cookie 还在、music key 失效：歌单照常能看，但 VIP/高音质
      // 曲目取不到播放地址）。由引擎侧播放失败复查 / Host 状态接口置位，QQ 面板据此
      // 显示「重新登录」提示条。false = 正常/未判定。
      qqAuthExpired: false,
      // 酷狗 / 网易云同一口径的「登录已过期」标记（Host 真校验得出，见各家 /status）。
      kgAuthExpired: false,
      ncAuthExpired: false,
      ...
      kgQueueRev: 0,
      qqQueueRev: 0,
      // 在线网易云播放队列 / 来源 / 来源歌单 / 队列来源变更计数（对齐 qq/kg，独立持久化）。
      ncQueue: [], ncSource: '', ncQueueFrom: null, ncQueueRev: 0,
      // 网络电台：当前台对象（{...station}）与收藏/最近（GET /dsh-music/radio/... 拉取）。
      // 电台是直播流：无「队列/进度」，只有「当前台」；radioPlaying 仅在播放时置位，
      // 用于播放条 LIVE 语义（不 seek、不显示时长进度）。
      radioCurrent: null, radioPlaying: false,
      radioFavs: [], radioRecent: [], radioFavLoaded: false,
      ...
      toast: null,
      // 多标签页统一播放器：本页是否处于「镜像态」（声音由另一个标签页负责，本页只
      // 同步显示状态、操作转发过去）。仅用于渲染提示徽标，判定见 xtabMirroring。
      mirroring: false,
      // 播放条显示位置（系统配置「显示位置」）：'dock' | 'bottom' | 'float'。
      // barFloatPos = 悬浮模式的坐标 {x,y}；null = 还没拖过（CSS 贴右下角）。
      barPos: 'dock', barFloatPos: null,
    };
    ...
    function set(patch) {
      ...
      // 播放条显示位置：模式与悬浮坐标都落 Host prefs（跨标签页共享，与面板位置同款语义）。
      if ('barPos' in patch) savePref(PREF_BAR_POS, patch.barPos);
      if ('barFloatPos' in patch && patch.barFloatPos !== null) savePref(PREF_BAR_FLOAT_POS, JSON.stringify(patch.barFloatPos));
      // 多标签页统一播放器：出声页（owner）把可见播放态增量广播给其它页（见 xtabOnStoreChange）。
      xtabOnStoreChange(patch);
    ...
    // ======================================================================
    // 多标签页统一播放器（唯一出声页 + 播放态镜像）
    // ======================================================================
    // 问题：DSH 的 Web GUI 常被同时开好几个标签页。插件在每个页面里各有一份 React
    // 树、一个 <audio> 和一份 store，于是 A 页放着歌、切到 B 页再点一首，两页会
    // **同时出声**（两首不同的歌叠在一起），并且两页各自显示自己的曲目——看起来像
    // 两个互不相干的播放器。
    //
    // 方案：把多页收拢成「一个播放器」，只有一页真正出声（audio owner）：
    //   ① 唯一出声权（owner）。由「第一声真正响起的那页」认领，并用 (Lamport 计数,
    //      标签页 id) 的全序在并发认领时收敛到唯一赢家。其它页转为**镜像态**：
    //      <audio> 暂停 + muted，本地引擎（含讲书定时器/电台重试）停止驱动。
    //   ② 播放态镜像。owner 把「可见播放态」增量广播给其它页（曲目/歌手/进度/时长/
    //      歌词/播放模式/音量/讲书章节/缓冲与错误…），镜像页把它们当自己的 store 用，
    //      因此任取一页看到的都是同一个播放器（曲目同名、进度同步、播放/暂停图标一致）。
    //   ③ 操作统一。镜像页上的任何操作都**不本地起播**，而是转成指令发给 owner 执行：
    //      传输类（播放/暂停、上下一首、上下一章、停止、音量、播放模式、讲书声音、
    //      章节跳转、重试）与内容点播（点歌/点榜单/点歌单/点在线曲目/点电台/点书/
    //      点新闻期次）全部走 xtabForward → owner 的引擎原地执行 → 广播新状态。
    //      这样既不会「两页抢播」，也没有「在无用户手势的页里自动起播被浏览器拦截」
    //      的坑——真正出声的那一页本来就是用户手势发起/长期在播的那一页。
    //   ④ owner 消失（关页 pagehide 立即广播 bye，崩溃/杀进程由心跳 TTL 兜底）后，
    //      其余页把状态保留成「停在最后位置」并解除镜像态；此时任一页按 ▶ /点播都会
    //      就地接管（ownerId 已清空），讲书按 Host 上的共享进度续播。
    // 通道用 BroadcastChannel（同源 + 同浏览器 profile 的所有 DSH 标签页共享）。
    // 浏览器不支持 BroadcastChannel 时整块降级为「每页各自独立」——与加此功能前
    // 完全一致，绝不因为通道缺失让播放器不可用（现有测试环境 jsdom 就是这条路径）。
    const XTAB_CHANNEL = 'dsh-music-player';
    const XTAB_PING_MS = 2000;   // 心跳间隔（只报活；全量状态由 hello 应答/new owner 认领时给）
    const XTAB_TTL_MS = 5000;    // 心跳超时即视为该页已关闭（pagehide 的 bye 更快）
    const XTAB_POS_MS = 250;     // position/bookProgress 这类高频字段的广播节流
    // 跨页同步的「可见播放态」字段白名单。刻意**不含**浏览/面板类状态
    // （tab/panelOpen/scope/subTab/搜索词/歌单详情…）与体积大的在线队列：
    // 前者是各页自己的浏览位置，同步过去会让「在 B 页翻歌单」被 A 页的浏览动作顶掉；
    // 后者只有 owner 执行上/下一首时才需要（指令在 owner 侧执行，队列不用过通道）。
    const XTAB_FIELDS = [
      'currentId', 'currentName', 'currentArtists', 'currentQuality',
      'playing', 'position', 'duration', 'pendingId', 'pendingName',
      'mode', 'volume', 'voice', 'voiceSwitching', 'error',
      // 收藏态：收藏动作打在共享的 Host 账号数据上，但「爱心是否点亮」是本页数组算的，
      // 一并同步才不会出现「出声页已收藏、这页爱心还是灰的」。
      'qqFaved', 'kgFaved',
      'radioCurrent', 'radioPlaying',
      'bookProgress', 'currentSection', 'bookBuffering', 'bookBufferingSince',
      'bookBufferingSilent', 'bookError',
      'lyricText', 'lyricScan', 'lyricSource', 'lyricLines', 'lyricCur',
    ];
    const XTAB_HIGH_FREQ = new Set(['position', 'bookProgress', 'duration']);
    // 特性探测放在模块级：不支持时所有跨页路径都是 no-op，行为与从前逐字节一致。
    const xtabSupported = typeof BroadcastChannel === 'function';
    const xtabId = 'mt-' + Math.random().toString(36).slice(2, 10) + '-' + Date.now().toString(36);
    let xtab = null;             // BroadcastChannel 句柄（不支持时为 null）
    let xtabOwner = null;        // { id, counter } 当前唯一出声页；null = 尚无 owner
    let xtabCounter = 0;         // 本页 Lamport 计数（认领时递增，用于并发收敛）
    let xtabMirroring = false;   // 本页是否镜像态（别的页在出声，本页只显示不出声）
    let xtabRemote = false;      // 正在套用远端状态（抑制回广播）
    let xtabPeers = new Map();   // 其他标签页 id -> 最近心跳时刻
    let xtabPosTimer = null;
    let xtabLastSent = null;     // 上次广播出去的状态快照（用于算增量）

    function xtabPost(msg) {
      if (xtab === null) return;
      try { xtab.postMessage(msg); } catch (e) { /* 通道关闭等：非致命 */ }
    }
    // 清理过期心跳并返回存活页 id。TTL 判定是**懒计算**（读时判断），不依赖定时器，
    // 这样后台页被浏览器节流（心跳变慢）也不会误判——真正要判死只在「转发指令前」。
    function xtabLivePeers() {
      const now = Date.now();
      for (const [id, seen] of [...xtabPeers]) if (now - seen > XTAB_TTL_MS) xtabPeers.delete(id);
      return [...xtabPeers.keys()];
    }
    // 本页是否是当前 owner（未被认领时按「无主」处理，谁先播谁认领）。
    function xtabAudioFree() { return xtabOwner === null || xtabOwner.id === xtabId; }
    // owner 是否还活着（存在且心跳未过期）。owner 是自己时不算「另一个页」。
    function xtabOwnerLive() {
      if (xtabOwner === null || xtabOwner.id === xtabId) return false;
      xtabLivePeers();
      return xtabPeers.has(xtabOwner.id);
    }
    // 当前状态里需要跨页同步的部分（浅拷贝；数组/对象按引用比较算增量）。
    function xtabCollect() {
      const out = {};
      for (const k of XTAB_FIELDS) out[k] = store[k];
      return out;
    }
    function xtabSendState(force) {
      if (!xtabSupported || xtabMirroring) return;
      if (xtabOwner === null || xtabOwner.id !== xtabId) return;
      const full = xtabCollect();
      if (force || xtabLastSent === null) {
        xtabLastSent = full;
        xtabPost({ t: 'state', id: xtabId, full: true, counter: xtabCounter, state: full });
        return;
      }
      const delta = {};
      let n = 0;
      for (const k of XTAB_FIELDS) if (full[k] !== xtabLastSent[k]) { delta[k] = full[k]; n++; }
      if (n === 0) return;
      xtabLastSent = full;
      xtabPost({ t: 'state', id: xtabId, state: delta });
    }
    function xtabScheduleState() {
      if (xtabPosTimer !== null) return;
      xtabPosTimer = setTimeout(() => { xtabPosTimer = null; xtabSendState(false); }, XTAB_POS_MS);
    }
    // set() 的钩子：owner 页的状态变化即时广播（position/bookProgress 这类每帧级
    // 字段节流到 XTAB_POS_MS，其余——切歌/暂停/模式/音量——立即，另一页不该有可感延迟）。
    function xtabOnStoreChange(patch) {
      if (!xtabSupported || xtabRemote || xtabMirroring) return;
      if (xtabOwner === null || xtabOwner.id !== xtabId) return;
      const keys = Object.keys(patch);
      if (keys.length === 0) return;
      if (keys.every((k) => XTAB_HIGH_FREQ.has(k))) { xtabScheduleState(); return; }
      xtabSendState(false);
    }
    // 认领唯一出声权：先递增 Lamport 计数再广播，并发认领按 (counter, id) 字典序收敛。
    function xtabClaim() {
      if (!xtabSupported) return;
      xtabCounter += 1;
      xtabOwner = { id: xtabId, counter: xtabCounter };
      xtabLastSent = null;
      if (xtabMirroring) {
        xtabMirroring = false;
        if (store.mirroring) set({ mirroring: false });
      }
      audio.muted = false;
      xtabPost({ t: 'claim', id: xtabId, counter: xtabCounter, state: xtabCollect() });
    }
    // 就地接管（本页有用户手势、要自己起播）：先解除镜像并认领，再交给本地引擎。
    // 用于「出声页因无手势被浏览器拦截自动播放」这类转发永远解锁不了的场景。
    function xtabTakeOverLocal() {
      if (!xtabSupported) return;
      if (xtabMirroring) {
        xtabMirroring = false;
        if (store.mirroring) set({ mirroring: false });
      }
      if (xtabOwner === null || xtabOwner.id !== xtabId) xtabClaim();
    }
    // 转入镜像态：本页从此不出声（暂停 + 静音 + 拆掉采集探针 + 清讲书定时器）。
    // 必须在 audio.pause() **之前**置位 xtabMirroring——onPause 处理器据此忽略这次
    // 让位产生的 pause 事件，否则它会把镜像态刚拿到（或即将拿到）的 playing:true 抹掉。
    function xtabEnterMirror() {
      if (!xtabSupported || xtabMirroring) return;
      xtabMirroring = true;
      try { audio.pause(); } catch (e) {}
      try { preAudio.pause(); } catch (e) {}
      audio.muted = true;
      // 讲书定时器（看门狗/blob 兜底/预加载）与电台自动重连都可能绕过播放包装器直接
      // 起播，在镜像态下必须停掉；stopBookHelper 会清掉那几个定时器并作废在途异步。
      try { stopBookHelper(); } catch (e) {}
      radioRetryCount = RADIO_RETRY_MAX;
      releaseWakeLock();
      closeLiveViz();
      xtabLastSent = null;
      if (!store.mirroring) set({ mirroring: true });
    }
    // owner 消失（bye / 心跳超时 / 转发时发现已死）：解除镜像态，把状态留在
    // 「停在最后位置」——音乐/在线曲目把位置存进续播定位钉，之后按 ▶ 就从这里继续；
    // 讲书不设定位钉（块状态不可跨页搬运），按 ▶ 走 resumeOrPlayBook 读 Host 上的
    // 共享进度续播（见 togglePlay 的讲书分支）。
    function xtabOwnerGone() {
      if (xtabOwner !== null && xtabOwner.id !== xtabId) xtabOwner = null;
      xtabLastSent = null;
      if (!xtabMirroring) return;
      xtabMirroring = false;
      audio.muted = false;
      const cid = store.currentId === null ? '' : String(store.currentId);
      // 音乐/在线曲目把位置存进续播定位钉（直播电台无进度、讲书走 resumeOrPlayBook）。
      if (cid !== '' && !cid.startsWith('book:') && !cid.startsWith('radio:') && store.position > 0) restoredMusicPos = store.position;
      set({ mirroring: false, playing: false, radioPlaying: false });
    }
    function xtabApplyState(state) {
      if (state === null || typeof state !== 'object') return;
      const patch = {};
      let n = 0;
      for (const k of XTAB_FIELDS) if (Object.prototype.hasOwnProperty.call(state, k)) { patch[k] = state[k]; n++; }
      if (n === 0) return;
      xtabRemote = true;
      try {
        set(patch);
        // 音量在同步白名单里：镜像页的元素本身不出声，但记下来，将来接管出声时
        // 元素音量要与播放条显示的一致（否则条上 40%、实际按 80% 放）。
      ...
      } finally { xtabRemote = false; }
    }
    // 把指令转发给 owner（镜像态下的本地操作入口）。返回 true = 已转发，本地不要执行。
    // 若 owner 已经死了（关页但 bye 丢了/进程被杀），就地解除镜像态并返回 false，
    // 让本地引擎按「无主」路径接管——不会出现「指令发进黑洞、什么都点不动」。
    function xtabForward(cmd, arg) {
      if (!xtabSupported) return false;
      if (xtabOwner === null || xtabOwner.id === xtabId) return false;
      if (!xtabOwnerLive()) { xtabOwnerGone(); return false; }
      xtabPost({ t: 'cmd', id: xtabId, to: xtabOwner.id, cmd, arg });
      return true;
    }
    // owner 侧执行转发来的指令（只有 owner 会走到这里；引擎函数里的转发钩子此时
    // 看到「owner 是自己」直接返回 false，不会形成回环）。
    function xtabRunCmd(cmd, arg) {
      const a = (arg && typeof arg === 'object') ? arg : {};
      if (cmd === 'toggle') { togglePlay(); return; }
      if (cmd === 'pause') { audio.pause(); set({ playing: false }); return; }
      if (cmd === 'resume') {
        const p = audio.play();
      ...
      if (cmd === 'step') { step(Number(a.delta) || 1); return; }
      if (cmd === 'stepBook') { stepBook(Number(a.delta) || 1); return; }
      if (cmd === 'volume') { changeVolume(Number(a.value)); return; }
      if (cmd === 'mode') { set({ mode: a.mode, modeMenuOpen: false }); return; }
      if (cmd === 'voice') { setVoice(a.voice); return; }
      if (cmd === 'retryBook') { retryBook(); return; }
      if (cmd === 'playBook') { playBook(a.id, Number(a.from) || 0); return; }
      if (cmd === 'resumeBook') { resumeOrPlayBook(a.id); return; }
      if (cmd === 'playLocal') { startPlay(a.id); return; }
      if (cmd === 'playFrom') { startPlayFrom(a.id, a.kind, a.plId); return; }
      if (cmd === 'qqPlay') { startQQPlayback(a.song, a.queue, a.label, a.from); return; }
    ...
    function xtabHandle(ev) {
      const m = ev && ev.data;
      if (m === null || typeof m !== 'object' || typeof m.t !== 'string') return;
      if (m.id === xtabId) return; // BroadcastChannel 不回投自己，防御性判断
      const from = typeof m.id === 'string' ? m.id : '';
      if (from === '') return;
      xtabPeers.set(from, Date.now());
      if (m.t === 'hello') {
        xtabPost({ t: 'here', id: xtabId });
        xtabSendState(true); // 非 owner 页这里静默返回；owner 页借此把全量状态交给新页
        return;
      }
      if (m.t === 'here') return; // 只是让对方把我记进 peers
      if (m.t === 'ping') return; // 心跳：上面已按 id 刷新 peers，无需其它处理
      if (m.t === 'bye') {
        xtabPeers.delete(from);
        if (xtabOwner !== null && xtabOwner.id === from) xtabOwnerGone();
        return;
      }
      if (m.t === 'claim') {
        const c = Number(m.counter) || 0;
        const better = xtabOwner === null
          || c > xtabOwner.counter
          || (c === xtabOwner.counter && from > xtabOwner.id);
        if (!better) return;
        xtabOwner = { id: from, counter: c };
        xtabCounter = Math.max(xtabCounter, c);
        xtabEnterMirror();          // 让位：本页（若在播）立即停声
        xtabApplyState(m.state);
        return;
      }
      if (m.t === 'release') { // owner 关页：套用它的收尾状态（playing:false = 停在原地）
        if (xtabOwner !== null && xtabOwner.id === from) {
          xtabOwner = null;
          xtabApplyState(m.state);
          xtabOwnerGone();
        }
      ...
      if (m.t === 'state') {
        // 新开的页此前不知道谁是 owner：owner 的全量状态就是「认领通知书」。
        if (xtabOwner === null && m.full === true) {
          xtabOwner = { id: from, counter: Number(m.counter) || 0 };
          xtabCounter = Math.max(xtabCounter, Number(m.counter) || 0);
          xtabEnterMirror();
        }
        if (xtabOwner !== null && xtabOwner.id === from) xtabApplyState(m.state);
        return;
      }
      if (m.t === 'cmd') {
        if (xtabOwner !== null && xtabOwner.id === xtabId) xtabRunCmd(m.cmd, m.arg);
        return;
      }
    }
    // 关页收尾：owner 把自己交还给其它页，并把状态定格成「暂停在最后位置」——
    // 不能原样广播 playing:true，否则其它页会显示成「在播」却听不到声音。
    function xtabShutdown() {
      if (!xtabSupported) return;
      if (xtabOwner !== null && xtabOwner.id === xtabId) {
        const st = xtabCollect();
        st.playing = false;
        st.radioPlaying = false;
        xtabPost({ t: 'release', id: xtabId, counter: xtabCounter, state: st });
      }
      xtabPost({ t: 'bye', id: xtabId });
      try { xtab.close(); } catch (e) {}
      xtab = null;
    }
    if (xtabSupported) {
      try {
        xtab = new BroadcastChannel(XTAB_CHANNEL);
        xtab.addEventListener('message', xtabHandle);
      } catch (e) {
        xtab = null; // 构造失败（罕见）→ 整块降级为各页独立
      }
    }
    // 播放包装器：**唯一**的「本页要不要出声」闸门。所有引擎路径最终都经过
    // audio.play()（讲解书/在线/电台/本地全部如此），因此这一个点就能保证
    //   镜像页永不出声（拦下并静音） / 出声页起播即认领（广播 owner 与该曲目状态）。
    const xtabRawPlay = audio.play.bind(audio);
    audio.play = () => {
      if (xtabSupported) {
        if (xtabMirroring) {
          audio.muted = true;
        ...
        if (!xtabAudioFree()) xtabClaim(); // 别人是 owner 却本地起播（不该发生）→ 接管
        else if (xtabOwner === null) xtabClaim(); // 无主 → 认领
        audio.muted = false;
      }
      return xtabRawPlay();
    };
    ...
    // 电台「格式」徽章文案（纯函数，供 resolvePlayable / startRadioPlayback 等共用）：
    // HLS 台显示「HLS」（目录 codec 多为 UNKNOWN/空，无意义）；纯流台显示 codec+码率；
    // codec 为空/UNKNOWN 时返回空串（播放条不拼出无意义文本）。
    function radioQualityTxt(st) {
      if (!st) return '';
      if (st.hls) return 'HLS';
      const codec = String(st.codec || '').trim().toUpperCase();
      if (codec === '' || codec === 'UNKNOWN') return '';
      return codec + ((st.bitrate && st.bitrate > 0) ? ' · ' + st.bitrate + 'k' : '');
    }
    ...
    function resolvePlayable(id) {
      ...
      if (String(id).startsWith('radio:')) {
        // 电台：以 uuid 为稳定主键，从「当前台/收藏/最近」里找；刷新恢复时
        // store.radioCurrent 已在 restoreRadioPlayback 里还原。找不到则给出兜底台。
        const uid = String(id).slice(6);
        const st = (store.radioCurrent && String((store.radioCurrent.id || store.radioCurrent.stationuuid || '')) === uid)
          ? store.radioCurrent
          : [...(store.radioFavs || []), ...(store.radioRecent || [])].find((x) => String(x.id || x.stationuuid || '') === uid)
            || (store.radioCurrent || null);
        if (st === null) return null;
        // HLS 台 URL 追加 hls=1（Host 端 /radio/play 据此走转流；.m3u8 后缀的也会自动判别）。
        const hlsQ = st && st.hls ? '&hls=1' : '';
        return {
          id, name: st.name || '网络电台',
          url: '/dsh-music/radio/play?u=' + encodeURIComponent(st.url || '') + hlsQ,
          artists: [],
          quality: radioQualityTxt(st),
        };
      }
      ...
      if (String(id).startsWith('nc:')) {
        // 网易云在线曲目：id 即数字 songid，取链路由按 id 匿名/登录态取链。
        const nid = String(id).slice(3);
        const song = (store.ncQueue || []).find((t) => String(t.id) === nid);
        return { id, name: (song && song.title) || store.currentName || '网易云音乐', url: '/dsh-music/nc/play/' + nid, artists: (song && song.artists) || [] };
      }
    ...
    function activeIds() {
      ...
      if (s.kind === 'nc') return (store.ncQueue || []).map((t) => 'nc:' + String(t.id)); // 网易云在线队列
      // 电台：无队列概念；用「当前台 → 最近 → 收藏」组成一个轮换列表（「下一台」
      // 即切到列表里下一个台；列表内容变化时 currentId 仍在其首）。
      if (s.kind === 'radio') {
        const cur = store.radioCurrent ? 'radio:' + String(store.radioCurrent.id || store.radioCurrent.stationuuid || '') : null;
        const pool = [];
        if (cur) pool.push(cur);
        for (const st of [...(store.radioRecent || []), ...(store.radioFavs || [])]) {
          const id = 'radio:' + String(st.id || st.stationuuid || '');
    ...
    function scopeKey() {
      ...
      if (s.kind === 'nc') return 'nc';
      if (s.kind === 'radio') return 'radio';
    ...
    function isCurrentFaved() {
      ...
      // 网络电台：当前台是否在 store.radioFavs（Host /radio/favs 本地收藏）。
      if (store.currentId !== null && String(store.currentId).startsWith('radio:')) {
        const st = store.radioCurrent || radioIdToStation(store.currentId);
        if (!st) return false;
        const key = radioKeyOf(st);
        if (key === '') return false;
        return (store.radioFavs || []).some((f) => radioKeyOf(f) === key);
      }
    ...
    // 电台稳定键：uuid 优先，手动台（无 uuid）以 url 兜底（与面板 favedOf 一致）。
    function radioKeyOf(st) {
      return String((st && (st.id || st.stationuuid)) || '') || String((st && st.url) || '');
    }
    ...
    // 「登录已过期」标记复位（重新登录成功后调用，让 About/面板回到正常态）。
    function resetKgAuthExpired() { if (store.kgAuthExpired) set({ kgAuthExpired: false }); }
    // QQ 同款：播放失败判定「疑似登录失效」→ 置标记，QQ 面板的 [s.qqAuthExpired] effect 消费后
    // 切回登录 UI（两个登录入口由用户自己选）。msg 不进 store（文案由引擎那侧直接给 error）。
    function markQQAuthExpired() { set({ qqAuthExpired: true }); }
    function resetQQAuthExpired() { if (store.qqAuthExpired) set({ qqAuthExpired: false }); }
    ...
    function checkKgAuthAfterPlayFail() {
      ...
      fetch('/dsh-music/kg/status', { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
          if (!d || !String(store.currentId || '').startsWith('kg:')) return;
          // 判据只有「服务端是否已登出」——酷狗取链撞 20017/20018 时 kgWithFreshToken 会清会话，
          // 这是使用中发现的事实，不是主动探测。
          if (!d.loggedIn) { set({ kgAuthExpired: true }); markKgAuthDead('酷狗登录已失效（登录态与设备不匹配），请重新扫码登录'); }
        })
        .catch(() => {})
        .finally(() => { kgPlayFailCheckInFlight = false; });
    }
    // 在线曲目**整列都播不出来**时，把该家标记为「疑似登录失效」→ 面板切回登录 UI。
    // 判据只用「实际播放失败」这一个事实：**不做任何主动探测**（探测判不准会误报，
    // 而误报会把正常登录的用户踢回登录页）。单曲失败不走这里——那多半是版权/VIP 限制，
    // 由既有逻辑自动跳下一首，不该把用户丢回登录页。
    function markOnlineAuthFailed(kind, label) {
      const msg = label + '无法播放（可能是登录已失效），请重新登录';
      if (kind === 'qq') markQQAuthExpired();
      else if (kind === 'kg') set({ kgAuthExpired: true });
      else set({ ncAuthExpired: true });
      set({ error: msg, playing: false });
    }
    function resetNCAuthExpired() { if (store.ncAuthExpired) set({ ncAuthExpired: false }); }
    ...
    // 在线酷狗曲目走酷狗「我喜欢」；网络电台走 Host 本地收藏夹（/radio/favs）。
    function toggleFav() {
      ...
      if (store.currentId !== null && String(store.currentId).startsWith('nc:')) return; // 网易云收藏写操作未实现（爱心钮已隐藏，此处兜底）
      if (store.currentId !== null && String(store.currentId).startsWith('radio:')) { toggleRadioFav(); return; }
      const path = currentTrackPath();
      if (path === null) return;
      const fav = playlistById(FAV_PLAYLIST_ID);
      if (fav === null) return;
    ...
    // 播放条爱心：收藏/取消收藏当前电台（调 Host /radio/favs，成功后同步 store.radioFavs）。
    function toggleRadioFav() {
      const st = store.radioCurrent || radioIdToStation(store.currentId);
      if (!st || !st.url) return;
      const faved = isCurrentFaved();
      fetch('/dsh-music/radio/favs', {
        method: 'POST', cache: 'no-store',
        headers: { 'content-type': 'application/json' },
    ...
    function startPlayFrom(id, kind, plId) {
      // 多标签页：镜像态下「点某首歌」= 让出声页切到这首歌（scope 也一并带过去，
      // 出声页的队列才是决定上/下一首的那份），本页不出声也不改自己的浏览位置。
      if (xtabForward('playFrom', { id, kind, plId })) return;
    ...
    function savePlayback() {
      ...
      if (String(store.currentId).startsWith('radio:')) {
        // 电台是直播流：不存进度，只存「当前台对象」（station 含 url/id/name 等），
        // 刷新后恢复为暂停态、点 ▶ 重新拉流。
        const st = store.radioCurrent;
        if (!st || !st.url) return;
        savePref(PREF_PLAYBACK_RADIO, JSON.stringify({ station: st, ts: Date.now() }));
        return;
      }
      ...
      if (String(store.currentId).startsWith('nc:')) {
        // 网易云在线曲目：保存曲目+队列+进度（id 是稳定主键，恢复后重新取链播放）。
        const nid = String(store.currentId).slice(3);
        const song = (store.ncQueue || []).find((t) => String(t.id) === nid);
        if (!song) return;
        savePref(PREF_PLAYBACK_NC, JSON.stringify({
          id: store.currentId, name: store.currentName,
          artists: store.currentArtists || [],
          position: (restoredMusicPos !== null && restoredMusicPos > 0) ? restoredMusicPos : (audio.currentTime || 0),
          duration: Number.isFinite(audio.duration) ? audio.duration : 0,
          queue: store.ncQueue || [], source: store.ncSource || '在线',
          queueFrom: store.ncQueueFrom || null,
          ts: Date.now(),
        }));
        return;
      }
    ...
    function loadNCPlayback() {
      const raw = loadPref(PREF_PLAYBACK_NC);
      if (raw === null) return null;
      try { const p = JSON.parse(raw); if (p && typeof p.id === 'string') return p; } catch (e) {}
      return null;
    }
    ...
    function loadLyricForTrack(id) {
      resetLyric();
      lyricTrackId = id;
      // 电台直播流无歌词：跳过（避免走本地 path 查询报错）。
      if (String(id).startsWith('radio:')) return;
    ...
    // ---- 「无人在场」判定：熄屏/锁屏/页面不可见时不要自动起播 ----
    // 定时新闻播报在熄屏期间自动开播有两个后果（pmset 日志实测）：
    //   ① 对着空房间把整期播完（9/18 10:01:24–10:05:45，屏幕 02:06 就灭了；
    //      9/17 10:01:04–10:04:35 同理）；
    //   ② 块与块之间反复 pause → 换 src → play，播放期的 screen wake lock 随之
    //      「释放/重新申请」（pmset 里就是 Blink Wake Lock 每 20~45 秒创建一次），
    //      音频设备跟着进出低功耗态，解锁瞬间那几声「咚咚」即由此而来。
    // 判定用两个信号，缺一不可：
    //   pageHidden()    —— 后台标签/最小化（document.visibilityState !== 'visible'）。
    //   framesStalled() —— 页面长时间没有渲染帧。macOS 熄屏/锁屏时 Chrome 并不会把
    //     页面置为 hidden（这也是锁屏后网页还能继续放音的原因），但合成器停了 →
    //     requestAnimationFrame 停摆，用「距上一帧超过 FRAME_STALL_MS」兜住这部分
    //     场景（窗口被完全遮挡时 rAF 同样会停，一并按「无人观看」处理）。
    // 从未收到过帧（lastFrameAt === 0，例如测试环境 rAF 是空实现）视为未停摆，
    // 保持既有行为，不误判。
    const FRAME_STALL_MS = 5000;
    let lastFrameAt = 0; // 最近一次渲染帧时刻（0 = 还没有过帧）
    const pageHidden = () => {
      if (typeof document === 'undefined') return false;
      if (typeof document.visibilityState === 'string') return document.visibilityState !== 'visible';
      return document.hidden === true;
    };
    const framesStalled = () => lastFrameAt !== 0 && (Date.now() - lastFrameAt) >= FRAME_STALL_MS;
    // 自动起播守卫（只拦「自动」起播：宿主推送的播放意图；面板上点播/点 ▶ 属人工
    // 操作，不受限）。本来就有内容在播（store.playing）视为「人在场」——熄屏听
    // 音乐/讲书时到点插播新闻是既定行为（snapshotNewsResume 播完切回原内容），
    // 不在这里拦。
    const shouldDeferAutoplay = () => !store.playing && (pageHidden() || framesStalled());
    ...
    function startRaf() {
      ...
      const tick = () => {
        if (!rafRunning) return;
        // 渲染帧心跳：熄屏/锁屏/页面被遮挡时合成器停发帧（rAF 不再被调用），
        // framesStalled() 靠这个时间戳判定「无人在场」（见上）。
        const resumedFromStall = lastFrameAt !== 0 && (Date.now() - lastFrameAt) >= FRAME_STALL_MS;
        lastFrameAt = Date.now();
        // 画面从停摆恢复（人回来了：解锁/切回前台）：若还在播，补回播放期的
        // screen wake lock —— 停摆期间我们不申请它（申请也点不亮屏幕，只会让
        // 系统与音频设备在低功耗与防熄屏之间来回切换）。
        if (resumedFromStall && store.playing) acquireWakeLock();
        rafId = requestAnimationFrame(tick); drawViz(); karaokeFrame();
      };
      tick();
    }
    function stopRaf() {
    ...
    function startPlay(id) {
      // 兜底转发（正常都在 startPlayFrom/播放包装器那一层处理）：镜像页永不出声。
      if (xtabForward('playLocal', { id })) return;
      const track = resolvePlayable(id);
      ...
      // 网易云同理：startPlay 是切歌/自动续播/上下曲的通用路径，不走 startNCPlayback，
      // 必须在这里也补一次轻量 HEAD 探测「真实品质」，否则自动切到下一首/上下曲时
      // 音质徽章只显示「网易云」、不显示 无损/高音质/标准（网易云曲目无本地 path）。
      if (String(id).startsWith('nc:')) {
        loadNCQuality(String(id).slice(3), track.url);
      }
    ...
    function retryBook() {
      // 镜像态下「重试」针对的是出声页那份合成失败的块，转发过去重试。
      if (xtabForward('retryBook')) return;
      const id = currentBookId();
      if (id !== null) {
        unlockAutoplay();
    ...
    function togglePanel() {
      ...
      if (opening) {
        ...
        const isNC = cid !== null && String(cid).startsWith('nc:');
        const isRadio = cid !== null && String(cid).startsWith('radio:');
        ...
        set({ tab: isNews ? 'news' : (isBook ? 'book' : (isQQ ? 'qq' : (isKG ? 'kg' : (isNC ? 'nc' : (isRadio ? 'radio' : 'music'))))) });
      }
    ...
    function resumeOrPlayBook(id) {
      // 镜像态下「续播这本书」必须由出声页执行：续播点（块号/块内位置）是块态的，
      // 搬不过来，出声页手里的那份才是准的。
      if (xtabForward('resumeBook', { id })) return;
      const book = bookById(id);
      if (book === null) return;
    ...
    function snapshotNewsResume(newsId) {
      ...
      if (s.startsWith('nc:')) return { newsId, kind: 'nc' };
      if (s.startsWith('qq:')) return { newsId, kind: 'qq' };
      // 电台：直播流无 position，只记「当前台对象」即可（stop 会清 radio 持久化，
      // 因此恢复不能走 restoreRadioPlayback 读盘，需快照时把 station 直接带出来）。
      if (s.startsWith('radio:')) {
        const st = store.radioCurrent || radioIdToStation(cid);
        if (st && st.url) return { newsId, kind: 'radio', station: st };
        return null; // 电台对象丢失 → 放弃恢复
      }
    ...
    function resumeAfterNews() {
      ...
      // 电台恢复：先取出 station（stop 会清 radio 持久化，不能依赖读盘）。
      const radioStation = snap.kind === 'radio' ? (snap.station || null) : null;
      stop();
      if (snap.kind === 'book') { resumeOrPlayBook(snap.id); return true; }
      if (snap.kind === 'radio' && radioStation) {
        // 直播流重新拉流播放（无 position 概念，从头听；保留该台）。
        unlockAutoplay();
        startRadioPlayback(radioStation);
        return true;
      }
      ...
      else if (snap.kind === 'nc') restoreNCPlayback();
    ...
    function maybeAdvanceBook() {
      ...
      if (bookFromRef + 1 < bookTotal) {
        ...
        // 字幕也不动，又没有任何反馈）。
        // warmed 的语义修正：不能只看「预取请求已发出」（preloadBook 同步置位
        // bookBufferedFrom），还要确认 preAudio 真的拿到了可播数据——否则预取请求
        // 卡在慢合成（如 TTS 上游 67s 才返回）时会被误判为「已预热」而静默切块，
        // 用户看到的就是无提示的卡住。真实 <audio> 用 readyState/buffered 判断；
        // jsdom/FakeAudio 无媒体能力（二者 undefined）→ 按就绪处理，保持既有语义。
        const preloadActuallyReady = () => {
          const rs = preAudio.readyState;
          const buf = preAudio.buffered;
          if (rs === undefined && buf === undefined) return true; // 测试环境回退
          return (Number(rs) >= 1) || (buf !== null && buf !== undefined && buf.length > 0);
        };
        const warmed = bookBufferedFrom === bookFromRef + 1 && preloadActuallyReady();
    ...
    function togglePlay() {
      ...
      // 变成「本页起播」，所以转成指令让 owner 原地切暂停/续播。
      // 例外：当前是「自动播放被拦截、点了 ▶ 才能播」的待播态——这时必须由**有用户
      // 手势的这一页**自己起播，转发给没有手势的出声页永远解锁不了（Chrome 的
      // sticky activation 按文档算），所以改成就地接管。
      const pendingBlocked = store.pendingId !== null && store.currentId === null;
      if (!pendingBlocked && xtabForward('toggle')) return;
      if (pendingBlocked) xtabTakeOverLocal();
      ...
      if (audio.paused) {
        ...
        // 刷新恢复的网易云曲目不经过 startNCPlayback，同样补一次流地址加载 + 轻量
        // HEAD（与 QQ/KG 同款）：否则恢复态点 ▶ 时 <audio> 无 src（网易云无本地
        // path、也不能沿用旧流地址，每次经代理按 id 重新取链），播不出声；HEAD 让
        // 「真实品质」标签（X-DSH-NC-Quality）在续播后也能显示。
        if (String(store.currentId).startsWith('nc:')) {
          const track = resolvePlayable(store.currentId);
          if (track !== null && audio.currentSrc !== new URL(track.url, window.location.href).href) {
            audio.src = track.url;
          ...
          if (track !== null) loadNCQuality(String(track.id).slice(3), track.url);
        }
        ...
        // 跨页接管讲书：曲目来自「另一个标签页在放讲书」，本页从没加载过任何块
        // （bookRestorePos<0）——按 Host 上的共享进度续播（与点书单里的书同一条路径；
        // 块状态不跨页搬运，进度每 5s 由出声页写进共享的 Host prefs）。
        if (bookRestorePos < 0 && String(store.currentId).startsWith('book:')) {
          const adoptBookId = currentBookId();
          if (adoptBookId !== null) { resumeOrPlayBook(adoptBookId); return; }
        }
        // 电台（radio:）刷新恢复续播：restore 只还原了「当前台」未动 <audio>，
        // 点 ▶ 需重新挂流地址（直播流每次经代理重新获取）。
        if (String(store.currentId).startsWith('radio:')) {
          const st = store.radioCurrent || radioIdToStation(store.currentId);
          if (st && st.url) {
            const hlsQ = st && st.hls ? '&hls=1' : '';
            const url = '/dsh-music/radio/play?u=' + encodeURIComponent(st.url) + hlsQ;
            if (audio.currentSrc !== new URL(url, window.location.href).href) {
              closeLiveViz();
              audio.src = url;
              audio.load();
              setupLiveViz();
            }
            set({ radioPlaying: true });
          }
        }
        const promise = audio.play();
        if (promise !== undefined && typeof promise.then === 'function') {
    ...
    function step(delta) {
      if (xtabForward('step', { delta })) return;
      const ids = activeIds();
    ...
    function stepBook(delta) {
      if (xtabForward('stepBook', { delta })) return;
      const id = currentBookId();
      if (id === null) return;
    ...
    function changeVolume(value) {
      // 镜像态下音量归出声页：转发过去（音量在同步白名单里，owner 改完会广播回来）。
      if (xtabForward('volume', { value })) return;
      const v = Math.min(1, Math.max(0, value));
      audio.volume = v;
      set({ volume: v });
    }
    // 播放模式（单曲/顺序/乱序）切换。镜像态下必须转发：模式决定出声页的「下一首」
    // 怎么走，而且 mode 在同步白名单里——本地直接 set 会被 owner 的下一次广播顶掉。
    function changeMode(mode) {
      if (xtabForward('mode', { mode })) { set({ modeMenuOpen: false }); return; }
    ...
    function setVoice(voice) {
      // 镜像态下换声音归出声页（新声音要重新合成并热切换当前块，必须由持有该块的页做）。
      if (xtabForward('voice', { voice })) return;
    ...
    function stop() {
      // 多标签页：镜像态下「停止」由出声页执行（本地 stop 只会清掉本页的显示状态，
      // 出声页会继续播，两边显示立刻不一致）。
      if (xtabForward('stop')) return;
      ...
      if (store.currentId !== null) {
        if (String(store.currentId).startsWith('radio:')) stoppedMusicKey = PREF_PLAYBACK_RADIO;
        else if (String(store.currentId).startsWith('qq:')) stoppedMusicKey = PREF_PLAYBACK_QQ;
        else if (String(store.currentId).startsWith('kg:')) stoppedMusicKey = PREF_PLAYBACK_KG;
        else if (String(store.currentId).startsWith('nc:')) stoppedMusicKey = PREF_PLAYBACK_NC;
      ...
      stopBookHelper();
      bookAutoRetried = false;
      set({ currentId: null, currentName: null, playing: false, position: 0, duration: 0, pendingId: null, pendingName: null, bookBuffering: false, bookError: '', bookBufferingSince: 0, bookBufferingSilent: false, tocOpen: false, currentSection: '', lyricText: '', voiceSwitching: false, bookProgress: 0, radioPlaying: false });
      resetLyric();
    ...
    async function acquireWakeLock() {
      // 熄屏/锁屏/页面被遮挡时不申请：屏幕已经灭了，申请点不亮它，反而让系统与
      // 音频设备在「低功耗 ↔ 防熄屏」之间反复切换（每次切块 play 都申请一次，
      // pmset 里就是那串每 20~45 秒一次的 Blink Wake Lock 创建/释放）。
      // 判定见 pageHidden/framesStalled；渲染恢复时由 startRaf 的心跳补申请。
      if (!wakeLockSupported || !store.playing || pageHidden() || framesStalled()) return;
      if (wakeLock !== null) return; // already held
      try {
        const sentinel = await navigator.wakeLock.request('screen');
        // request 是异步的：await 期间页面可能已转为 hidden / 合成器已停摆（熄屏），
        // 那就立刻还回去，不持有。
        if (pageHidden() || framesStalled()) { try { sentinel.release(); } catch (e) {} return; }
    ...
    function bindAudio() {
      ...
      const onTime = () => {
        // 多标签页镜像态：本页元素已让位暂停，绝不能再用它的时间/时长/歌词去覆盖
        // 出声页广播过来的状态（那会让两页显示各自不同的进度）。
        if (xtabMirroring) return;
      ...
      const onDur = () => {
        if (xtabMirroring) return; // 镜像态：时长以出声页广播的为准
      ...
      const onPlay = () => { if (xtabMirroring) return; set({ playing: true, error: null }); acquireWakeLock(); if (!String(store.currentId || '').startsWith('book:')) setupLiveViz(); resumeVizCtx(); };
      const onPause = () => { if (xtabMirroring) return; set({ playing: false }); savePlayback(); releaseWakeLock(); };
      const onEnded = () => {
        if (xtabMirroring) return; // 镜像态：绝不自动切下一首（切歌由出声页决定）
        ...
        // 电台直播流被服务器掐断（ended）：尝试自动重连当前台（30s 内至多一次）。
        if (String(store.currentId).startsWith('radio:')) {
          if (Date.now() - lastRadioReconnectAt < 30000) { stop(); return; }
          lastRadioReconnectAt = Date.now();
          unlockAutoplay();
          startRadioPlayback(store.radioCurrent);
          return;
        }
      ...
      const onError = () => {
        if (xtabMirroring) return; // 镜像态：错误提示由出声页广播过来
        ...
        } else if (store.currentId !== null && String(store.currentId).startsWith('radio:')) {
          // 电台加载失败：常见原因是瞬时网络抖动 / 上游 Icecast 偶发掐断 / HLS 分片滚动边界 /
          // Chromium 对部分裸 AAC(aacp) 无限流解码器初始化概率性失败（如 AsiaFM高清音乐台，
          // 表现为「偶发失败、再点就好」）。对策：自动退避重试至多 RADIO_RETRY_MAX 次
          //（每次 startRadioPlayback 重置计数），仍失败才提示。不自动切台（避免连环跳台）。
          if (radioRetryCount < RADIO_RETRY_MAX) {
            radioRetryCount++;
            const retryStation = store.radioCurrent || radioIdToStation(store.currentId);
            const failedId = store.currentId;
            if (retryStation) {
              set({ playing: false, radioPlaying: false, error: null });
              const delay = 600 * radioRetryCount; // 退避：0.6s / 1.2s / 1.8s
              setTimeout(() => {
                // 仅在「仍是这个电台且未在播其它内容」时重试；用户已停止/换台则不打扰。
                const cur = store.currentId;
                if (cur === null || String(cur) !== String(failedId) || store.playing) return;
                unlockAutoplay();
                startRadioPlayback(retryStation);
              }, delay);
              return;
            }
          }
          lastRadioReconnectAt = Date.now();
          set({ error: '电台播放失败（台站可能已失效或格式不受支持），请换一个电台试试', playing: false, radioPlaying: false });
        } else {
          // 在线队列（QQ/酷狗/网易云）：某首歌因版权下架/拿不到播放地址而加载失败时，自动
          // 跳到下一首继续播放（不因单曲失败中断整个队列）；只有队列里就这一
          // 首、或连续跳过次数已达队列长度（整列都试过）才停下报错——且停止后
          // 不再 step，杜绝无限循环跳歌。
          const cidKind = String(store.currentId || '');
          const onlineQueueLen = cidKind.startsWith('kg:') ? (store.kgQueue || []).length
            : cidKind.startsWith('nc:') ? (store.ncQueue || []).length
            : (store.qqQueue || []).length;
          const isOnlineTrack = store.currentId !== null
            && (cidKind.startsWith('qq:') || cidKind.startsWith('kg:') || cidKind.startsWith('nc:'));
          // 酷狗取链失败：服务端在业务报 20017/20018 时**已经**清了会话（使用中发现失效），
          // 这里复查一次 /kg/status，看到 loggedIn:false 就把面板切回扫码登录页。
          if (isOnlineTrack && cidKind.startsWith('kg:')) checkKgAuthAfterPlayFail();
          // 还能跳下一首 → 继续（单曲失败多半是版权/VIP 限制，不该把用户丢回登录页）。
          if (isOnlineTrack && onlineQueueLen > 1 && qqErrorSkipCount < onlineQueueLen) {
            qqErrorSkipCount++;
            step(1);
            return;
          }
          // 整列（或单曲）都播不出来：这是**实际使用中发现播不了**的唯一情形——若该家原本
          // 已登录，就提示「可能登录已失效」并把面板切回登录 UI（用户重登后仍播不了，
          // 那就是版权限制，文案里已写明「可能」）。
          if (isOnlineTrack && cidKind.startsWith('qq:') && store.qqLoggedIn) {
            markOnlineAuthFailed('qq', 'QQ 音乐');
            return;
          }
          if (isOnlineTrack && cidKind.startsWith('kg:') && store.kgLoggedIn) {
            markOnlineAuthFailed('kg', '酷狗音乐');
            return;
          }
          if (isOnlineTrack && cidKind.startsWith('nc:') && store.ncLoggedIn) {
      ...
      // 「在线曲目失败自动跳下一首」的计数只在**真实起播**（playing：已有数据、真正开始播）
    ...
    function restoreScope(plists) {
      ...
      try {
        ...
        if (o && o.kind === 'nc') { set({ scope: { kind: 'nc' } }); return; }
        if (o && o.kind === 'radio') { set({ scope: { kind: 'radio' } }); return; }
    ...
    // Restore online-NetEase playback（PREF_PLAYBACK_NC）——与 QQ/KG 版同构，作用域 kind:'nc'。
    function restoreNCPlayback(saved) {
      if (!saved) saved = loadNCPlayback();
      if (saved === null) return;
      const pos = Number.isFinite(saved.position) && saved.position > 0 ? saved.position : 0;
      const savedDur = Number.isFinite(saved.duration) && saved.duration > 0 ? saved.duration : 0;
      set({
        currentId: saved.id,
        currentName: saved.name || '网易云音乐',
        currentArtists: Array.isArray(saved.artists) ? saved.artists : [],
        scope: { kind: 'nc' },
        ncQueue: Array.isArray(saved.queue) ? saved.queue : [],
        ncSource: saved.source || '在线',
        ncQueueFrom: (saved.queueFrom && typeof saved.queueFrom === 'object') ? saved.queueFrom : null,
        // 恢复出带来源歌单的队列 → bump 一次 rev，让面板「恢复后刷新」跟随最新歌单。
        ncQueueRev: (saved.queueFrom && typeof saved.queueFrom === 'object') ? (store.ncQueueRev || 0) + 1 : (store.ncQueueRev || 0),
        position: pos, duration: savedDur,
        pendingId: null, pendingName: null, error: null,
        tab: 'nc',
        currentQuality: '',
    ...
    function restoreLatest(list) {
      ...
      let ncTs = -1;
      try {
        const n = JSON.parse(loadPref(PREF_PLAYBACK_NC) || 'null');
        if (n && typeof n.ts === 'number') ncTs = n.ts;
      } catch (e) {}
      let radioTs = -1;
      try {
        const r = JSON.parse(loadPref(PREF_PLAYBACK_RADIO) || 'null');
        if (r && typeof r.ts === 'number') radioTs = r.ts;
      } catch (e) {}
      ...
      if (ncTs >= 0 && scopeKind === 'nc') { restoreNCPlayback(); restoreLyricForCurrent(); return; }
      if (qqTs >= 0 && scopeKind === 'qq') { restoreQQPlayback(); restoreLyricForCurrent(); return; }
      if (radioTs >= 0 && scopeKind === 'radio') { restoreRadioPlayback(); return; }
      ...
      if (ncTs > bestTs) { best = 'nc'; bestTs = ncTs; }
      if (radioTs > bestTs) { best = 'radio'; bestTs = radioTs; }
      ...
      if (best === 'nc') { restoreNCPlayback(); restoreLyricForCurrent(); return; }
      if (best === 'radio') { restoreRadioPlayback(); return; }
    ...
    async function loadTracks() {
      ...
      try {
        ...
        set({
          ...
          // 三家的「登录已过期」标记同口径随 manifest 下发（Host 各 /status 的真校验结论），
          // 保证启动首帧「关于」页与面板一致，而不是等各自面板挂载后才纠正。
          qqAuthExpired: !!result.qqAuthExpired,
          kgLoggedIn: !!result.kgLoggedIn, kgNickname: result.kgNickname || '', kgAuthExpired: !!result.kgAuthExpired,
          ncLoggedIn: !!result.ncLoggedIn, ncNickname: result.ncNickname || '', ncAuthExpired: !!result.ncAuthExpired,
    ...
    // 收音机图标（网络电台直播的名称前缀，与音符/话筒/报纸同尺寸同色系）：
    // 一台带天线的收音机，一眼区别于「本地/在线点播」的音符——电台是直播流不是曲库。
    function RadioIcon(props) {
      const cls = props.className || '';
      return React.createElement('svg', { className: cls, width: 12, height: 12, viewBox: '0 0 24 24', fill: 'currentColor', 'aria-hidden': true },
        React.createElement('path', { d: 'M3.24 6.15C2.51 6.43 2 7.17 2 8v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8c0-1.11-.89-2-2-2H8.3l8.26-3.34L15.88 1 3.24 6.15zM7 20c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm13-8h-2v-2h-2v2H4V8h16v4z' }));
    }
    // 爱心图标（收藏钮通用）：收藏=实心(currentColor)、未收藏=空心描边。播放条与电台行
    // 收藏钮共用，替代 ♥/♡ 文本字形——字符宽度/基线随字体变化，SVG 恒定尺寸、颜色精确可控。
    function HeartIcon({ className = '', filled = false, size = 16 }) {
      return React.createElement('svg', {
        className: className, width: size, height: size, viewBox: '0 0 24 24',
        fill: filled ? 'currentColor' : 'none', stroke: 'currentColor', strokeWidth: 2, 'aria-hidden': true,
    ...
    function BookStatus() {
      ...
      useEffect(() => {
        ...
        // 立即用当前时间同步基准，再启动每秒 tick：interval 每次因 bookBuffering
        // 翻转而重建，若首帧仍用冻结的旧 now（挂载或上次 interval 结束时残留），
        // 会与新的 bookBufferingSince 算出错误的大秒数——表现是「合成中…300s」
        // 闪现一帧后跳回「合成中…1s」。先同步一次即可让首帧即为真实差（≈0s 起）。
        setNow(Date.now());
        const t = setInterval(() => setNow(Date.now()), 1000);
        return () => clearInterval(t);
      }, [s.bookBuffering]);
      if (s.bookBuffering && !s.bookBufferingSilent) {
        // 秒数下限保护：now 异常早于 since（时钟回拨/极端冻结）时钳到 0，
        // 绝不闪现负秒或荒谬的大数。
        const secs = s.bookBufferingSince > 0
          ? Math.max(0, Math.floor((now - s.bookBufferingSince) / 1000))
          : 0;
    ...
    // 页面底部模式的锚定度量：把条对齐到「会话列」的横向跨度（贴底由 CSS 的 bottom: 0 负责）。
    // 锚点用 DSH 明确暴露的 [data-composer-seat]（composer 底座：横跨会话列宽，且询问
    // 弹窗接管输入区时它仍在），它的父级就是「会话头下方的滚动带」——取它的 left/width
    // 即会话列的横向跨度，条与输入卡片的左右边界对齐（窗口被侧边栏/右栏挤压时也跟着变）。
    // 测不到锚点（切到设置 / 插件管理面板时会话列整个卸载）→ 回退「视口宽」（再由内层
    // max-width:780px 居中）。
    // 侧边栏折叠/拖拽会动画改变列宽：ResizeObserver 盯住锚点；另加 1s 兜底轮询处理
    // 「锚点出现/消失」（面板切换）。1s 只是一次 rect 读取，且只在页面底部模式下跑。
    function useColumnMetrics(enabled) {
      const [metrics, setMetrics] = useState(null);
      // 用 useEffect（不是 useLayoutEffect）：后者在 renderToString 的用例里会给每个
      // 播放条渲染冒一条 React 警告。首帧不画（见渲染处的 metrics===null 守卫），
      // 测量完成后才挂上去，所以不会出现「先画错位置再跳」的闪动。
      useEffect(() => {
        if (!enabled || typeof document === 'undefined') return undefined;
        let anchor = null;
        let ro = null;
        const measure = () => {
          const next = { left: 0, width: window.innerWidth };
          let band = null;
          try {
            const seat = document.querySelector('[data-composer-seat]');
            band = seat === null ? null : seat.parentElement;
            if (band !== null) {
              const r = band.getBoundingClientRect();
              if (r.width > 0) { next.left = Math.round(r.left); next.width = Math.round(r.width); }
            }
          } catch (e) { /* 测量失败即用回退值 */ }
          // 只在真的变了才 set：否则每秒一次轮询会白白触发 React 渲染。
          setMetrics((prev) => (prev !== null && prev.left === next.left
            && prev.width === next.width ? prev : next));
          if (band !== anchor) {
            if (anchor !== null && ro !== null) { try { ro.disconnect(); } catch (e) {} }
            anchor = band;
            if (anchor !== null && ro !== null) { try { ro.observe(anchor); } catch (e) {} }
          }
        };
        try { if (typeof ResizeObserver === 'function') ro = new ResizeObserver(measure); } catch (e) { ro = null; }
        measure();
        window.addEventListener('resize', measure);
        ...
        return () => {
          ...
          clearInterval(timer);
          if (ro !== null) { try { ro.disconnect(); } catch (e) {} }
        };
      }, [enabled]);
      return metrics;
    }

    // placement：'dock' = 挂在输入框上方的 conversation.input.dock；
    //            'overlay' = 挂在 root 作用域的 shell.overlay（页面底部 / 悬浮两种模式）。
    // 两个挂载点按 store.barPos 互斥渲染，保证同一时刻**只有一条**播放条
    // （频谱画布 barCanvasNode 是模块级单引用，两条会互相覆盖）。
    function NowPlayingBar({ placement }) {
      const s = useStore();
      // 本实例实际采用的落位：dock 挂载点只在 dock 模式渲染，overlay 挂载点只在
      // bottom / float 模式下渲染（见文件末尾两处 slots.register）。
      const isDockMount = placement === 'dock';
      const mode = isDockMount ? 'dock' : (s.barPos === 'bottom' ? 'bottom' : 'float');
      const isBottom = mode === 'bottom';
      const isFloat = mode === 'float';
      const columnMetrics = useColumnMetrics(isBottom);
      const wrapRef = useRef(null);
      const dragRef = useRef(null);
      // 拖动结束会补发一次 click（歌名单击=打开面板）：捕获阶段吞掉那一次。
      ...
      // 悬浮坐标越界兜底（窗口变小 / 上次拖得很靠边）：渲染后按当前视口把坐标拉回可视
      // 范围并写回 prefs；窗口 resize 时同样处理。只依赖 isFloat，避免拖动过程中反复重跑。
      useEffect(() => {
        if (!isFloat) return undefined;
        const fit = () => {
          const cur = store.barFloatPos;
          const el = wrapRef.current;
          if (cur === null || el === null) return;
          const r = el.getBoundingClientRect();
          const x = Math.max(0, Math.min(cur.x, Math.max(0, window.innerWidth - r.width)));
          const y = Math.max(0, Math.min(cur.y, Math.max(0, window.innerHeight - r.height)));
          if (x !== cur.x || y !== cur.y) set({ barFloatPos: { x, y } });
        };
        fit();
        window.addEventListener('resize', fit);
        return () => window.removeEventListener('resize', fit);
      }, [isFloat]);
      // 页面底部模式：给 DSH 的输入区底座（[data-composer-seat]）挂一个标记属性，CSS 据此
      // 给底座补 BAR_BOTTOM_PAD 的下内边距——输入卡片整体上移，贴窗口底边的条既压不住
      // 输入框、也遮不住任何对话内容（切走/切模式时立刻摘掉，不留副作用）。
      useEffect(() => {
        if (!isBottom || typeof document === 'undefined') return undefined;
        const root = document.documentElement;
        root.setAttribute('data-dsh-music-bar-bottom', '');
        return () => root.removeAttribute('data-dsh-music-bar-bottom');
      }, [isBottom]);
      // 两个挂载点各注册一个实例，这里互斥地只让其中一个真的渲染（hooks 必须都已调用完
      // 才能提前 return）。dock 模式挂 dock，其它模式挂 shell.overlay；页面底部模式在拿到
      // 会话列度量前先不画（一帧都不会闪到错误位置）。
      if (isDockMount && s.barPos !== 'dock') return null;
      if (!isDockMount && s.barPos === 'dock') return null;
      if (isBottom && columnMetrics === null) return null;
      ...
      const isRadio = s.currentId !== null && String(s.currentId).startsWith('radio:');
      ...
      // 歌名不是文件名，原样保留。
      const displayName = (isQQTrack || isRadio) ? name : stripExt(name);
      // 名称前的图标：新闻用报纸图标，讲书用话筒图标，网络电台用收音机图标，
      // 音乐用音符图标（空闲态无曲目 = 音乐）。
      const note = React.createElement(isNews ? NewsIcon : (isBook ? MicIcon : (isRadio ? RadioIcon : MusicNote)), { className: 'dsh-music-note' });
      ...
      // 音乐来源徽标：在线 QQ/酷狗/网易云 曲目在歌名后分别标「QQ音乐」「酷狗音乐」
      // 「网易云」，取到「真实品质」时附加「 · 无损/高音质/标准」，取不到则只显示来源名。
      ...
      if (showQuality && s.currentId !== null) {
        ...
        const isNC = String(s.currentId).startsWith('nc:');
        if (isQQ || isKG || isNC) {
          const q = (s.currentQuality || '').trim();
          const label = isQQ ? 'QQ音乐' : (isKG ? '酷狗音乐' : '网易云');
          sourceBadge = React.createElement('span', { className: 'dsh-music-bar-src', title: q ? (label + '（在线）· ' + q) : label + '（在线）' },
            label + (q ? ' · ' + q : ''));
        } else if (isRadio) {
          // 电台徽标：格式与码率（如「电台 · AAC 128k」）；无则只标「电台」。
          const q = (s.currentQuality || '').trim();
          sourceBadge = React.createElement('span', { className: 'dsh-music-bar-src', title: (s.radioCurrent && s.radioCurrent.country) ? ('网络电台 · ' + s.radioCurrent.country) : '网络电台（直播）' },
            '电台' + (q ? ' · ' + q : ''));
        }
      }
      // 本地音乐的「格式 · 音质」标签（如 FLAC · 无损 / MP3 · 高音质），与在线三档一致；
      // 解析不出（未知格式/不可读）则不显示。在线 QQ/酷狗/网易云 用上面的来源徽标，互不叠加。
      let localQualityBadge = null;
      if (showQuality && s.currentId !== null && !String(s.currentId).startsWith('qq:') && !String(s.currentId).startsWith('kg:') && !String(s.currentId).startsWith('nc:') && !String(s.currentId).startsWith('book:') && !String(s.currentId).startsWith('radio:') && s.currentQuality) {
        localQualityBadge = React.createElement('span', { className: 'dsh-music-bar-src', title: s.currentQuality }, s.currentQuality);
      }
      // 多标签页统一播放器：本页是镜像态（声音由另一个标签页负责）时给一个明确标识。
      // 否则「播放条显示正在播、却听不到声音、频谱也不动」会被当成故障。带上
      // dsh-music-bar-src 类是为了复用「纯信息展示，单击不打开面板」的既有语义。
      const mirrorBadge = (s.mirroring && s.playing)
        ? React.createElement('span', {
            className: 'dsh-music-bar-src dsh-music-bar-xout',
            title: '声音由另一个标签页（DSH 窗口）播放：本页静音，只同步显示同一个播放器的状态。'
              + '此页的播放/暂停、上下一首、音量、切歌等操作都会转发到那个标签页执行。',
          }, '此页静音')
        : null;
      ...
      // 网络电台收藏到本地收藏夹（Host /radio/favs）；网易云收藏写操作未实现 →
      // nc 曲目不显示爱心钮（toggleFav 亦有兜底）。
      const faved = hasTrack && !isBook && isCurrentFaved();
      const kgCur = hasTrack && String(s.currentId || '').startsWith('kg:');
      const ncCur = hasTrack && String(s.currentId || '').startsWith('nc:');
      const favTitle = kgCur
        ? (faved ? '取消收藏（从酷狗「我喜欢」移除）' : '收藏到酷狗「我喜欢」')
        : (isRadio
          ? (faved ? '取消收藏（从我的电台移除）' : '收藏到我的电台')
          : (faved ? '取消收藏（从「我最喜欢」移除）' : '收藏到「我最喜欢」'));
      const heartBtn = hasTrack && !isBook && !ncCur ? React.createElement('button', {
        className: 'dsh-music-bar-btn fav' + (faved ? ' on' : ''),
        title: favTitle,
        onClick: toggleFav,
      }, React.createElement(HeartIcon, { filled: faved })) : null;
      ...
      const hoverActive = barHover || anyPopOpen || !hasTrack;
      // 悬浮条（float）：小条上「悬停才滑出按钮」没法用，控件组**常显**；但闲置时仍按
      // 「沉浸感」整体变淡，时间/歌词等内容也常显（内容不再随闲置态隐藏）——安静待在一边。
      const active = hoverActive || isFloat;
      const barDimmed = !hoverActive;
      // 悬浮条没有那个「右端热区」（.dsh-music-bar-hotspot 在悬浮模式下是 display:none），
      // 所以移入/移出改由整条来报：移入 → 完全不透明；移出 → 1s 后回到「沉浸感」配置的
      // 透明度（与 dock 模式鼠标移入移出播放条右端的行为一致）。控件行自己的 onMouseLeave
      // 在悬浮模式下要关掉，否则鼠标在条内从控件行移到歌词行就会误排一次「变淡」。
      const onWrapEnter = isFloat ? showBarBtns : undefined;
      const onWrapLeave = isFloat ? onBarLeave : undefined;
      // 两条不同的显隐规则，别混：① 时长/直播标记只在「操作态」显示（闲置态收起，
      // 给歌词让位）；② 歌词反之，只在「闲置态」显示（操作时收起，不给滑入的按钮组
      // 让路）。悬浮条（float）控件本来就常显、歌词独占一行，所以两条都常显。
      const timeVisible = !barDimmed || isFloat;
      const lyricVisible = barDimmed || isFloat;
      // 频谱画布的挂载条件：dock / 页面底部下只在播放中挂（暂停时不留一块空画布）；
      // 悬浮条里**常挂**——画布在第二行里一进一出会让第三行（控制条）上下漂移，
      // 悬浮条不能「一按暂停就抖一下」。暂停时 drawViz 会把画布 clearRect（看不见）。
      const vizMounted = !isBook && hasTrack && s.showViz && !s.mirroring && (s.playing || isFloat);
      ...
      // ---- 悬浮条：整条拖动 + 位置记忆（与播放面板同款的 pointer 拖动 + clamp）----
      // 拖动阈值 4px：小于阈值算点击（歌名单击=打开面板、按钮照常可点），过了阈值才算
      // 拖动；拖动结束补发的那次 click 由 onWrapClickCapture 在捕获阶段吞掉。
      // 位置写进 store.barFloatPos → set() 落 Host prefs；null（没拖过/已重置）时由
      // CSS 的 right/bottom 贴右下角。
      const clampFloat = (x, y) => {
        const el = wrapRef.current;
        if (el === null) return { x: Math.round(x), y: Math.round(y) };
        const r = el.getBoundingClientRect();
        const maxX = Math.max(0, window.innerWidth - Math.max(r.width, 120));
        const maxY = Math.max(0, window.innerHeight - Math.max(r.height, 32));
        return { x: Math.round(Math.max(0, Math.min(x, maxX))), y: Math.round(Math.max(0, Math.min(y, maxY))) };
      };
      const onWrapPointerDown = (e) => {
        ...
        if (e.button !== undefined && e.button !== 0) return;
        const t = e.target;
        // 交互控件（按钮/输入框/徽章）不参与拖动，避免抢掉它们的点击。
        if (t && typeof t.closest === 'function' && t.closest('button, input, textarea, select, .dsh-music-bar-src')) return;
        const el = wrapRef.current;
        if (el === null) return;
        const r = el.getBoundingClientRect();
        dragRef.current = { sx: e.clientX || 0, sy: e.clientY || 0, ox: r.left, oy: r.top, moved: false, id: e.pointerId, el };
      };
      const onWrapPointerMove = (e) => {
        const d = dragRef.current;
        if (d === null) return;
        const dx = (e.clientX || 0) - d.sx;
        const dy = (e.clientY || 0) - d.sy;
        if (!d.moved && Math.abs(dx) + Math.abs(dy) < 4) return; // 还在点击阈值内
        if (!d.moved) {
          d.moved = true;
          if (typeof d.el.setPointerCapture === 'function' && d.id !== undefined) {
            try { d.el.setPointerCapture(d.id); } catch (err) { /* 环境不支持：照常按坐标拖 */ }
          }
        }
        set({ barFloatPos: clampFloat(d.ox + dx, d.oy + dy) });
      };
      ...
      const onWrapClickCapture = (e) => {
        if (!suppressClickRef.current) return;
        suppressClickRef.current = false;
        e.stopPropagation();
        e.preventDefault();
      };
      // 落位样式/类名：页面底部模式用会话列度量内联定位（贴底由 CSS 的 bottom:0 负责）；
      // 悬浮模式用记忆坐标（没有则交给 CSS 贴右下角——一旦有坐标就必须清掉 right/bottom，
      // 否则 left+right 同时生效会把固定元素拉宽）。
      const wrapClassName = 'dsh-music-bar-wrap' + (isBottom ? ' bottom' : (isFloat ? ' float' : ''));
      const wrapStyle = isBottom
        ? (columnMetrics === null ? undefined : { left: columnMetrics.left + 'px', width: columnMetrics.width + 'px' })
        : (isFloat && s.barFloatPos !== null
          ? { left: s.barFloatPos.x + 'px', top: s.barFloatPos.y + 'px', right: 'auto', bottom: 'auto' }
          : undefined);
      return React.createElement('div', {
        id: 'dsh-music-bar-wrap',
        ...
        onClickCapture: onWrapClickCapture,
        ...
        React.createElement('div',
          ...
          hasTrack
            ? React.createElement('span', { className: 'dsh-music-bar-name', title: displayName + (artistText ? ' - ' + artistText : '') + (chapterEl ? ' - ' + s.currentSection : '') + '\n（单击打开播放列表）', onClick: togglePanelOnName },
                ...
                artistEl, sourceBadge, localQualityBadge, chapterEl, afterName, mirrorBadge)
            : React.createElement('span', { className: 'dsh-music-bar-idle', title: '单击打开播放列表', onClick: togglePanel }, note, ' DSH音乐播放器'),
          vizMounted ? React.createElement('canvas', { className: 'dsh-music-viz', width: 60, height: 20, ref: (el) => { barCanvasNode = el; } }) : null,
          // 歌词/字幕：位于频谱之后、时长之前；dock 下仅"非使用态"（控件组已折叠、播放条
          // 半透明）显示——正在操作时收起，不给滑入的按钮组让路；悬浮条（float）里歌词
          // 独占一行、控件本来就常显，所以常驻显示（鼠标移上去不该把歌词弄没）。换行动效
          // 由系统配置驱动，跑马灯/边缘渐隐恒开（BarLyric 内部处理结构/CSS，textContent 恒为当前行）。
          // 悬浮条里这一行是**固定行**：没有歌词（纯音乐/讲书还没出字幕/空闲态）时用同款
          // 空占位撑住行高，避免「有词三行、没词两行」让条的高度跳来跳去。
          // 「歌词显示」被关掉时不占这行（那是用户主动关的，条就固定成两行）。
          s.lyricText && lyricVisible && hasTrack && s.showLyric
            ...
            : (isFloat && s.showLyric
              ? React.createElement('div', { className: 'dsh-music-bar-lyric dsh-music-bar-lyric-empty' })
              : null),
          ...
          React.createElement('div', { className: 'dsh-music-bar-controls' + (active ? ' on' : ''), onMouseLeave: isFloat ? undefined : onBarLeave },
            hasTrack
              ? (showHint
                  ? React.createElement('span', { className: 'dsh-music-bar-hint' }, '⚠ 自动播放被拦截，点击▶解锁')
                  : (isBook
                      ? (timeVisible ? React.createElement('span', { className: 'dsh-music-bar-time' }, Math.round(s.bookProgress * 100) + '%') : null)
                      : (isRadio
                          ? (timeVisible ? React.createElement('span', { className: 'dsh-music-bar-live', title: '网络电台直播' }, s.playing ? '● LIVE' : '电台') : null)
                          : (timeVisible ? React.createElement('span', { className: 'dsh-music-bar-time' }, fmtTime(s.position) + ' / ' + fmtTime(s.duration)) : null))))
              : null,
            React.createElement('div', { className: 'dsh-music-bar-btns' },
              heartBtn,
              hasTrack ? React.createElement('button', { className: 'dsh-music-bar-btn', title: isBook ? '上一章' : (isRadio ? '上一个台' : '上一首'), onClick: () => (isBook ? stepBook(-1) : step(-1)) }, React.createElement(PrevIcon, null)) : null,
              ...
              hasTrack ? React.createElement('button', { className: 'dsh-music-bar-btn', title: isBook ? '下一章' : (isRadio ? '下一个台' : '下一首'), onClick: () => (isBook ? stepBook(1) : step(1)) }, React.createElement(NextIcon, null)) : null,
          ...
          volOpen ? portalToBody(React.createElement('div', {
            ...
            style: isBook ? anchorPopAbove(volRef.current, 136) : anchorAbove(volRef.current, 36, 108),
            ref: volPopRef,
          },
    ...
    function ModeDropdown() {
      ...
      return React.createElement('div',
        ...
        open ? portalToBody(React.createElement('div', { className: 'dsh-music-mode-pop', style: anchorAbove(ref.current, 120, 108), ref: popRef },
    ...
    function NewsPane({ panelRef }) {
      ...
      const failureLine = lastFailure !== null
        ? React.createElement('div', { className: 'dsh-music-news-failure' },
            React.createElement('span', { className: 'dsh-music-news-failure-text' },
              '⚠ ' + fmtTs(lastFailure.ts) + ' 收集'
                + (lastFailure.kind === 'empty' ? '无结果' : lastFailure.kind === 'model' ? '模型失效' : '失败')
                + (lastFailure.reason ? ' · ' + lastFailure.reason : '')),
            React.createElement('button', {
      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (pRaw === null) {
          ...
          // ⚠ 受控 <select value> 在「值不在选项里」时会**静默回退显示第一项**：DSH 侧模型
          // 改名/下线后，pin 住的旧值会让界面看起来已经选中新模型，而实际存盘/执行仍是旧值；
          // 用户再去点那一项（DOM 值没变）连 change 事件都不触发，改也白改。因此失效值必须
          // 显式列成选项，让下拉框如实展示真实存值，任何重选都产生 change。
          const modelRow = models !== null && models.length > 0 ? (() => {
            ...
            const staleProvider = (sel.provider && curProvider === null) ? sel.provider : null;
            const staleModel = (sel.model && curProvider !== null && !modelsFor.some((m) => m.id === sel.model))
              ? sel.model : null;
              ...
              React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' } },
                React.createElement('select', {
                  ...
                  onChange: (e) => {
                    ...
                    // 该 provider 一个模型都没有：不落一个 { provider, model: '' } 的半残选择
                    // （服务端会规整成 null，静默退回兜底模型）——保持原样，让用户先选别的。
                    if (pid !== '' && !first) return;
                    updateForm({ ...p, model: pid === '' ? null : { provider: pid, model: first.id } });
                  },
                  style: { padding: '2px 6px' },
                },
                  React.createElement('option', { value: '' }, '跟随当前会话'),
                  staleProvider !== null
                    ? React.createElement('option', { key: '__stale-provider', value: staleProvider }, '⚠ ' + staleProvider + '（已失效）')
                    : null,
                  models.map((pr) => React.createElement('option', {
                    key: pr.id, value: pr.id,
                    // 无可用模型的 provider 置灰不可选（选中它只会得到一个无效的模型偏好）。
                    disabled: !(pr.models && pr.models.length > 0),
                  }, pr.name + ((pr.models && pr.models.length > 0) ? '' : '（无可用模型）'))),
                ),
                curProvider ? React.createElement('select', {
                  ...
                  staleModel !== null
                    ? React.createElement('option', { key: '__stale-model', value: staleModel }, '⚠ ' + staleModel + '（已失效，请重选）')
                    : null,
    ...
    function startQQPlayback(song, queue, sourceLabel, from) {
      // 多标签页：镜像态下点在线曲目 = 让出声页切过去（队列一并带过去，之后的上/下一首
      // 才和点播的那张列表一致）。
      if (xtabForward('qqPlay', { song, queue, label: sourceLabel, from })) return;
    ...
    function startKGPlayback(song, queue, sourceLabel, from) {
      // 多标签页：同 startQQPlayback，镜像态下转发给出声页。
      if (xtabForward('kgPlay', { song, queue, label: sourceLabel, from })) return;
    ...
    // ---- 在线网易云音乐（nc:）：与 QQ/KG 同构的播放/歌词/品质链路 ----
    // 取链响应头 X-DSH-NC-Quality（Host 回传服务端授予档位）；X-DSH-NC-Trial 标记试听片段。
    function loadNCQuality(id, url) {
      fetch(url, { method: 'HEAD', cache: 'no-store' }).then((r) => {
        if (store.currentId !== 'nc:' + id) return; // 响应返回时已切歌 → 丢弃
        const hdrs = r && r.headers;
        if (!hdrs || typeof hdrs.get !== 'function') return;
        let q = '';
        const raw = hdrs.get('X-DSH-NC-Quality');
        if (raw) { try { q = decodeURIComponent(raw); } catch { q = raw; } }
        if (hdrs.get('X-DSH-NC-Trial')) q = '试听' + (q ? ' · ' + q : '');
        if (q) set({ currentQuality: q });
      }).catch(() => {});
    }
    // 网易云歌词：Host 按 id 取 LRC+翻译（匿名可用），YRC 逐字优先。
    function loadNCLyric(id, song) {
      resetLyric();
      const q = new URLSearchParams({ id: String((song && song.id) || '') });
      fetch('/dsh-music/nc/lyric?' + q.toString(), { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
          if (!d || !d.ok) return;
          if (store.currentId !== id) return; // 取词期间已切歌
          if (Array.isArray(d.wordLines) && d.wordLines.length > 0) {
            musicWordLyric = d.wordLines;
            syncLyricPanelData();
            noteLyricSource(d.source || 'nc-yrc', d.wordLines.length + ' 行（YRC 逐字行窗口）');
            updateLyric();
            return;
          }
          if (!Array.isArray(d.lrc)) return;
          musicLyric = mergeLyricTrans(d.lrc, d.trans);
          syncLyricPanelData();
          noteLyricSource('netease', d.lrc.length + ' 行（网易云在线歌曲，整行 LRC' + (d.trans && d.trans.length ? '，含逐句翻译' : '') + '）');
          updateLyric();
        })
        .catch(() => {});
    }
    function startNCPlayback(song, queue, sourceLabel, from) {
      // 多标签页：同 startQQPlayback，镜像态下转发给出声页。
      if (xtabForward('ncPlay', { song, queue, label: sourceLabel, from })) return;
      const id = String((song && song.id) || '');
      if (id === '') return;
      const url = '/dsh-music/nc/play/' + id;
      const q = (Array.isArray(queue) && queue.length > 0) ? queue.slice() : [song];
      restoredMusicPos = null;
      bookRestorePos = -1;
      closeLiveViz();
      ...
      setupLiveViz();
      set({
        currentId: 'nc:' + id, currentName: song.title, currentArtists: (song.artists || []),
        scope: { kind: 'nc' }, ncQueue: q, ncSource: sourceLabel || (q.length > 1 ? '在线' : ''),
        ncQueueFrom: from === undefined ? (store.ncQueueFrom || null) : from,
        error: null, currentQuality: '', position: 0, duration: 0,
      });
      loadNCQuality(id, url);
      loadNCLyric('nc:' + id, song);
      savePlayback();
      const p = audio.play();
      if (p !== undefined && typeof p.catch === 'function') {
        p.catch((err) => { if (!isAutoplayBlocked(err)) return; set({ error: '浏览器拦截了自动播放，请在播放条点击▶解锁', pendingId: 'nc:' + id, pendingName: song.title }); });
      }
    }

    // ===================== 网络电台（radio:）播放 =====================
    // 电台是「直播无限流」：与本地/在线点播的关键差异是
    //   1. 不 seek：无进度条、不恢复中途位置（刷新后回到台首/重新拉流）；
    //   2. 不显示时长：播放条显示 LIVE；
    //   3. ended 由断流触发 → 尝试自动重连而不是「下一首」。
    // 播放语义与 startQQPlayback 同构（audio.src = 同源代理 URL → load → play），
    // 但 id 前缀是 radio:，且保存的不是 position 而是「当前台对象」。
    const radioStationId = (st) => String((st && (st.id || st.stationuuid)) || '');
    let lastRadioReconnectAt = 0; // 断流自动重连守卫（30s 内至多一次）
    let radioRetryCount = 0; // 电台加载失败自动重试计数（每次 startRadioPlayback 重置，≤3 次）
    function startRadioPlayback(station) {
      const st = station || store.radioCurrent;
      if (!st || !st.url) return;
      // 多标签页：镜像态下点电台 = 让出声页换台（直播流没法在两页各挂一份而只出一声）。
      if (xtabForward('radioPlay', { station: st })) return;
      const uid = radioStationId(st);
      const id = uid === '' ? 'radio:' + st.url : 'radio:' + uid;
      // HLS 台追加 hls=1：Host /radio/play 据此走转流（.m3u8 后缀的 URL 也会自动判别）。
      const hlsQ = st && st.hls ? '&hls=1' : '';
      const url = '/dsh-music/radio/play?u=' + encodeURIComponent(st.url) + hlsQ;
      restoredMusicPos = null;
      bookRestorePos = -1;
      radioRetryCount = 0; // 新一轮播放重置自动重试计数
      closeLiveViz();
      audio.src = url;
      audio.load();
      setupLiveViz();
      // 电台无歌词：清掉旧歌词。收藏走本地收藏夹（爱心在播放条/列表均可用）。
      resetLyric();
      // 刷新后直接恢复电台播放时 radioFavs 可能尚未加载（面板没开过）——
      // 拉一次让播放条爱心状态准确。
      loadRadioFavsIfNeeded();
      set({
        currentId: id, currentName: st.name || '网络电台', currentArtists: [],
        scope: { kind: 'radio' }, radioCurrent: st, radioPlaying: true,
        error: null, currentQuality: radioQualityTxt(st),
        position: 0, duration: 0, qqFaved: false, kgFaved: false,
      });
      // 记入 Host「最近播放」。
      fetch('/dsh-music/radio/recent', {
        method: 'POST', cache: 'no-store',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ station: st }),
      }).then((r) => r.json()).then((d) => {
        if (d && d.ok) { /* recent 已持久化；本地 recent 由面板拉取刷新 */ }
      }).catch(() => {});
      savePlayback();
      const p = audio.play();
      if (p !== undefined && typeof p.catch === 'function') {
        p.catch((err) => { if (!isAutoplayBlocked(err)) return; set({ error: '浏览器拦截了自动播放，请在播放条点击▶解锁', pendingId: id, pendingName: st.name }); });
      }
    }
    // 播放条点击电台的「播放/暂停」时使用（togglePlay 对 restore 的电台同样走这）。
    function isRadioId(id) {
      return id !== null && String(id).startsWith('radio:');
    }
    function radioIdToStation(id) {
      const uid = String(id).slice(6);
      const st = store.radioCurrent;
      if (st && String(radioStationId(st)) === uid) return st;
      return [...(store.radioRecent || []), ...(store.radioFavs || [])].find((x) => String(radioStationId(x)) === uid) || st || null;
    }
    // 电台收藏夹惰性加载：播放条爱心/收藏需要 radioFavs，但面板没打开过时它没拉过。
    // 首次需要时 GET 一次（幂等：radioFavLoaded 后不再拉）。
    function loadRadioFavsIfNeeded() {
      if (store.radioFavLoaded) return;
      fetch('/dsh-music/radio/favs', { cache: 'no-store' })
        .then((r) => r.json()).then((d) => {
          if (d && d.ok && !store.radioFavLoaded) {
            set({ radioFavs: (d.favs || []).slice(), radioFavLoaded: true });
          }
        }).catch(() => {});
    }
    // 电台恢复（PREF_PLAYBACK_RADIO 只存当前台对象，不存位置）：刷新后停在「该台已选、
    // 暂停态」，点 ▶ 重新拉流播放。
    function restoreRadioPlayback(saved) {
      if (!saved) saved = loadRadioPlayback();
      if (saved === null) return;
      const st = (saved && saved.station) || null;
      if (!st || !st.url) return;
      const uid = radioStationId(st);
      set({
        currentId: (uid === '' ? 'radio:' + st.url : 'radio:' + uid),
        currentName: st.name || '网络电台',
        currentArtists: [], radioCurrent: st, radioPlaying: false,
        scope: { kind: 'radio' },
        currentQuality: radioQualityTxt(st),
    ...
    function loadRadioPlayback() {
      const raw = loadPref(PREF_PLAYBACK_RADIO);
      if (raw === null) return null;
      try { const p = JSON.parse(raw); if (p && p.station && p.station.url) return p; } catch (e) {}
      return null;
    }
    // ------------------------------------------------------------------
    ...
    function KGOnlinePanel({ panelRef }) {
      ...
      function resetKgToLogin(msg) {
        ...
        kgFavPromise = null;
        set({ kgQueue: [], kgSource: '', kgQueueFrom: null, kgQueueRev: 0, kgFaved: false, kgFavHashes: [], kgFavFiles: [], kgFavListId: 0, kgAuthDeadMsg: '', kgLoggedIn: false, kgNickname: '' });
        setLoggedIn(false); setUserid(''); setNickname(''); setMinePlays([]); setMineLoaded(false);
        setKgPlCounts(null);
        setBrowseTab('recommend'); setBrowseErr(''); setMineErr(''); setQError('');
        setLayer('main'); setActivePl(null); setPlLoading(false);
        // 与 QQ「过期就直接进登录 UI」同一约定：酷狗先落到登录页（含失效原因），扫码弹窗
        // 由上面的「页签守卫」effect 自动打开并出码（酷狗只有扫码一种方式，不存在选错方式）。
        setLoginReason(msg || '酷狗登录已失效（登录态与设备不匹配），请重新扫码登录');
      }
      useEffect(() => {
      ...
      const [nickname, setNickname] = useState(''); // 酷狗账号昵称（对齐网易云显示「退出登录（昵称）」）
      const [qrImage, setQrImage] = useState('');
      const [qrKey, setQrKey] = useState('');
      const [loginMsg, setLoginMsg] = useState('');
      // 「登录已失效/已过期」的原因单独存：自动打开扫码弹窗会让 loginMsg 变成二维码状态文案，
      // 共用一份会把最关键的那句失效原因冲掉（登录页上就只剩「二维码获取失败」）。
      const [loginReason, setLoginReason] = useState('');
      const [loginBusy, setLoginBusy] = useState(false);
      const [loginOpen, setLoginOpen] = useState(false);
      const pollTimer = useRef(null);
      // 登录失效/已过期 + 当前正停在「酷狗」页签 → 自动打开扫码弹窗并出码（不用再点一次「登录」）。
      // 三个在线面板都是常驻挂载、弹窗 portal 到 body，所以必须按页签守卫：否则用户在别的页签
      // 听本地音乐时会被二维码盖住。loginReason 是本地持久信号（登录成功才清），切走再切回也能补上。
      useEffect(() => {
        if (s.tab !== 'kg') return;
        if (loginReason === '' || loginOpen) return;
        setLoginOpen(true);
        startLogin();
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [s.tab, loginReason, loginOpen]);
      useEffect(() => {
        jsonGet('/dsh-music/kg/status').then((d) => {
          if (d) {
            setLoggedIn(!!d.loggedIn); setUserid(d.userid || ''); setNickname(d.nickname || '');
            // 登录态只反映「服务端是否还认这个会话」；失效是使用时（业务报 20017/20018 后清会话）
            // 才发现的。面板正常打开时不主动探测，也不在这里清「疑似失效」标记。
            if (d.loggedIn) resetKgAuthExpired();
          }
        }).catch(() => {});
          ...
          const tick = () => {
            fetch('/dsh-music/kg/login/check?key=' + encodeURIComponent(d.key), { cache: 'no-store' })
              ...
              .then((c) => {
                ...
                if (c.status === 'success') {
                  setLoggedIn(true); setUserid(c.userid || ''); setNickname(c.nickname || ''); setQrImage(''); setQrKey('');
      ...
      function doLogout() {
        jsonPost('/dsh-music/kg/login/logout', {}).then(() => {
          ...
          kgFavPromise = null;
          set({ kgFaved: false, kgFavHashes: [], kgFavFiles: [], kgFavListId: 0, kgLoggedIn: false, kgNickname: '' });
        }).catch(() => {});
      }
      ...
      if (!loggedIn) {
        return React.createElement('div', { className: 'dsh-music-qq dsh-music-qq-login' },
          React.createElement('div', { className: 'dsh-music-qq-login-center' },
            (loginReason || loginMsg) ? React.createElement('div', { className: 'dsh-music-qq-login-dead' }, loginReason || loginMsg) : null,
      ...
      const head = React.createElement('div', { className: 'dsh-music-qq-head' },
        React.createElement('div', { className: 'dsh-music-qq-toolbar' },
          ...
          React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: doLogout }, '退出登录' + (nickname ? '（' + nickname + '）' : ''))),
    ...
    // ------------------------------------------------------------------
    // 在线网易云面板（NCOnePanel）：结构与 KG/QQ 面板同构（dsh-music-qq-* 样式类复用）。
    // 差异点：
    //  - 登录不是硬前提：网易云搜索/歌单/歌词/取链全部匿名可用，未登录也能浏览+播放
    //    （免费歌 320k 直链、VIP 歌 45s 试听）；登录解锁「我的歌单」与更高音质。
    //  - 扫码登录两跳直达 MUSIC_U（比 QQ 少一层换票、比酷狗少设备注册）。
    //  - YRC 逐字歌词优先，其次整行 LRC+逐句翻译。
    // ------------------------------------------------------------------
    function NCOnePanel({ panelRef }) {
      const s = useStore();
      const json = (url) => jsonGet(url).catch(() => ({ ok: false, error: '网络错误' }));
      const ncPost = (url, body) => jsonPost(url, body).catch(() => ({ ok: false, error: '网络错误' }));
      // ---- 登录态 ----
      const [loggedIn, setLoggedIn] = useState(null); // null=检测中 false=未登录 true=已登录
      ...
      const [qrKey, setQrKey] = useState('');
      const [loginMsg, setLoginMsg] = useState('');
      // 与酷狗同款：失效/过期原因单独存，避免自动出码把原因冲成「二维码获取失败」。
      const [loginReason, setLoginReason] = useState('');
      const [loginBusy, setLoginBusy] = useState(false);
      const [loginOpen, setLoginOpen] = useState(false);
      const pollTimer = useRef(null);
      // 同酷狗：登录已过期 + 正停在「网易云」页签 → 自动打开扫码弹窗出码（按页签守卫，
      // 避免在其他页签被二维码盖住；loginReason 登录成功才清，切走再切回也能补上）。
      useEffect(() => {
        if (s.tab !== 'nc') return;
        if (loginReason === '' || loginOpen) return;
        setLoginOpen(true);
      ...
      // 引擎侧检出登录过期（播放中取链失败复查）→ 立刻按未登录处理并进登录页（与 QQ/酷狗同规矩）。
      useEffect(() => {
        if (!s.ncAuthExpired) return;
        setLoggedIn(false);
        setLoginReason('网易云登录可能已失效，请重新扫码登录');
        setLayer('main'); setActivePl(null);
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [s.ncAuthExpired]);
      useEffect(() => {
        jsonGet('/dsh-music/nc/status').then((d) => {
          if (!d) return;
          setLoggedIn(!!d.loggedIn); setNickname(d.nickname || '');
          // status 只反映「cookie 是否存在」（Host 不做主动探测）；「疑似登录失效」由播放失败
          // 置位（onError → markOnlineAuthFailed），这里不主动清，避免刚置位就被抹掉。
          if (d.loggedIn) resetNCAuthExpired();
        }).catch(() => setLoggedIn(false));
        return () => { if (pollTimer.current !== null) clearTimeout(pollTimer.current); };
      }, []);
      function stopPoll() { if (pollTimer.current !== null) { clearTimeout(pollTimer.current); pollTimer.current = null; } }
      function startLogin() {
        if (loginBusy) return;
        setLoginBusy(true); setQrImage(''); setQrKey(''); setLoginMsg('正在获取二维码…');
        stopPoll();
        ncPost('/dsh-music/nc/login/start', {}).then((d) => {
          if (!d || !d.ok || !d.key) { setLoginMsg((d && d.error) || '二维码获取失败'); return; }
          setQrKey(d.key); setQrImage(d.image || '');
          setLoginMsg('请用网易云音乐 App 扫码登录');
          const tick = () => {
            fetch('/dsh-music/nc/login/check?key=' + encodeURIComponent(d.key), { cache: 'no-store' })
              .then((r) => r.json())
              .then((c) => {
                if (!c || !c.ok) { setLoginMsg((c && c.error) || '状态未知'); return; }
                if (c.status === 'success') {
                  setLoggedIn(true); setNickname(c.nickname || ''); setQrImage(''); setQrKey('');
                  setLoginOpen(false); setLoginMsg(''); setLoginReason('');
                  resetNCAuthExpired(); // 新登录态：清掉「疑似登录失效」标记
      ...
      function doLogout() {
        ncPost('/dsh-music/nc/login/logout', {}).then(() => {
          const playingNC = String(store.currentId || '').startsWith('nc:') || store.scope?.kind === 'nc';
          if (playingNC) stop();
          clearPref(PREF_PLAYBACK_NC);
          setLoggedIn(false); setNickname(''); setMinePlays([]); setMineLoaded(false);
          setBrowseTab('recommend');
          resetNCAuthExpired();
          set({ ncQueue: [], ncSource: '', ncQueueFrom: null, ncQueueRev: 0, ncLoggedIn: false, ncNickname: '' });
        }).catch(() => {});
      }
      // ---- 两层 UI：main / playlist ----
      const [layer, setLayer] = useState('main');
      const [activePl, setActivePl] = useState(null); // { name, songs, source, mine? }
      const [plLoading, setPlLoading] = useState(false);
      const [browseTab, setBrowseTab] = useState(loggedIn ? 'mine' : 'recommend'); // mine|recommend|category|tops|search
      // 搜索
      const [q, setQ] = useState('');
      ...
      const [resultTab, setResultTab] = useState('songs');
      const [qError, setQError] = useState('');
      // 搜索分页（对齐 QQ/酷狗「加载更多」：歌曲与歌单均用接口 total 判定）
      const [searchPage, setSearchPage] = useState(1);
      const [searchTotal, setSearchTotal] = useState(0);
      const [searchingMore, setSearchingMore] = useState(false);
      const [plSearchPage, setPlSearchPage] = useState(1);
      ...
      const [recs, setRecs] = useState([]);
      const [recLoading, setRecLoading] = useState(false);
      // 推荐歌单「加载更多」：官方推荐一次 30 条、接口不支持翻页，续载「全部」分类
      // 的分页歌单并去重（对齐 QQ「热门推荐 + 全部分类续页」做法）。
      const [recPage, setRecPage] = useState(1);
      const [recLoadingMore, setRecLoadingMore] = useState(false);
      const [recHasMore, setRecHasMore] = useState(true);
      const [cats, setCats] = useState([]);
      ...
      // 分类折叠/展开 + 分类歌单分页（对齐 QQ 音乐：默认折叠到 8 个分类、歌单「加载更多」）
      ...
      const [topDetail, setTopDetail] = useState(null);
      const [topLoading, setTopLoading] = useState(false);
      const [topOffset, setTopOffset] = useState(0);      // 已加载榜单歌曲数（供「加载更多」与元信息）
      const [topLoadingMore, setTopLoadingMore] = useState(false);
      const [minePlays, setMinePlays] = useState([]);
      const [mineLoaded, setMineLoaded] = useState(false);
      const [mineErr, setMineErr] = useState('');
      const [ncCollecting, setNcCollecting] = useState(false); // 收藏歌单请求进行中
      // 「加入歌单」弹窗：{ song, x, y }（锚点=歌曲行「＋」按钮的视口坐标，与 QQ/KG 同款）。
      const [ncJoin, setNcJoin] = useState(null);
      const ncJoinRef = useRef(null);
      // 搜索历史写回（与 QQ/KG 同款：最近在前、最多 10 条；以持久层为准重算，避免
      // 用滞后的组件 state 导致并发写入丢条目）。
      function pushHist(kw) {
        const t = String(kw || '').trim();
        if (!t) return;
        const next = [t, ...loadHist().filter((x) => x !== t)].slice(0, 10);
        setHist(next);
        savePref(PREF_NC_HISTORY, JSON.stringify(next));
      }
      function clearHist() { clearPref(PREF_NC_HISTORY); setHist([]); }
      // 搜索历史读取（与 QQ/酷狗同款）：挂载时读一次；Host 预置快照异步到达后就绪时重读，
      // 避免刷新后因 prefs 快照晚于面板挂载而读不到历史。
      const loadHist = () => { try { const a = JSON.parse(loadPref(PREF_NC_HISTORY)); return Array.isArray(a) ? a.filter((x) => typeof x === 'string' && x.trim()) : []; } catch { return []; } };
      useEffect(() => { setHist(loadHist()); }, []);
      useEffect(() => {
        if (!s.prefsReady) return;
        setHist(loadHist());
      ...
      // 点击搜索框外关闭历史下拉（与 QQ/酷狗同款：搜索框与 portal 到 body 的下拉
      // 本身不关闭，点其它任意位置收起）。
      useEffect(() => {
        const onDocClick = (e) => {
          if (histRef.current !== null && histRef.current.contains(e.target)) return;
      ...
      // 登录态变为已登录时：默认切到「我的歌单」并加载（QQ/酷狗同款行为）。放副作用
      // 而非登录成功回调——刷新后登录态由 /nc/status 异步确认（null→true），这里一并
      // 兜住；且避免用旧的 loggedIn 闭包值导致 refreshMine 里的「未登录直接返回」误判。
      useEffect(() => {
        if (!loggedIn) return;
        setBrowseTab('mine');
        refreshMine();
      ...
      async function loadRecommended() {
        ...
        setRecLoading(true); setBrowseErr('');
        const d = await json('/dsh-music/nc/playlists');
        if (d && d.ok) { setRecs(d.playlists || []); setRecPage(1); }
        else setBrowseErr((d && d.error) || '推荐歌单加载失败');
        setRecLoading(false);
      }
      // 推荐歌单「加载更多」：官方推荐接口一次 30 条且不支持翻页（offset 无效），
      // 续载「全部」分类的 hot 分页歌单并去重（对齐 QQ 的热门推荐 + 全部分类续页）。
      async function loadMoreRecommended() {
        if (recLoadingMore || !recHasMore || recLoading) return;
        setRecLoadingMore(true); setBrowseErr('');
        const nextPage = recPage + 1;
        const d = await json('/dsh-music/nc/playlists?category=' + encodeURIComponent('全部') + '&page=' + nextPage);
        if (d && d.ok && Array.isArray(d.playlists)) {
          const existing = new Set(recs.map((p) => String(p.id)));
          const fresh = d.playlists.filter((p) => !existing.has(String(p.id)));
          setRecs((prev) => [...prev, ...fresh]);
          setRecPage(nextPage);
          // 接口无 total：以「本页是否取满 20」判定还有更多；本页无新条目标记到底。
          if (fresh.length === 0 || d.playlists.length < 20) setRecHasMore(false);
        } else {
      ...
      async function loadCategories() {
        ...
        const d = await json('/dsh-music/nc/playlist-categories');
        if (d && d.ok) setCats(d.categories || []);
        else setBrowseErr((d && d.error) || '分类加载失败');
      }
      async function loadCategory(cat) {
        setCurCategory(cat); setCatLoading(true); setBrowseErr(''); setCatPlays([]); setCatPage(1); setCatHasMore(false);
        const d = await json('/dsh-music/nc/playlists?category=' + encodeURIComponent(cat.id));
        if (d && d.ok) {
          const pls = d.playlists || [];
          setCatPlays(pls);
          setCatHasMore(pls.length >= 20); // 接口无 total，以「本页是否取满」判定还有更多
        } else setBrowseErr((d && d.error) || '分类歌单加载失败');
        setCatLoading(false);
      }
      // 分类歌单「加载更多」（对齐 QQ 音乐：按页追加）。
      async function loadMoreCategory() {
        if (!curCategory || catLoadingMore || !catHasMore) return;
        setCatLoadingMore(true);
        const d = await json('/dsh-music/nc/playlists?category=' + encodeURIComponent(curCategory.id) + '&page=' + (catPage + 1));
        setCatLoadingMore(false);
        if (!d || !d.ok) return;
        const more = d.playlists || [];
        setCatPlays((prev) => [...prev, ...more]);
        setCatPage((p) => p + 1);
        setCatHasMore(more.length >= 20);
      }
      async function loadTopLists() {
        if (topLoaded) return;
        setTopLoading(true);
        const d = await json('/dsh-music/nc/top-lists');
        if (d && d.ok) { setTopGroups(d.groups || []); setTopLoaded(true); }
        else setBrowseErr((d && d.error) || '榜单加载失败');
        setTopLoading(false);
      }
      async function openToplist(t) {
        setTopLoading(true); setBrowseErr(''); setTopDetail(null); setTopOffset(0);
        const d = await json('/dsh-music/nc/top-songs?rankId=' + encodeURIComponent(t.id));
        setTopLoading(false);
        if (d && d.ok && d.toplist) {
          setTopDetail({ ...d.toplist, source: t.name || d.toplist.name, updateTime: t.updateTime || d.toplist.updateTime || '' });
          setTopOffset((d.toplist.songs || []).length);
        } else setBrowseErr((d && d.error) || '榜单详情加载失败');
      }
      // 榜单「加载更多」：按 offset 追加下一页（对齐 QQ/酷狗）。
      async function loadMoreTopSongs() {
        if (!topDetail || topLoadingMore) return;
        setTopLoadingMore(true);
        const d = await json('/dsh-music/nc/top-songs?rankId=' + encodeURIComponent(topDetail.id) + '&offset=' + topOffset + '&num=30');
        setTopLoadingMore(false);
        if (!d || !d.ok || !d.toplist) return;
        const more = d.toplist.songs || [];
        setTopDetail((prev) => ({ ...(prev || {}), songs: [...((prev && prev.songs) || []), ...more] }));
        setTopOffset((o) => o + more.length);
      }
      function refreshMine() {
        if (!loggedIn) return;
        setMineLoaded(true); setMineErr('');
        jsonGet('/dsh-music/nc/my-playlists').then((d) => {
          if (d && d.ok) setMinePlays(d.playlists || []);
          else if (d && d.error) { setMineErr(d.error); if (/未登录|登录态/.test(d.error)) setLoggedIn(false); }
        }).catch(() => setMineErr('网络错误'));
      }
      function openPublicPlaylist(pl) {
        setLayer('playlist'); setActivePl({ ...pl, songs: [], public: true }); setPlLoading(true);
        // 预载「我的歌单」（判定该公开歌单是否已收藏，供详情页收藏按钮点亮）。
        if (loggedIn && !mineLoaded) refreshMine();
        jsonGet('/dsh-music/nc/playlist/' + encodeURIComponent(String(pl.id))).then((d) => {
          if (store.tab !== 'nc') return;
          setPlLoading(false);
          if (d && d.ok && d.playlist) setActivePl((prev) => ({ ...(prev || {}), ...d.playlist, mine: false }));
          else setBrowseErr((d && d.error) || '歌单加载失败');
        }).catch(() => { setPlLoading(false); setBrowseErr('网络错误'); });
      }
      function openMyPlaylist(pl) {
        setLayer('playlist'); setActivePl({ ...pl, songs: [], mine: true }); setPlLoading(true);
        jsonGet('/dsh-music/nc/playlist/' + encodeURIComponent(String(pl.id))).then((d) => {
          if (store.tab !== 'nc') return;
          setPlLoading(false);
          if (d && d.ok && d.playlist) setActivePl((prev) => ({ ...(prev || {}), ...d.playlist, mine: true, kind: pl && pl.kind }));
          else setBrowseErr((d && d.error) || '歌单加载失败');
        }).catch(() => { setPlLoading(false); setBrowseErr('网络错误'); });
      }
      function backToMain() { setLayer('main'); setActivePl(null); setPlLoading(false); setTopDetail(null); }
      // 收藏 / 取消收藏当前公开歌单（eapi 主路 + weapi 备路，需登录；对齐酷狗 collectActivePlaylist）。
      async function collectActivePlaylist() {
        const pl = activePl;
        if (!pl || ncCollecting) return;
        const isCollected = minePlays.some((p) => p.kind === 'collect' && String(p.id) === String(pl.id));
        setNcCollecting(true);
        try {
          const d = await ncPost('/dsh-music/nc/playlist-collect', { id: String(pl.id), action: isCollected ? 'uncollect' : 'collect' });
          if (!d || !d.ok) throw new Error((d && d.error) || '操作失败');
          showToast((isCollected ? '已取消收藏「' : '已收藏「') + (pl.name || '') + '」', true);
          refreshMine(); // 刷新「我的歌单」，让收藏/取消反映到列表
        } catch (err) {
          showToast(String((err && err.message) || err) || '操作失败', false);
        }
        setNcCollecting(false);
      }
      // 「我的歌单」里收藏的歌单：卡片右上角「☆」取消收藏（对齐酷狗 mine 卡片）。
      async function uncollectMinePlaylist(pl) {
        const id = String((pl && pl.id) || '');
        if (!/^[0-9]+$/.test(id)) { showToast('缺少歌单 id，无法取消收藏', false); return; }
        openConfirm('取消收藏',
          '确定取消收藏歌单「' + ((pl && pl.name) || '') + '」？取消后不影响原歌单及其创作者，之后仍可在网易云重新收藏。',
          async () => {
            try {
              const d = await ncPost('/dsh-music/nc/playlist-collect', { id, action: 'uncollect' });
              if (!d || !d.ok) throw new Error((d && d.error) || '取消收藏失败');
              setMinePlays((prev) => prev.filter((p) => String(p.id) !== id));
              // 若正在详情页看的就是这个收藏歌单 → 回到我的歌单列表。
              if (layer === 'playlist' && activePl && String(activePl.id || '') === id) backToMain();
              showToast('已取消收藏「' + ((pl && pl.name) || '') + '」', true);
            } catch (err) { showToast(String((err && err.message) || err) || '取消收藏失败', false); }
          }, '取消收藏', true);
      }
      // ---- 网易云歌曲「加入歌单」弹窗（对齐 QQ/KG：歌曲行「＋」→ 弹窗列我的歌单 + 新建）----
      function openNCJoin(song, e) {
        const r = e.currentTarget.getBoundingClientRect();
        if (minePlays.length === 0 && !mineLoaded) refreshMine();
        setNcJoin({ song, x: r.right, y: r.top });
      }
      async function ncJoinAddTo(pl) {
        const song = ncJoin && ncJoin.song;
        if (!song) return;
        const name = (pl && pl.name) || '歌单';
        try {
          const d = await ncPost('/dsh-music/nc/playlist-add', { id: String(pl.id), songIds: [String(song.id)] });
          if (!d || !d.ok) throw new Error((d && d.error) || '加入歌单失败');
          if (pl) {
            const pid = String(pl.id);
            // 本地乐观 +1（refreshMine 前先更新，避免闪烁；随后异步刷新为准）
      ...
      async function ncJoinCreate() {
        const song = ncJoin && ncJoin.song;
        openPrompt('新建歌单名称', '', async (trimmed) => {
          if (!trimmed) return;
          try {
            const d = await ncPost('/dsh-music/nc/playlist-create', { name: trimmed });
            if (!d || !d.ok || !d.playlist) throw new Error((d && d.error) || '创建歌单失败');
            const created = d.playlist;
            if (song) {
              const add = await ncPost('/dsh-music/nc/playlist-add', { id: String(created.id), songIds: [String(song.id)] });
              if (!add || !add.ok) throw new Error((add && add.error) || '加入新歌单失败');
            }
            refreshMine();
            setNcJoin(null);
      ...
      const ncJoinMenu = ncJoin ? portalToBody((() => {
        const openUp = (ncJoin.y || 0) > ((window.innerHeight || 0) - 240);
        const style = {
          left: Math.max(8, (ncJoin.x || 0) - 150),
          top: openUp ? (ncJoin.y || 0) - 6 : (ncJoin.y || 0) + 8,
          transform: openUp ? 'translateY(-100%)' : 'none',
        };
        return React.createElement('div', { className: 'dsh-music-add-pop', ref: ncJoinRef, style },
          (() => {
            // 收藏的歌单是别人的，不可加入歌曲；只有自建歌单（含「我喜欢的音乐」）可作加入目标。
            const ownPlays = minePlays.filter((p) => p.kind !== 'collect');
            return ownPlays.length > 0 ? ownPlays.map((p) => React.createElement('button', {
              key: String(p.id), className: 'dsh-music-add-pop-item',
              title: '加入「' + p.name + '」',
              onClick: () => ncJoinAddTo(p),
            }, p.name + (p.trackCount ? '（' + p.trackCount + '）' : ''))) : React.createElement('div', { className: 'dsh-music-hint', style: { padding: '2px 8px' } }, '暂无自建歌单，请先创建。');
          })(),
          React.createElement('button', { className: 'dsh-music-add-pop-item new', onClick: ncJoinCreate }, '＋ 新建歌单'),
        );
      })()) : null;
      // 从「我的歌单」当前歌单移除歌曲（weapi manipulate/tracks op=del）。
      async function removeFromActivePlaylist(song) {
        const pl = activePl;
        if (!pl || !pl.id) return;
        const id = String(song.id || '');
        try {
          const d = await ncPost('/dsh-music/nc/playlist-remove', { id: String(pl.id), songIds: [id] });
          if (!d || !d.ok) throw new Error((d && d.error) || '从歌单移除失败');
          // 就地移除该行，并刷新「我的歌单」数目。
          setActivePl((cur) => (cur ? { ...cur, songs: (cur.songs || []).filter((t) => String(t.id) !== id) } : cur));
          refreshMine();
          showToast('已从歌单移除', true);
        } catch (err) { showToast(String((err && err.message) || err) || '从歌单移除失败', false); }
      }
      // 删除自建歌单（weapi /weapi/playlist/delete；「我喜欢」不可删、收藏歌单走取消收藏）。
      async function deleteMinePlaylist(pl) {
        const id = String((pl && pl.id) || '');
        if (!/^[0-9]+$/.test(id)) { showToast('缺少歌单 id，无法删除', false); return; }
        openConfirm('删除歌单', '确定删除歌单「' + ((pl && pl.name) || '') + '」？删除后不可恢复。', async () => {
          try {
            const d = await ncPost('/dsh-music/nc/playlist-delete', { id });
            if (!d || !d.ok) throw new Error((d && d.error) || '删除失败');
            setMinePlays((prev) => prev.filter((p) => String(p.id) !== id));
            if (layer === 'playlist' && activePl && String(activePl.id || '') === id) backToMain();
            showToast('已删除歌单「' + ((pl && pl.name) || '') + '」', true);
          } catch (err) { showToast(String((err && err.message) || err) || '删除失败', false); }
        }, '删除', true);
      }
      // 工具栏「播放列表」：把当前网易云播放队列当歌单打开（对齐酷狗 openQueue）。
      function openNCQueue() {
        const q = store.ncQueue || [];
        if (q.length === 0) return;
        setLayer('playlist'); setActivePl({ name: store.ncSource || '在线播放列表', songs: q, mine: false });
      }
      function doSearch(kw) {
        const w = String(kw === undefined ? q : kw).trim();
        if (w === '' || searching) return;
        if (kw !== undefined) setQ(w);
        setHistOpen(false); // 发起搜索即收起历史下拉（对齐 QQ/酷狗）
        setSearching(true); setSearched(true); setQError(''); setResults([]); setPlResults([]);
        setSearchPage(1); setSearchTotal(0);
        setPlSearchPage(1); setPlSearchTotal(0);
        pushHist(w);
        jsonGet('/dsh-music/nc/search?w=' + encodeURIComponent(w)).then((d) => {
          if (!d || !d.ok) { setQError((d && d.error) || '搜索失败'); return; }
          setResults(d.results || []);
          setSearchTotal(Number(d.total) || (d.results || []).length);
        }).catch(() => setQError('网络错误')).finally(() => setSearching(false));
        jsonGet('/dsh-music/nc/playlist-search?w=' + encodeURIComponent(w)).then((d) => {
          if (d && d.ok) { setPlResults(d.playlists || []); setPlSearchTotal(Number(d.total) || (d.playlists || []).length); }
        }).catch(() => {});
      }
      // 「加载更多」（分页追加，与 QQ/酷狗同款；搜歌曲与搜歌单各自独立翻页）。
      function loadMoreSongs() {
        const w = q.trim();
        if (w === '' || searchingMore) return;
        setSearchingMore(true);
        jsonGet('/dsh-music/nc/search?w=' + encodeURIComponent(w) + '&page=' + (searchPage + 1)).then((d) => {
          if (!d || !d.ok) return;
          setSearchPage((p) => p + 1);
          setResults((prev) => [...prev, ...(d.results || [])]);
          if (d.total !== undefined) setSearchTotal(Number(d.total));
        }).catch(() => {}).finally(() => setSearchingMore(false));
      }
      function loadMorePls() {
        const w = q.trim();
        if (w === '' || plSearchingMore) return;
        setPlSearchingMore(true);
        jsonGet('/dsh-music/nc/playlist-search?w=' + encodeURIComponent(w) + '&page=' + (plSearchPage + 1)).then((d) => {
          if (!d || !d.ok) return;
          setPlSearchPage((p) => p + 1);
          setPlResults((prev) => [...prev, ...(d.playlists || [])]);
          if (d.total !== undefined) setPlSearchTotal(Number(d.total));
        }).catch(() => {}).finally(() => setPlSearchingMore(false));
      }
      function playAllSongs(songs, name, from) {
        if (!songs || songs.length === 0) return;
        startNCPlayback(songs[0], songs, name || '在线', from);
      }
      // ---- 渲染辅助 ----
      const fmtCount = (n) => { const v = Number(n) || 0; if (v >= 1e8) return (v / 1e8).toFixed(1).replace(/\.0$/, '') + '亿'; if (v >= 1e4) return (v / 1e4).toFixed(1).replace(/\.0$/, '') + '万'; return String(v); };
      const ncSongRow = (song, queue, sourceLabel, opts = {}) => {
        const id = String(song.id || '');
        const active = s.currentId === 'nc:' + id;
        const playing = active && s.playing;
        // 网易云 fee：0 免费 / 1 VIP / 4 数字专辑 / 8 版权受限（低音质免费可听）。
        const fee = Number(song.fee) || 0;
        const vip = fee === 1 || fee === 4;
        const artists = (song.artists || []).join('/');
        const queueFrom = (activePl && activePl.mine)
          ? { kind: 'my', listId: String(activePl.id || '') }
          : (activePl && activePl.id === undefined ? (s.ncQueueFrom || null) : null);
        return React.createElement('div', { key: id, className: 'dsh-music-track-row' + (active ? ' active' : '') },
          React.createElement('button', {
            className: 'dsh-music-track' + (active ? ' active' : ''),
            title: song.title + ' - ' + artists,
            onClick: () => { if (active) togglePlay(); else startNCPlayback(song, queue, sourceLabel, queueFrom); },
          },
            React.createElement('span', { className: 'dsh-music-track-name qq' },
              React.createElement('span', { className: 'dsh-music-track-title' }, (playing ? '▶ ' : '') + (song.title || '(未知曲名)')),
              vip ? React.createElement('span', { className: 'dsh-music-online-tag vip' }, fee === 4 ? '专辑' : 'VIP') : null),
            React.createElement('span', { className: 'dsh-music-online-tag' }, artists || (song.album || '网易云'))),
          // 我的歌单详情：歌曲已在该歌单 → 显示「−」从该歌单移除；收藏歌单（kind=collect，
          // 别人的）只读不可改；其它场景显示「＋」加入歌单。
          opts.mine && activePl && activePl.id && activePl.kind !== 'collect'
            ? React.createElement('button', {
              className: 'dsh-music-playlist-mini remove',
              title: '从「' + ((activePl && activePl.name) || '当前歌单') + '」移除',
              onClick: (e) => { e.stopPropagation(); removeFromActivePlaylist(song); },
            ...
            : React.createElement('button', {
              className: 'dsh-music-playlist-mini add',
              title: '加入我的歌单',
              onClick: (e) => { e.stopPropagation(); openNCJoin(song, e); },
            }, '＋'));
      };
      const plCard = (pl) => React.createElement('button', {
        key: String(pl.id), className: 'dsh-music-playlist-card',
        title: (pl.description || pl.name || '') + ' - ' + (pl.creator || ''),
        onClick: () => openPublicPlaylist(pl),
      },
        plCoverEl(pl),
        React.createElement('span', { className: 'dsh-music-playlist-info' },
          React.createElement('span', { className: 'dsh-music-playlist-name' }, pl.name || '(未命名歌单)'),
          React.createElement('span', { className: 'dsh-music-playlist-meta' },
            (pl.trackCount ? pl.trackCount + ' 首' : '') + (pl.playCount ? ' · 播放 ' + fmtCount(pl.playCount) : '') + (pl.creator ? ' · ' + pl.creator : ''))));
      const catTab = (cat) => React.createElement('button', {
        key: cat.id, className: 'dsh-music-qq-cat' + (curCategory && curCategory.id === cat.id ? ' active' : ''),
        onClick: () => loadCategory(cat),
      }, cat.name);
      const browseTabBtn = (key, label) => React.createElement('button', {
        ...
        onClick: () => {
          ...
          else if (key === 'mine' && !mineLoaded) refreshMine();
        },
      }, label);
      const resultTabBtn = (key, label) => React.createElement('button', {
        className: 'dsh-music-qq-resulttab' + (resultTab === key ? ' active' : ''),
      ...
      const searchBox = React.createElement('div', { className: 'dsh-music-qq-search', ref: histRef },
        React.createElement('div', { className: 'dsh-music-qq-inputwrap' },
          React.createElement('input', {
            className: 'dsh-music-qq-input', type: 'text', placeholder: '搜索网易云音乐（歌曲 / 歌单）',
            value: q,
            onChange: (e) => { setQ(e.target.value); if (e.target.value === '') setSearched(false); },
            onKeyDown: (e) => { if (e.key === 'Enter') doSearch(); },
            onFocus: () => { if (hist.length > 0) setHistOpen(true); },
      ...
      const histPop = (histOpen && hist.length > 0)
        ...
        : null;
      // 扫码框以面板中心为基准居中。
      const ncLoginStyle = panelCenterStyle(panelRef, loginOpen, 170, 460);
      const loginModal = loginOpen ? portalToBody(React.createElement('div', { className: 'dsh-music-picker-overlay' },
        React.createElement('div', { className: 'dsh-music-picker qq-login', style: ncLoginStyle },
          React.createElement('div', { className: 'dsh-music-picker-head' },
            React.createElement('span', { className: 'dsh-music-picker-title' }, '网易云音乐登录'),
            React.createElement('button', { className: 'dsh-music-icon-btn', title: '关闭', onClick: () => setLoginOpen(false) }, '✕')),
          React.createElement('div', { className: 'dsh-music-qq-login-body' },
            loginBusy && qrImage === '' ? React.createElement('div', { className: 'dsh-music-loading' }, '生成二维码…')
              : qrImage ? React.createElement('img', { className: 'dsh-music-qq-qr', src: qrImage, alt: '网易云登录二维码' }) : null,
            React.createElement('div', { className: 'dsh-music-qq-login-status' }, loginMsg || ''),
            loginMsg !== '登录成功' ? React.createElement('div', { className: 'dsh-music-qq-login-actions' },
              React.createElement('button', { className: 'dsh-music-settings-btn', onClick: startLogin }, '刷新二维码'),
              React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => setLoginOpen(false) }, '取消')) : null,
      ...
      // 未登录：与 QQ/酷狗面板一致——居中扫码登录按钮 + 免责声明，登录后才展示浏览/搜索。
      // （网易云搜索/歌词/取链接口匿名可用，但面板 UX 与其他两个在线源保持统一门禁；
      //   匿名取链能力留给 agent 的 music_play source=netease 与本地歌词兜底链使用。）
      if (!loggedIn) {
        return React.createElement('div', { className: 'dsh-music-qq dsh-music-qq-login' },
          React.createElement('div', { className: 'dsh-music-qq-login-center' },
            (loginReason || loginMsg) ? React.createElement('div', { className: 'dsh-music-qq-login-dead' }, loginReason || loginMsg) : null,
            React.createElement('button', { className: 'dsh-music-qq-login-btn', onClick: () => { setLoginOpen(true); startLogin(); } }, '网易云音乐APP登录'),
            React.createElement('div', { className: 'dsh-music-qq-login-warn' },
              React.createElement('div', { className: 'dsh-music-qq-login-warn-title' }, '使用声明（重要）'),
              React.createElement('div', { className: 'dsh-music-qq-login-warn-p' },
                '在线网易云音乐功能通过非官方接口访问网易云音乐资源，所播放的内容版权归版权方及网易云音乐平台所有。本功能仅供个人学习、技术研究、日常试听使用，严禁用于任何商业用途、公开传播、二次分发或盈利行为。使用本功能即表示您已知悉并同意：'),
              React.createElement('div', { className: 'dsh-music-qq-login-warn-item' },
                React.createElement('span', { className: 'dsh-music-qq-login-warn-num' }, '1'),
                React.createElement('span', null, '您应对自己的使用行为及其后果负责。')),
              React.createElement('div', { className: 'dsh-music-qq-login-warn-item' },
      ...
      if (layer === 'playlist' && activePl) {
        const pl = activePl;
        // 公开歌单是否已收藏：在我的歌单里找 kind=collect 且 id 匹配的项（网易云 user/playlist
        // 的收藏歌单直接用原歌单 id，无需像酷狗那样比对内部 gid）。
        const isCollected = !!pl.public && minePlays.some((p) => p.kind === 'collect' && String(p.id) === String(pl.id));
        const collectBtn = (pl.public && pl.id)
          ? React.createElement('button', {
            className: 'dsh-music-qq-collect-pl' + (isCollected ? ' collected' : ''),
            title: isCollected ? '已收藏到我的歌单' : '收藏这个歌单到我的歌单',
            onClick: (e) => { e.stopPropagation(); collectActivePlaylist(); },
            disabled: ncCollecting,
          }, isCollected ? '★ 已收藏' : (ncCollecting ? '收藏中…' : '☆ 收藏'))
          : null;
        return React.createElement('div', { className: 'dsh-music-qq' },
          React.createElement('div', { className: 'dsh-music-qq-head' },
            React.createElement('div', { className: 'dsh-music-qq-detail-head' },
            ...
            ((pl.songs && pl.songs.length) || collectBtn)
              ? React.createElement('div', { className: 'dsh-music-qq-pl-actions' },
                (pl.songs && pl.songs.length)
                  ? React.createElement('button', { className: 'dsh-music-qq-playall', onClick: () => playAllSongs(pl.songs, pl.name, pl.mine ? { kind: 'my', listId: String(pl.id || '') } : null) }, '▶ 播放全部')
                  : null,
                collectBtn)
              : null),
          React.createElement('div', { className: 'dsh-music-qq-body' },
            plLoading
              ? React.createElement('div', { className: 'dsh-music-hint' }, '加载中…')
              : (pl.songs && pl.songs.length
                ? React.createElement('div', null, pl.songs.map((song) => ncSongRow(song, pl.songs, pl.name, { mine: !!pl.mine })))
                : React.createElement('div', { className: 'dsh-music-empty' }, '暂无歌曲。'))),
          loginModal,
          ncJoinMenu);
      }
      ...
      if (browseTab === 'search') {
        ...
        if (searching) {
          ...
          resultContent = React.createElement('div', null, results.map((song) => ncSongRow(song, results, '搜索结果')), songMoreBtn);
        }
        searchRow = React.createElement('div', { className: 'dsh-music-qq-searchrow' }, searchBox, histPop);
        // body 只放结果内容（在 .dsh-music-qq-body 滚动容器内）。
        body = searched ? React.createElement('div', null, resultContent) : null;
      } else if (browseTab === 'recommend') {
        const recMoreBtn = recs.length > 0 && recHasMore
          ? React.createElement('div', { className: 'dsh-music-qq-loadmore' },
            React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: loadMoreRecommended },
              recLoadingMore ? '加载中…' : '加载更多'))
          : null;
        body = recLoading
          ...
          : (recs.length > 0
            ? React.createElement('div', null,
              React.createElement('div', { className: 'dsh-music-pl-grid' }, recs.map((p) => plCard(p))),
              recMoreBtn)
            : React.createElement('div', { className: 'dsh-music-empty' }, '暂无推荐歌单。'));
      } else if (browseTab === 'category') {
        const catMoreBtn = curCategory && catPlays.length > 0 && catHasMore
          ? React.createElement('div', { className: 'dsh-music-qq-loadmore' },
            React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: loadMoreCategory },
              catLoadingMore ? '加载中…' : '加载更多'))
          : null;
        // 分类折叠/展开（对齐 QQ 音乐：默认折叠到前 8 个，可「展开全部分类 / 收起」）。
        const CAT_COLLAPSED_COUNT = 8;
        const shownCats = catExpanded ? cats : cats.slice(0, CAT_COLLAPSED_COUNT);
        const catToggle = cats.length > CAT_COLLAPSED_COUNT
          ? React.createElement('button', { className: 'dsh-music-qq-cat-toggle', onClick: () => setCatExpanded((v) => !v) },
            catExpanded ? '收起' : '展开全部分类（' + cats.length + '）')
          : null;
        body = React.createElement('div', null,
          React.createElement('div', { className: 'dsh-music-qq-cats' },
            shownCats.length ? shownCats.map(catTab) : React.createElement('span', { className: 'dsh-music-hint' }, '加载分类中…')),
          catToggle,
          curCategory ? React.createElement('div', null,
            (catLoading ? React.createElement('div', { className: 'dsh-music-hint' }, '加载中…')
              : (catPlays.length > 0 ? React.createElement('div', { className: 'dsh-music-pl-grid' }, catPlays.map((p) => plCard(p)))
                : React.createElement('div', { className: 'dsh-music-hint' }, '该分类暂无歌单。'))),
            catMoreBtn) : null);
      } else if (browseTab === 'tops') {
        if (topDetail) {
          const rows = topDetail.songs && topDetail.songs.length
            ? topDetail.songs.map((song) => ncSongRow(song, topDetail.songs, topDetail.source || topDetail.name))
            : React.createElement('div', { className: 'dsh-music-hint' }, '该榜单暂无歌曲。');
          const topHasMore = topDetail.total ? (topOffset < topDetail.total) : false;
          const moreBtn = topHasMore
            ? React.createElement('div', { className: 'dsh-music-qq-loadmore' },
              React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: loadMoreTopSongs },
                topLoadingMore ? '加载中…' : '加载更多'))
            : null;
          body = React.createElement('div', null, rows, moreBtn);
        } else {
          const lists = (topGroups[0] && topGroups[0].toplists) || [];
          body = topLoading
            ? React.createElement('div', { className: 'dsh-music-hint' }, '加载中…')
            : (lists.length > 0 ? React.createElement('div', { className: 'dsh-music-pl-grid' },
                lists.map((t) => React.createElement('button', {
                  key: t.id, className: 'dsh-music-playlist-card',
                  title: t.name, onClick: () => openToplist(t),
                }, plCoverEl({ cover: t.cover, name: t.name }),
                  React.createElement('span', { className: 'dsh-music-playlist-info' },
                    React.createElement('span', { className: 'dsh-music-playlist-name' }, t.name),
                    React.createElement('span', { className: 'dsh-music-playlist-meta' }, t.updateTime || t.intro || '')))))
              : React.createElement('div', { className: 'dsh-music-empty' }, '暂无榜单。'));
        }
      } else if (browseTab === 'mine') {
        if (!loggedIn) {
          body = React.createElement('div', { className: 'dsh-music-qq-login-center' },
            React.createElement('button', { className: 'dsh-music-qq-login-btn', onClick: () => { setLoginOpen(true); startLogin(); } }, '登录网易云音乐'),
            React.createElement('div', { className: 'dsh-music-hint' }, '登录后可查看我的歌单（匿名也可浏览/搜索/试听）。'));
        } else if (mineErr) {
          body = React.createElement('div', { className: 'dsh-music-error' }, mineErr);
        } else if (!mineLoaded) {
          body = React.createElement('div', { className: 'dsh-music-hint' }, '加载中…');
        } else if (minePlays.length > 0) {
          body = React.createElement('div', { className: 'dsh-music-pl-grid' },
            minePlays.map((p) => {
              // 「我喜欢的音乐」（首个）与自建/收藏歌单统一卡片。
              const meta = (p.trackCount ? p.trackCount + ' 首' : '') + (p.playCount ? ' · 播放 ' + fmtCount(p.playCount) : '');
              // 「我的歌单」卡片类别徽章（对齐 QQ/酷狗）：系统默认歌单（我喜欢的音乐）
              // 标「默认」（主题色）、自建歌单标「自建」、收藏的歌单标「收藏」（金色）。
              const isCollect = p.kind === 'collect';
              const kindTag = React.createElement('span', {
                className: 'dsh-music-online-tag' + (p.isLike ? ' default' : (isCollect ? ' collect' : '')),
                title: p.isLike ? '系统默认歌单（我喜欢）' : (isCollect ? '收藏的歌单' : '自己创建的歌单'),
              }, p.isLike ? '默认' : (isCollect ? '收藏' : '自建'));
              const card = React.createElement('button', {
                key: String(p.id), className: 'dsh-music-playlist-card',
                title: (p.name || '') + ' - ' + (p.creator || ''),
                onClick: () => openMyPlaylist(p),
              }, plCoverEl(p),
                React.createElement('span', { className: 'dsh-music-playlist-info' },
                  React.createElement('span', { className: 'dsh-music-playlist-name-row' },
                    React.createElement('span', { className: 'dsh-music-playlist-name' }, p.name || '(未命名歌单)'),
                    kindTag),
                  React.createElement('span', { className: 'dsh-music-playlist-meta' },
                    (isCollect && p.creator ? p.creator + ' · ' : '') + meta)));
              // 「我喜欢的音乐」不可删除/取消收藏；收藏歌单（kind=collect）右上角
              // 「☆」取消收藏；自建歌单（kind=own）右上角「✕」删除（对齐 QQ/酷狗 mine 卡片）。
              if (p.isLike) return card;
              const isOwn = p.kind === 'own';
              return React.createElement('div', { key: String(p.id), className: 'dsh-music-qq-mine-card' },
                card,
                isOwn
                  ? React.createElement('button', {
                    className: 'dsh-music-qq-mine-del',
                    title: '删除歌单「' + (p.name || '') + '」',
                    onClick: (e) => { e.stopPropagation(); deleteMinePlaylist(p); },
                  }, '✕')
                  : React.createElement('button', {
                    className: 'dsh-music-qq-mine-del uncollect',
                    title: '取消收藏歌单「' + (p.name || '') + '」',
                    onClick: (e) => { e.stopPropagation(); uncollectMinePlaylist(p); },
                  }, '☆'));
            }));
        } else {
          body = React.createElement('div', { className: 'dsh-music-hint' }, '暂无歌单。可到网易云音乐 App 创建或收藏歌单后再来查看。');
        }
      }
      const head = React.createElement('div', { className: 'dsh-music-qq-head' },
        React.createElement('div', { className: 'dsh-music-qq-toolbar' },
          React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: openNCQueue }, '播放列表'),
          React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: doLogout }, '退出登录' + (nickname ? '（' + nickname + '）' : ''))),
        React.createElement('div', { className: 'dsh-music-qq-viewtabs' },
          browseTabBtn('mine', '我的歌单'),
          browseTabBtn('recommend', '推荐歌单'),
          ...
          browseTabBtn('tops', '排行榜'),
          browseTabBtn('search', '搜索')));
      // 榜单详情头（返回 + 封面/榜名/元信息 + 播放全部）放在滚动容器 .dsh-music-qq-body
      // 之外（与 QQ/酷狗一致）：头部固定、滚动条从歌曲列表开始，不被头遮挡。
      const topHeadRow = (browseTab === 'tops' && topDetail)
        ? React.createElement('div', { className: 'dsh-music-qq-tophead-fixed' },
            React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: () => setTopDetail(null) }, '← 返回'),
            React.createElement('div', { className: 'dsh-music-qq-topdetail-head' },
              topDetail.cover ? React.createElement('img', {
                className: 'dsh-music-playlist-cover', src: topDetail.cover, alt: '', loading: 'lazy',
                onError: (e) => { e.currentTarget.style.display = 'none'; },
              ...
              React.createElement('div', null,
                React.createElement('div', { className: 'dsh-music-playlist-name' }, topDetail.name || '排行榜'),
                React.createElement('div', { className: 'dsh-music-hint' },
                  (topDetail.updateTime ? '更新于 ' + topDetail.updateTime + ' · ' : '')
                  + (topDetail.total ? (topDetail.songs || []).length + ' / ' + topDetail.total + ' 首' : '')))),
            (topDetail.songs && topDetail.songs.length)
              ? React.createElement('button', { className: 'dsh-music-qq-playall', onClick: () => playAllSongs(topDetail.songs, topDetail.source || topDetail.name) }, '▶ 播放全部')
              : null)
        : null;
      return React.createElement('div', { className: 'dsh-music-qq' },
        head,
    ...
    // ===================== 网络电台面板（RadioPanel）=====================
    // 目录源 radio-browser（Host /dsh-music/radio/*，无需登录）。本地视图：
    //   view: 'top'（热门）| 'cn'（中文）| 'search'（搜索）| 'mine'（我的电台=收藏）| 'recent'（最近播放）
    // 热门/中文两视图支持按主题分组浏览：view 顶部有分组 pill 行（如 全部/新闻/音乐…），
    // 点哪组只拉哪组的数据（Host 按 group 透传 tag 给 radio-browser）。
    // 行点击播放（直播流，无进度），收藏用独立 ♥ 钮（存 Host radio.json，不依赖远端账号；
    // ♥ 为 HeartIcon SVG：收藏实心 / 未收藏空心描边，与播放条收藏钮同源同款）。
    // 复用 .dsh-music-qq* 布局外壳与 .dsh-music-track 行样式，保持与在线音乐面板一致。
    // 主题分组 key 必须与 lib/radio.js 的 CN_GROUP_TAGS / WORLD_GROUP_TAGS 一致。
    const RADIO_TOP_GROUPS = [
      { key: 'all', label: '全部' }, { key: 'music', label: '音乐' }, { key: 'news', label: '新闻' },
      { key: 'classical', label: '古典' }, { key: 'rock', label: '摇滚' }, { key: 'jazz', label: '爵士' }, { key: 'talk', label: '谈话' },
    ]
    const RADIO_CN_GROUPS = [
      { key: 'all', label: '全部' }, { key: 'news', label: '新闻' }, { key: 'music', label: '音乐' },
      { key: 'traffic', label: '交通' }, { key: 'economics', label: '财经' }, { key: 'literature', label: '文艺' },
      { key: 'storytelling', label: '故事' }, { key: 'sport', label: '体育' },
    ]
    function RadioPanel({ panelRef }) {
      const s = useStore();
      const json = (url) => jsonGet(url).catch(() => ({ ok: false, error: '网络错误' }));
      const [view, setView] = useState('mine'); // mine | recent | cn | top | search
      // 当前所选主题分组（热门/中文各自独立记忆；默认「全部」）。
      const [topGroup, setTopGroup] = useState('all');
      const [cnGroup, setCnGroup] = useState('all');
      const [q, setQ] = useState('');
      const [hist, setHist] = useState([]);
      const [histOpen, setHistOpen] = useState(false);
      const histRef = useRef(null);
      const histPopRef = useRef(null);
      const [rows, setRows] = useState(null);   // null=loading [] =空（mine/recent 共用）
      // 热门/中文目录：按 group 分页累积缓存（本面板会话内，切回不重拉）。
      // topMap / cnMap：{ [group]: { rows: [...], offset: N, done: bool } }；
      // 每页 PAGE 条，「加载更多」带 offset 追加下一页；done=true 表示目录已到底
      //（上次返回不足一页）不再显示加载按钮。loadingRef 防重复请求。
      const PAGE = 50;
      ...
      const [dirFail, setDirFail] = useState(null);
      const [searching, setSearching] = useState(false);
      // 搜索结果独立存放（与 rows 分开）：搜索视图有自己的一份列表，切走再切回
      // 「搜索」时仍显示上次搜索结果，绝不会把热门/收藏列表显示在搜索页里。
      // null = 尚未搜索（显示引导）；[] = 搜索过但无结果。
      const [searchResults, setSearchResults] = useState(null);
      // 收藏状态（从 store.radioFavs 派生：currentId 的台是否已收藏）。
      const uidOf = (st) => String((st && (st.id || st.stationuuid)) || '');
      // ---- 数据加载（热门/中文：按组分页，首屏 50 + 加载更多）----
      // kind: 'top' | 'cn'。已有该组缓存且 !more → 直接返回；否则拉 offset 页追加。
      // 首屏失败自动退避重试 DIR_RETRY_MAX 次（上游 radio-browser 多镜像偶发全部不可达，
      // 稍候通常可恢复；「加载更多」失败不自动重试，靠用户再点）。重试耗尽仍失败：
      // 标 dirFail → 视图显示「加载失败 + 重试」而不是永远停在加载中。用户切走该
      // 视图/分组/卸载组件后，异步结果只更新缓存、不再落地错误/失败态。
      const DIR_RETRY_MAX = 2; // 自动重试次数（首屏：总尝试 = 1 + DIR_RETRY_MAX）
      const dirRetryTimer = useRef(null); // 退避等待定时器（卸载时清理）
      const aliveRef = useRef(true);      // 组件存活标记（卸载后不再继续重试）
      // 「当前正在看的目录」：{kind:'top'|'cn', group} 或 null（在其它视图）。异步加载
      // 完成后据此判断结果是否仍应落地到 err/dirFail（避免后台重试失败串到别的视图）。
      const activeDirRef = useRef(null);
      useEffect(() => {
        activeDirRef.current = (view === 'top' || view === 'cn') ? { kind: view, group: view === 'top' ? topGroup : cnGroup } : null;
      }, [view, topGroup, cnGroup]);
      // 卸载清理：取消退避等待，停止后续重试。
      useEffect(() => {
        aliveRef.current = true;
        return () => {
          aliveRef.current = false;
          if (dirRetryTimer.current !== null) { try { clearTimeout(dirRetryTimer.current); } catch { /* ignore */ } dirRetryTimer.current = null; }
        };
      }, []);
      async function loadDirPage(kind, group, more) {
        const g = group || 'all';
        const busyRef = kind === 'top' ? topBusy : cnBusy;
        const setMap = kind === 'top' ? setTopMap : setCnMap;
        const urlBase = kind === 'top' ? '/dsh-music/radio/top' : '/dsh-music/radio/cn';
        const cur = (kind === 'top' ? topMap : cnMap)[g];
        if (cur !== undefined && cur.done && !more) return; // 已到底并缓存 → 切回不再重拉
        if (cur !== undefined && cur.offset > 0 && !more) return; // 已缓存并翻过页 → 切回不重拉
        if (cur !== undefined && cur.offset === 0 && !more && cur.rows.length > 0) return; // 首屏已缓存
        if (cur !== undefined && cur.done && more) return; // 已到底
        if (busyRef.current[g]) return; // 防重复
        const stillActive = () => {
          const a = activeDirRef.current;
          return aliveRef.current && a !== null && a.kind === kind && a.group === g;
        };
        ...
        const qs = '?limit=' + PAGE + '&offset=' + offset + (g !== 'all' ? '&group=' + encodeURIComponent(g) : '');
        let d = await json(urlBase + qs);
        // 首屏失败（无缓存可显示）自动退避重试：退避 0.6s / 1.2s（与播放流 RADIO_RETRY_MAX
        // 同款节奏）。用户已切走/组件已卸载则放弃后续重试（busy 由下方统一释放）。
        let retried = 0;
        while (!(d && d.ok && Array.isArray(d.stations)) && !more && retried < DIR_RETRY_MAX && stillActive()) {
          await sleep(600 * (retried + 1));
          if (!stillActive()) break;
          d = await json(urlBase + qs);
          retried++;
        }
        ...
        if (d && d.ok && Array.isArray(d.stations)) {
          ...
          // 按 station id 去重追加（跨页可能因上游翻页/重复条目撞车）。
          const seen = new Set(prev.map((st) => uidOf(st) || st.url));
          const added = page.filter((st) => !seen.has(uidOf(st) || st.url));
          const nextRows = prev.concat(added);
          setMap((m) => ({ ...m, [g]: { rows: nextRows, offset: offset + page.length, done: page.length < PAGE } }));
          // 成功结果写入缓存（即使已切走也缓存，切回即显）；错误提示只落当前视图。
          if (showResult) {
            if (more && page.length === 0) setErr('没有更多电台了');
            else if (nextRows.length === 0) setErr(kind === 'top' ? '热门电台为空（目录源可能不可达）' : '没有找到可播的中文电台（目录源可能不可达）');
          }
        } else if (showResult) {
          const msg = (d && d.error) || (kind === 'top' ? '热门电台加载失败' : '中文电台加载失败');
          setErr(msg);
          // 首屏失败（不是「加载更多」）：标记失败 → 视图区显示手动重试按钮。
          if (!more) setDirFail({ kind, group: g, msg });
        }
      }
      const loadTop = (g, more) => loadDirPage('top', g, more);
      const loadCn = (g, more) => loadDirPage('cn', g, more);
      async function loadFavs() {
        setErr(''); setRows(null);
        const d = await json('/dsh-music/radio/favs');
        if (d && d.ok) {
          setRows(d.favs || []);
          if (s.radioFavs === undefined || (d.favs || []).length !== (s.radioFavs || []).length) set({ radioFavs: d.favs || [], radioFavLoaded: true });
        } else setErr((d && d.error) || '收藏加载失败');
      }
      async function loadRecent() {
        setErr(''); setRows(null);
        const d = await json('/dsh-music/radio/recent');
        if (d && d.ok) setRows(d.recent || []);
        else setErr((d && d.error) || '最近播放加载失败');
      }
      async function doSearch(kw) {
        const t = String(kw || '').trim();
        if (t === '') return;
        setQ(t); setErr(''); setSearching(true); setSearchResults(null);
        setHistOpen(false);
        const next = [t, ...loadHist().filter((x) => x !== t)].slice(0, 10);
        savePref(PREF_RADIO_HISTORY, JSON.stringify(next));
        setHist(next);
        const d = await json('/dsh-music/radio/search?name=' + encodeURIComponent(t) + '&limit=30');
        setSearching(false);
        if (d && d.ok && Array.isArray(d.stations)) {
          setSearchResults(d.stations);
          if (d.stations.length === 0) setErr('没有找到与「' + t + '」匹配的电台（试试国家代码或标签，如 China）');
        } else setErr((d && d.error) || '搜索失败');
      }
      const loadHist = () => { try { const a = JSON.parse(loadPref(PREF_RADIO_HISTORY)); return Array.isArray(a) ? a.filter((x) => typeof x === 'string' && x.trim()) : []; } catch { return []; } };
      useEffect(() => { setHist(loadHist()); }, []);
      useEffect(() => { if (!s.prefsReady) return; setHist(loadHist()); }, [s.prefsReady]);
      useEffect(() => {
        const onDocClick = (e) => {
          if (histRef.current !== null && histRef.current.contains(e.target)) return;
          if (histPopRef.current !== null && histPopRef.current.contains(e.target)) return;
      ...
      // 视图切换时加载对应数据（初次挂载 + 切 tab + 切回本 tab 时 view 变了才拉）。
      // search 视图数据独立存于 searchResults（doSearch 写入），切走再切回时保留上次
      // 结果——因此这里不做任何加载，只清掉其它视图残留的错误提示。
      // 热门/中文按当前分组拉取首屏；分组切换（topGroup/cnGroup 变）由下方 effect 触发。
      useEffect(() => {
        setErr('');
        if (view === 'top') { loadTop(topGroup, false); return; }
        if (view === 'cn') { loadCn(cnGroup, false); return; }
        if (view === 'mine') { loadFavs(); return; }
        if (view === 'recent') { loadRecent(); return; }
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [view]);
      // 热门/中文视图内切换主题分组：拉取该组首屏（该组已缓存则不重拉）。
      useEffect(() => {
        if (view === 'top') loadTop(topGroup, false);
        else if (view === 'cn') loadCn(cnGroup, false);
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [topGroup, cnGroup]);
      // 收藏增删：调 Host 后同步本地 store.radioFavs。
      function toggleFavStation(st) {
        const key = uidOf(st);
        const faved = (s.radioFavs || []).some((f) => uidOf(f) === key && key !== '');
        fetch('/dsh-music/radio/favs', {
          method: 'POST', cache: 'no-store',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ action: faved ? 'remove' : 'add', station: st }),
          ...
          if (d && d.ok) {
            set({ radioFavs: (d.favs || []).slice(), radioFavLoaded: true });
            if (view === 'mine') setRows(d.favs || []);
          } else setErr((d && d.error) || '收藏操作失败');
        }).catch(() => setErr('收藏操作失败'));
      }
      // 行点击：当前台 → 播放/暂停；否则播放新台。
      // HLS（m3u8）台已可播：Host 端 /radio/play 对 .m3u8 走转流（见 lib/hls.js），
      // 与纯流台同一播放链路（<audio> 收 ADTS 连续流），无需区分。
      function playStation(st) {
        const key = uidOf(st);
        const cur = s.currentId !== null && String(s.currentId).startsWith('radio:');
        const curIsThis = cur && (String(s.currentId).slice(6) === key || (key === '' && s.radioCurrent && s.radioCurrent.url === st.url));
        if (curIsThis && s.playing) { togglePlay(); return; } // 同一台且在播 → 暂停
        unlockAutoplay();
        startRadioPlayback(st);
      }
      function favedOf(st) {
        const key = uidOf(st);
        return (s.radioFavs || []).some((f) => uidOf(f) === key && key !== '');
      }
      function isNowPlaying(st) {
        if (s.currentId === null || !String(s.currentId).startsWith('radio:')) return false;
        const key = uidOf(st);
        return String(s.currentId).slice(6) === key || (key === '' && s.radioCurrent && s.radioCurrent.url === st.url);
      }
      // ---- 渲染 ----
      const viewBtn = (key, label) => React.createElement('button', {
        className: 'dsh-music-qq-viewtab' + (view === key ? ' active' : ''),
        onClick: () => setView(key),
      }, label);
      ...
      const listEl = (() => {
        // 行渲染（对任一 station 数组）。
        // 交互对齐本地/在线歌曲列表：整行可点播（未播→播放本台；正在播→暂停；
        // 双击不误触暂停），不再每行放独立 ▶ 按钮。播放态由行高亮 + 名字前 SVG 图标
        // 表达（播放中=❚❚ 提示点击暂停，暂停态=▶），与播放条 PlayIcon/PauseIcon 同款。
        const renderRows = (list) => list.map((st) => {
          const faved = favedOf(st);
          const active = isNowPlaying(st);
          const playing = active && s.playing;
          const isHls = !!(st && st.hls);
          // HLS 台目录 codec 多为 UNKNOWN/空：格式徽章统一走 radioQualityTxt（HLS 台显「HLS」）。
          const codecTxt = radioQualityTxt(st);
          const meta = [st.country, st.language, codecTxt].filter((x) => String(x || '').trim() !== '').join(' · ');
          const cls = 'dsh-music-qq-station' + (active ? ' active' : '');
          return React.createElement('div', { key: uidOf(st) || st.url, className: cls },
            React.createElement('button', {
              className: 'dsh-music-track' + (active ? ' active' : ''),
              title: st.url || '',
              onClick: (e) => { if (shouldIgnoreRowClick(e, active)) return; playStation(st); },
            },
              React.createElement('span', { className: 'dsh-music-qq-station-name' },
                (playing || active) ? React.createElement('span', { className: 'dsh-music-qq-station-state' },
                  playing ? React.createElement(PauseIcon, null)
                    : React.createElement(PlayIcon, null)) : null,
                React.createElement('span', { className: 'dsh-music-qq-station-title' }, st.name || '未命名电台'),
                isHls ? React.createElement('span', { className: 'dsh-music-qq-station-tag hls' }, 'HLS') : null,
                st.lastcheckok === false ? React.createElement('span', { className: 'dsh-music-qq-station-tag bad' }, '可能失效') : null),
              meta !== '' ? React.createElement('span', { className: 'dsh-music-qq-station-meta' }, meta) : null),
            React.createElement('button', {
              className: 'dsh-music-qq-station-fav' + (faved ? ' on' : ''), title: faved ? '取消收藏' : '收藏',
              onClick: (e) => { e.stopPropagation(); toggleFavStation(st); },
            }, React.createElement(HeartIcon, { filled: faved, size: 15 })),
          );
        });
        const loadingEl = (txt) => React.createElement('div', { className: 'dsh-music-loading' }, txt);
        const emptyEl = (txt) => React.createElement('div', { className: 'dsh-music-empty' }, txt);
        // 搜索视图：独立数据源 searchResults（null=未搜/搜索中 引导态，[]=无结果）。
        if (view === 'search') {
          if (searching) return loadingEl('搜索中…');
          if (searchResults === null) {
            return emptyEl('在上方输入台名 / 国家 / 语言开始搜索，例如 China、Classical、中国之声。');
          }
          if (searchResults.length === 0) {
            return emptyEl('没有找到电台。试试输入台名（如 China Plus）或国家名（如 China）。');
          }
          return renderRows(searchResults);
        }
        // 中文电台视图：按当前分组从 cnMap 取（undefined=加载中）。
        if (view === 'cn') {
          const data = cnMap[cnGroup];
          if (data === undefined) {
            // 首屏失败（自动重试耗尽）：显示失败 + 手动「重试」，不再永远停在加载中。
            const fail = (dirFail && dirFail.kind === 'cn' && dirFail.group === cnGroup) ? dirFail : null;
            if (fail) return React.createElement(React.Fragment, null,
              React.createElement('div', { className: 'dsh-music-empty' },
                '中文电台加载失败（' + (fail.msg || '目录源不可达') + '），自动重试后仍未恢复。请检查网络后重试。'),
              React.createElement('div', { style: { display: 'flex', justifyContent: 'center', padding: '4px 0 10px' } },
                React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => loadCn(cnGroup, false) }, '重试')));
            return loadingEl('加载中…');
          }
          const rowsEl = data.rows.length === 0 ? emptyEl('没有找到可播的中文电台（目录源可能不可达）。') : renderRows(data.rows);
          return React.createElement(React.Fragment, null,
            rowsEl,
            loadMoreFooter('cn', data));
        }
        // 热门电台视图：按当前分组从 topMap 取（undefined=加载中）。
        if (view === 'top') {
          const data = topMap[topGroup];
          if (data === undefined) {
            const fail = (dirFail && dirFail.kind === 'top' && dirFail.group === topGroup) ? dirFail : null;
            if (fail) return React.createElement(React.Fragment, null,
              React.createElement('div', { className: 'dsh-music-empty' },
                '热门电台加载失败（' + (fail.msg || '目录源不可达') + '），自动重试后仍未恢复。请检查网络后重试。'),
              React.createElement('div', { style: { display: 'flex', justifyContent: 'center', padding: '4px 0 10px' } },
                React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => loadTop(topGroup, false) }, '重试')));
            return loadingEl('加载中…');
          }
          const rowsEl = data.rows.length === 0 ? emptyEl('热门电台为空（目录源可能不可达）') : renderRows(data.rows);
          return React.createElement(React.Fragment, null,
            rowsEl,
            loadMoreFooter('top', data));
        }
        // 收藏/最近视图：rows（null=加载中）。
        if (rows === null) return loadingEl('加载中…');
        if (rows.length === 0) {
          return emptyEl(view === 'mine'
            ? '还没有收藏的电台。浏览「热门」或搜索后点行尾 ♥ 收藏。'
            : '还没有播放记录。播放任意电台后会出现在这里。');
        }
        return renderRows(rows);
      })();
      // 搜索框行：仅「搜索」视图显示；其它视图顶部显示当前视图提示（对齐 QQ 面板
      // 「搜索 tab 才有搜索行」的交互：热门/收藏/最近各自专注自己的列表）。
      let searchRow = null;
      if (view === 'search') {
        const searchBox = React.createElement('div', { className: 'dsh-music-qq-search', ref: histRef },
          React.createElement('div', { className: 'dsh-music-qq-inputwrap' },
            React.createElement('input', {
              className: 'dsh-music-qq-input', type: 'text', placeholder: '搜索网络电台（台名 / 国家 / 语言，如 China、Classical）',
              value: q,
            ...
            React.createElement('button', {
              className: 'dsh-music-qq-clear' + (q === '' ? ' hidden' : ''),
              type: 'button', title: '清空输入',
              onClick: () => { setQ(''); setSearchResults(null); setErr(''); },
            }, '✕')),
          React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => doSearch(q) }, searching ? '搜索中…' : '搜索'));
        const histPop = (histOpen && hist.length > 0)
          ? portalToBody(React.createElement('div', { className: 'dsh-music-qq-hist', style: anchorBelow(histRef.current, 420), ref: histPopRef },
            React.createElement('div', { className: 'dsh-music-qq-hist-head' },
              React.createElement('span', { className: 'dsh-music-hint' }, '搜索历史'),
              React.createElement('button', { className: 'dsh-music-qq-hist-clear', title: '清空历史', onClick: () => { clearPref(PREF_RADIO_HISTORY); setHist([]); } }, '清空')),
            hist.map((kw) => React.createElement('button', { key: kw, className: 'dsh-music-qq-hist-item', onClick: () => doSearch(kw) }, kw))))
          : null;
        searchRow = React.createElement('div', { className: 'dsh-music-qq-searchrow' }, searchBox, histPop);
      }
      // 主题分组 pill 行：仅「热门电台」/「中文电台」视图显示（分组浏览入口）。
      const groupPill = (list, cur, onPick) => list.map((g) => React.createElement('button', {
        key: g.key, className: 'dsh-music-qq-cat' + (cur === g.key ? ' active' : ''),
        onClick: () => onPick(g.key),
      }, g.label));
      let groupRow = null;
      if (view === 'top') {
        groupRow = React.createElement('div', { className: 'dsh-music-qq-cats', style: { margin: '0 0 8px' } },
          groupPill(RADIO_TOP_GROUPS, topGroup, (g) => { setErr(''); setTopGroup(g); }));
      } else if (view === 'cn') {
        groupRow = React.createElement('div', { className: 'dsh-music-qq-cats', style: { margin: '0 0 8px' } },
          groupPill(RADIO_CN_GROUPS, cnGroup, (g) => { setErr(''); setCnGroup(g); }));
      }
      // 视图提示文字：热门/中文/我的电台/最近播放等列表视图均不显示（列表上方保持
      // 干净），仅「搜索」视图保留提示（配合搜索输入框）。
      const hintTxt = view === 'search' ? '按台名 / 国家 / 语言搜索全球电台目录' : null;
      // 当前视图是否正展示「目录首屏失败」（dirFail 命中本视图/分组）。命中时错误文案
      // 已在 listEl 失败块里显示，顶层 err 条不再重复渲染。
      const activeFail = (view === 'cn' || view === 'top') && dirFail !== null
        && dirFail.kind === view && dirFail.group === (view === 'cn' ? cnGroup : topGroup)
        ? dirFail : null;
      return React.createElement('div', { className: 'dsh-music-qq dsh-music-radio' },
        React.createElement('div', { className: 'dsh-music-qq-head' },
          React.createElement('div', { className: 'dsh-music-qq-toolbar' },
            React.createElement('div', { className: 'dsh-music-qq-viewtabs' },
              viewBtn('mine', '我的电台'),
              ...
              viewBtn('search', '搜索'))),
          groupRow,
          hintTxt !== null ? React.createElement('div', { className: 'dsh-music-hint', style: { margin: '2px 0 6px' } }, hintTxt) : null),
        searchRow,
        React.createElement('div', { className: 'dsh-music-qq-body' },
          (err !== '' && activeFail === null) ? React.createElement('div', { className: 'dsh-music-error' }, err) : null,
          listEl));
    }
    ...
    function QQOnlinePanel({ panelRef }) {
      ...
      useEffect(() => {
        setHist(loadHist());
        jsonGet('/dsh-music/qq/status').then((d) => {
          if (!d) return;
          // status 只反映「cookie 是否存在」（Host 不做主动探测）。「疑似登录失效」的标记
          // 由**播放失败**置位（见 onError → markOnlineAuthFailed），这里不主动清它，
          // 否则刚播失败切回登录 UI、一拉 status 又被清掉。
          setLoggedIn(!!d.loggedIn); setUin(d.uin || ''); setNickname(d.nickname || '');
          // 只在 status 真的给了登录方式时才覆盖 store（无脑写空会把 manifest 下发的
          // 「已登录（微信）」抹成「已登录」——关于页就是这么被弄花的）。
          if (d.loginFrom) set({ qqLoginFrom: d.loginFrom });
        }).catch(() => {});
      ...
      // 引擎侧检出登录过期（播放中取链失败复查 / Host 状态）→ 面板立即回到登录 UI：
      // 不让用户停在「看着已登录、VIP 却播不了」的界面里；登录方式由他在登录页自己选。
      useEffect(() => {
        if (!s.qqAuthExpired) return;
        setLoggedIn(false); setLayer('main'); setActivePl(null); setPlLoading(false);
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [s.qqAuthExpired]);
      ...
      function playSong(song, queue, sourceLabel, from) {
        // 未登录（或登录已过期）时点击 VIP 歌曲：不启动注定失败的播放（否则播放条会误报
        // 「频谱不可用/音频加载失败」，重试也无济于事），只提示。**不替用户选择登录方式**——
        // 此时面板本就停在登录 UI（QQ / 微信两个按钮都在），用户自己点哪个都可以。
        const isVip = song.payplay === 1 || (song.pay && song.pay.payplay === 1);
        if (isVip && (!loggedIn || s.qqAuthExpired)) {
          setQError(s.qqAuthExpired ? 'QQ 音乐登录已过期，VIP 歌曲需重新登录' : 'VIP 歌曲需先登录才能播放');
          return;
        }
      ...
      async function logout() {
        ...
        resetQQAuthExpired(); // 主动登出：清掉「已过期」提示，回到正常登录页
      ...
      // ---- 未登录 / 登录已过期：只显示居中两个登录按钮（QQ 登录 / 微信登录，分两行）+ 风险提示 ----
      // 过期与从未登录走同一个入口：**绝不替用户预设登录方式**（Host 的 loginFrom 只用来
      // 提示「上次用哪种」，按钮仍然两个都给）。
      if (!loggedIn) {
        return React.createElement('div', { className: 'dsh-music-qq dsh-music-qq-login' },
          React.createElement('div', { className: 'dsh-music-qq-login-center' },
            s.qqAuthExpired
              ? React.createElement('div', { className: 'dsh-music-qq-login-tip' },
                  '登录可能已失效，请重新登录' + (s.qqLoginFrom === 'qq' ? '（上次使用 QQ 登录）' : s.qqLoginFrom === 'wx' ? '（上次使用微信登录）' : ''))
              : null,
    ...
    function PlayerPanel() {
      ...
      // Esc 关闭播放面板：仅当有「更内层、自带 Esc 语义」的弹层在场时不生效——
      // 名称输入/确认/登录/定时任务编辑等全屏 overlay（.dsh-music-picker-overlay）
      // 以及版本更新弹窗（.dsh-music-whatsnew，自带 document 级 Esc）打开时，
      // Esc 属于它们（关弹层或先由其处理），不在这里把整个面板一起关掉。
      // 监听挂在 capture 阶段：必须在任何内层处理器运行前先看到弹层还在场——
      // 若挂在冒泡阶段，焦点在弹层输入框内按 Esc 时，内层先关弹层（React 在
      // 容器上同步刷新 DOM），事件冒泡到 document 时弹层已消失，会误把面板
      // 一起关掉（一次 Esc 连关两层）。capture 先到场检查则无此竞态。
      // 歌词/字幕面板与播放条弹层（模式/音量/目录）暂不处理 Esc（本次改动最小）。
      useEffect(() => {
        if (!s.panelOpen) return;
        const hasInnerEscModal = () => document.body.querySelector('.dsh-music-picker-overlay') !== null
          || document.body.querySelector('.dsh-music-whatsnew') !== null;
        const onKey = (e) => {
          if (e.key !== 'Escape') return;
          if (hasInnerEscModal()) return;
          e.preventDefault();
          set({ panelOpen: false });
        };
      ...
      const listBody = React.createElement('div', { className: 'dsh-music-list-body' },
        ...
        React.createElement('div', { className: 'dsh-music-qq-pane', style: paneStyle('nc') }, React.createElement(NCOnePanel, { panelRef })),
      ...
      const ttsHintLines = s.tab === 'book'
        ...
        : (s.tab === 'news'
          ...
          : (s.tab === 'music'
            ...
            : (s.tab === 'radio'
              ? ['目录数据来自开源社区 radio-browser（CC 开放数据）；播放的是各电台公开直播流。仅供个人收听，不录制、不二次分发，内容版权归台站/版权方所有。']
      ...
      return React.createElement('div', { className: 'dsh-music-panel', ref: panelRef, style: rootStyle },
        ...
        React.createElement('div', { className: 'dsh-music-panel-body' },
          ...
          React.createElement('div', { className: 'dsh-music-tabs' },
            tabBtn('music', '本地音乐'), tabBtn('qq', 'QQ音乐'), tabBtn('kg', '酷狗音乐'), tabBtn('nc', '网易云'), tabBtn('radio', '网络电台'), tabBtn('book', 'AI讲书'), tabBtn('news', '新闻播报'), tabBtn('config', '系统配置'), tabBtn('about', '关于')),
          React.createElement('div', { className: 'dsh-music-panel-content' },
            (s.tab === 'qq' || s.tab === 'kg' || s.tab === 'nc' || s.tab === 'radio' || s.tab === 'news' || s.tab === 'config' || s.tab === 'about') ? null : React.createElement(DirectorySetting, { panelRef }),
    ...
    function SystemSetting() {
      ...
      // 播放条位置：放在系统配置的最下方（它是「整个播放条往哪放」的总开关，与上面
      // 逐项显示开关不是一类）。三选一，落 Host prefs；「悬浮可拖动」下条可直接拖着挪地方。
      const barPosOptions = [
        ['dock', '输入框上方'],
        ['bottom', '页面底部'],
        ['float', '悬浮可拖动'],
      ];
      const barPosRow = React.createElement('div', { className: 'dsh-music-config-row' },
        React.createElement('div', { className: 'dsh-music-config-info' },
          React.createElement('span', { className: 'dsh-music-config-label' }, '播放条位置'),
          React.createElement('span', { className: 'dsh-music-config-desc' }, '根据喜好选择播放条位置')),
        React.createElement('div', { className: 'dsh-music-config-seg' },
          barPosOptions.map(([val, label]) => React.createElement('button', {
            key: val,
            className: 'dsh-music-config-seg-btn' + (s.barPos === val ? ' on' : ''),
            onClick: () => set({ barPos: val }),
            'aria-pressed': s.barPos === val,
            title: label,
          }, label))));
      // 悬浮位置复位：清掉记住的坐标后回到默认的右下角。只在悬浮模式下才有意义。
      const barPosResetRow = React.createElement('div', { className: 'dsh-music-config-row' },
        React.createElement('div', { className: 'dsh-music-config-info' },
          React.createElement('span', { className: 'dsh-music-config-label' }, '悬浮位置'),
          React.createElement('span', { className: 'dsh-music-config-desc' },
            s.barFloatPos === null ? '还没拖动过：默认贴在窗口右下角' : '已记住拖动后的位置')),
        React.createElement('button', {
          className: 'dsh-music-config-btn',
          disabled: s.barFloatPos === null,
          onClick: () => { clearPref(PREF_BAR_FLOAT_POS); set({ barFloatPos: null }); },
        }, '重置位置'));
      return React.createElement('div', { className: 'dsh-music-config' },
        ...
        immerseRow,
        // 播放条位置：放在最下方——它是「整个播放条放在哪」的落位选择，
        // 与上面那些逐项显示开关不是一类；悬浮模式下紧跟一行「悬浮位置」复位。
        configCard(barPosRow, s.barPos === 'float' ? barPosResetRow : null),
      );
    }
    ...
    function About() {
      ...
      const kgTxt = s.kgLoggedIn ? ('已登录' + (s.kgNickname ? '（' + s.kgNickname + '）' : '')) : '未登录（扫码后可用）';
      const ncTxt = s.ncLoggedIn ? ('已登录' + (s.ncNickname ? '（' + s.ncNickname + '）' : '')) : '未登录（匿名可用）';
      return React.createElement('div', { className: 'dsh-music-about' },
        React.createElement('div', { className: 'dsh-music-about-card' },
          ...
          infoRow('网易云音乐', ncTxt, s.ncLoggedIn ? 'ok' : '')),
        React.createElement('div', { className: 'dsh-music-about-card' },
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        startRaf();
        // 多标签页统一播放器：向已在运行的其它标签页打招呼，出声页会把当前播放态
        // 全量回给我们（新开的页因此立刻显示成「同一个播放器」而不是空白/过期的恢复态）。
        xtabPost({ t: 'hello', id: xtabId });
        // 心跳：只报「这页还活着」。转发指令前要判断 owner 是否还在，而 owner 暂停/
        // 空闲时不会广播状态，没有心跳就会被误判成已关页（点 ▶ 变成错误地在本页接管）。
        const xtabPing = setInterval(() => {
          if (xtab === null) return;
          xtabPost({ t: 'ping', id: xtabId });
        }, XTAB_PING_MS);
        ...
        const onPageHide = () => {
          try { audio.pause(); } catch (e) {}
          try { preAudio.pause(); } catch (e) {}
          // 多标签页统一播放器：关页时把出声权交还其它页（bye 立即生效，收尾状态定格
          // 成「暂停在最后位置」），否则其它页会以为声音还在这个已经关掉的页里。
          xtabShutdown();
      ...
      const intentTimer = setInterval(() => {
        jsonGet('/dsh-music/intent').then((intent) => {
          ...
          // 多标签页：镜像态下这些指令转给唯一出声页执行（本页元素是暂停的，
          // 直接本地执行会变成「本页起播」或者把本页显示状态改得和出声页不一致）。
          if (action === 'pause') { if (xtabForward('pause')) return; audio.pause(); set({ playing: false }); return; }
          if (action === 'resume') {
            if (xtabForward('resume')) return;
            const p = audio.play();
            if (p !== undefined && typeof p.catch === 'function') p.catch((err) => { if (!isPlayAborted(err)) set({ error: '播放失败' }); });
            return;
          }
          ...
          // online NetEase track (agent requested source=netease, or panel click).
          if (intent.kind === 'nc' && typeof intent.id === 'string' && /^[0-9]+$/.test(intent.id)) {
            const song = { id: intent.id, title: intent.name || '网易云音乐', artists: intent.artists || [], fee: 0, source: 'nc' };
            startNCPlayback(song, [song], '在线');
            return;
          }
          ...
          if (intent.kind === 'news' && typeof intent.id === 'string' && /^[A-Za-z0-9-]+$/.test(intent.id)) {
            // 熄屏/锁屏/无人观看时不自动起播：只刷新期次列表（该期保持「待播」，
            // 在面板里点一下即可播），既不对空房间播完整期，也不会在解锁瞬间由
            // 音频设备重新上电发出爆音（判定见 shouldDeferAutoplay）。
            if (shouldDeferAutoplay()) {
              try { console.info('[dsh-music] 熄屏/页面不可见：推迟自动播报 ' + intent.id + '（面板「新闻播报」里点播即可）'); } catch (e) {}
              void loadNewsEditions();
              return;
            }
          ...
          // （id/name/radioUrl/url/codec/bitrate/hls），还原成 station 对象直接开播。
          if (intent.kind === 'radio' && typeof intent.id === 'string' && typeof intent.radioUrl === 'string') {
            const st = {
              id: intent.id, stationuuid: intent.id,
              name: intent.name || '网络电台', url: intent.radioUrl,
              codec: intent.codec || '', bitrate: intent.bitrate || 0, hls: !!intent.hls,
              country: intent.country || '', countrycode: intent.countrycode || '',
              favicon: intent.favicon || '', tags: intent.tags || [],
            };
            startRadioPlayback(st);
            return;
          }
          ...
          if (track !== null) {
            // 多标签页：镜像态下由出声页来播这首（本页只更新显示状态；否则会出现
            // 本页悄悄起播、出声页还在放旧曲的双播）。
            if (xtabForward('playLocal', { id: intent.id })) return;
      ...
      // 播放条落位（系统配置「显示位置」）：dock 模式挂在输入框上方；页面底部/悬浮模式挂在
      // root 作用域的 shell.overlay 上。两处各注册一个实例，由 NowPlayingBar 按 barPos
      // 互斥渲染，保证同一时刻只有一条。
      // 挂在 shell.overlay（而非对话列）的三个好处：① 询问弹窗（conversation.composer
      // 链接管输入区时会隐藏整个输入区，见 ui-slots 的 overlay 语义）不再连播放条一起
      // 藏掉；② 会话列卸载（切到设置/插件管理面板）时播放条依然在；③ 该层在所有列之上、
      // 在它们的滚动容器之外，条不会被对话滚动或列裁剪影响。
      ctx.effect(() => slots.inject('conversation.input.dock', () => slots.register(
        { name: 'conversation.input.dock', id: 'music-player-bar', order: 40 },
        () => React.createElement(NowPlayingBar, { placement: 'dock' }),
      )), 'music-player: now playing bar');
      ctx.effect(() => slots.inject('shell.overlay', () => slots.register(
        { name: 'shell.overlay', id: 'music-player-bar-floating', order: 19 },
        () => React.createElement(NowPlayingBar, { placement: 'overlay' }),
      )), 'music-player: floating now playing bar');
      ctx.effect(() => slots.inject('shell.overlay', () => slots.register(
    ...
    const PLAYER_CSS = '\n' +
      ...
      // ---- 显示位置：页面底部（.bottom）----
      // 固定条贴在窗口底边：left/width 由 useColumnMetrics 内联给出（对齐会话列宽），
      // bottom 固定为 0，所以这里只负责「固定 + 悬浮层级 + 外壳配色」。z-index 900：在
      // shell.overlay 这一层（AppFrame 的 z-index 20 浮动层）内部压过对话内容，但低于
      // 插件自己的播放面板（1000 与 2000 的目录选择器/确认框）——真正的模态仍然盖住它。
      // 外层 pointer-events:none、只有条本身 auto：条左右两侧的空白带不吞掉下方点击。
      '.dsh-music-bar-wrap.bottom { position: fixed; left: 0; bottom: 0; z-index: 900; padding: 0; width: auto; pointer-events: none; }\n' +
      // 注意 :not(.bare)：外层背景/阴影的选择器比 `.dsh-music-bar.bare` 更具体，不排除
      // 「关闭背景显示」就会把它压掉（表现为配置项失效，切到这个落位后关不掉背景）。
      '.dsh-music-bar-wrap.bottom .dsh-music-bar:not(.bare) { background: var(--dsw-alias-bg-base, #fff); box-shadow: 0 -8px 20px rgba(0,0,0,0.18); }\n' +
      '.dsh-music-bar-wrap.bottom .dsh-music-bar { pointer-events: auto; border-radius: 10px 10px 0 0; }\n' +
      '.dsh-music-bar-wrap.bottom .dsh-music-bar-progress { border-radius: 0; }\n' +
      // 页面底部模式的「让位」规则：给 DSH 的输入区底座（[data-composer-seat]）补一段
      // 下内边距，输入卡片整体上移，贴底的播放条因此不覆盖输入框、也不遮住任何对话内容
      // （底座背景本来就是自上而下渐变成实色的遮罩，补出来的这条带子会是干净的底色）。
      // 标记属性只在页面底部模式下挂（见组件里的 effect），切走即摘除，不留副作用；
      // 属性不存在时这条规则完全不生效。
      ':root[data-dsh-music-bar-bottom] [data-composer-seat] { padding-bottom: ' + BAR_BOTTOM_PAD + 'px; }\n' +
      // ---- 显示位置：悬浮条（.float）----
      // 三行紧凑卡片：名称 / 歌词独占整行 / 控件常显（time 靠左、按钮靠右），进度线贴底。
      // 默认贴右下角（right/bottom）；一旦有记忆坐标，内联 left/top 会把 right/bottom 置回
      // auto（见 wrapStyle 注释），否则左右同时生效会把固定元素拉宽。
      // 整条可拖动：cursor: grab 提示可拖；交互控件（button 等）不参与拖动。
      '.dsh-music-bar-wrap.float { position: fixed; z-index: 900; padding: 0; width: auto; pointer-events: auto; right: ' + BAR_FLOAT_EDGE + 'px; bottom: ' + BAR_FLOAT_EDGE + 'px; }\n' +
      // 同上：背景/阴影只在「没关闭背景显示」时给，否则会压掉 `.dsh-music-bar.bare`。
      '.dsh-music-bar-wrap.float .dsh-music-bar { width: ' + BAR_FLOAT_W + 'px; max-width: none; margin: 0; flex-wrap: wrap; row-gap: 2px; border-radius: 12px; cursor: grab; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar:not(.bare) { background: var(--dsw-alias-bg-overlay, #1e1f22); box-shadow: 0 12px 32px rgba(0,0,0,0.35); }\n' +
      // 三行固定：① 歌词独占整行（第一行）② 名称 + 频谱 ③ 控件（时长靠左、按钮靠右）。
      // 歌词之所以单独占一行：和歌名挤同一行时，歌名（含歌手/音质徽章）会把它压到只剩一小截，
      // 中文歌词几乎每句都被截断；独占一行后可用宽度 ≈ 条宽 − 20px。
      // 行高固定（min-height/line-height 都是 20px）：没有歌词时那一行由占位元素撑着，
      // 有/无歌词、空闲/播放中都不改变条的高度——悬浮条不能一边放歌一边变高变矮。
      // 名称用 flex: 0 1 auto（不伸长、只收缩）：频谱因此紧跟在歌名/徽章后面，
      // 而不是被「占满整行」的歌名推到最右边；歌名过长时自己省略，频谱始终贴着徽章。
      // max-width 预留出频谱的位置（60px 画布 + 8px 间距 + 4px 余量，避免亚像素取整后
      // 刚好超出一行宽度）：flex-wrap 换行是按各条的「理想宽度」算的、不受 flex-shrink
      // 影响，不设上限时长歌名会把频谱挤到自己独占一行，条就从三行变四行。
      '.dsh-music-bar-wrap.float .dsh-music-bar-lyric { order: 0; flex: 1 0 100%; max-width: none; min-height: 20px; line-height: 20px; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-name { order: 1; flex: 0 1 auto; min-width: 0; max-width: calc(100% - 72px); }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-viz { order: 2; flex: none; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-lyric-empty { display: flex; align-items: center; justify-content: center; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-controls { order: 3; flex: 1 0 100%; margin-left: 0; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-time, .dsh-music-bar-wrap.float .dsh-music-bar-live { margin-right: auto; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-hotspot { display: none; }\n' +
      '.dsh-music-bar-wrap.float .dsh-music-bar-progress { border-radius: 0 0 12px 12px; }\n' +
      ...
      // 名称文本（音符图标 + 歌名）内层：承载单击打开面板；参与外层 flex、可省略截断。
      ...
      // 电台直播徽标：红色圆点 + LIVE（或暂停时灰色「电台」）。红色用错误态主色，
      // 深色浅色主题都醒目；文字不加粗（LIVE 本身信息量足够，避免抢戏）。
      '.dsh-music-bar-live { line-height: 1; font-variant-numeric: tabular-nums; animation: dsh-music-time-in 0.3s ease; color: var(--dsw-alias-state-error-primary, #e5534b); white-space: nowrap; }\n' +
      ...
      // 配置行右侧的小按钮（目前只有「重置位置」）：与分段按钮同款描边风格。
      '.dsh-music-config-btn { padding: 4px 10px; font-size: 11px; line-height: 1.5; border-radius: 12px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.35)); background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; white-space: nowrap; transition: background 0.15s, color 0.15s, border-color 0.15s; }\n' +
      '.dsh-music-config-btn:hover:not(:disabled) { color: var(--dsw-alias-label-primary, #e6e6e6); border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-config-btn:disabled { opacity: 0.45; cursor: default; }\n' +
      ...
      '.dsh-music-qq-login-tip { margin-bottom: 10px; padding: 6px 10px; border-radius: 6px; background: rgba(229, 83, 75, 0.12); color: var(--dsw-alias-state-error-primary, #e5534b); font-size: 12px; line-height: 1.5; text-align: center; }\n' +
      ...
      // ---- 网络电台（radio）station 行：与在线歌单行同宽的 flex 布局 ----
      // 行 = [播放钮] [名称+meta 信息区（点击即播）] [收藏钮]。active 行用主题色描边。
      '.dsh-music-qq-station { display: flex; align-items: center; gap: 6px; width: 100%; box-sizing: border-box; padding: 2px; border-radius: 8px; }\n' +
      // 整行点播主体（复用 .dsh-music-track 透明 button 语义）：电台行内改为纵向两行布局。
      '.dsh-music-qq-station .dsh-music-track { flex: 1; min-width: 0; flex-direction: column; align-items: flex-start; gap: 1px; padding: 6px 8px; border-radius: 6px; }\n' +
      '.dsh-music-qq-station:hover { background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); }\n' +
      '.dsh-music-qq-station.active { background: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 12%, transparent); }\n' +
      '.dsh-music-qq-station.active .dsh-music-track { background: transparent; }\n' +
      '.dsh-music-qq-station-name { display: inline-flex; align-items: center; gap: 4px; min-width: 0; max-width: 100%; font-size: 13px; color: var(--dsw-alias-label-primary, #e6e6e6); }\n' +
      '.dsh-music-qq-station.active .dsh-music-qq-station-name { color: var(--dsh-music-accent, #2f9e6e); font-weight: 600; }\n' +
      // 名字前缀状态图标（active 行）：播放中=❚❚ 暂停=▶ 的 SVG 版，与行高亮同色，恒定尺寸。
      '.dsh-music-qq-station-state { flex: none; display: inline-flex; align-items: center; }\n' +
      '.dsh-music-qq-station-state svg { display: block; width: 12px; height: 12px; }\n' +
      '.dsh-music-qq-station-title { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\n' +
      '.dsh-music-qq-station-meta { font-size: 11px; color: var(--dsw-alias-label-secondary, #8a8f98); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }\n' +
      '.dsh-music-qq-station-tag { flex: none; font-size: 10px; padding: 0 4px; margin-left: 6px; border-radius: 4px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.35)); color: var(--dsw-alias-label-secondary, #8a8f98); vertical-align: 1px; }\n' +
      // HLS 格式徽章：可播（Host 转流），用 accent 信息色而非警告黄。
      '.dsh-music-qq-station-tag.hls { color: var(--dsh-music-accent, #2f9e6e); border-color: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 45%, transparent); }\n' +
      '.dsh-music-qq-station-tag.bad { color: var(--dsw-alias-state-error-primary, #e5534b); border-color: color-mix(in srgb, var(--dsw-alias-state-error-primary, #e5534b) 45%, transparent); }\n' +
      '.dsh-music-qq-station-fav { flex: none; display: inline-flex; align-items: center; justify-content: center; border: none; background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; padding: 2px 4px; }\n' +
      '.dsh-music-qq-station-fav svg { display: block; }\n' +
      '.dsh-music-qq-station-fav:hover { color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-qq-station-fav.on { color: var(--dsh-music-accent, #2f9e6e); }\n' +
      ...
      // 多标签页统一播放器：「此页静音」徽标——中性色，避免与音质/来源徽标（主题色）混淆。
      '.dsh-music-bar-xout { color: var(--dsw-alias-label-secondary, #8a8f98); border-color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
