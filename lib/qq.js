...
/**
 * ⚠ 反面教材（别再踩）：判断「QQ 音乐登录态是否还有效」**不要做主动探测**。
 *
 * 2026-09-16 实测：`music.UserInfo.userInfo/GetLoginUserInfo` 对**刚登录、明确有效**的微信会话
 * 同样返回 `code 500003`（同一会话 `detectVip=true`、能取到 FLAC 直链；把 music key 改成垃圾值
 * 后结果不变）；`GetUserInfo`、`music.vip.VipInfo/GetVipInfo` 同理。歌单类读接口
 * （`music.srfDissInfo.DissInfo/CgiGetDiss`、`GetPlaylistByUin`）匿名也返回 code 0。
 * 也就是说：**没有一个便宜可靠的接口能判定登录态**，主动探测只会误报，而误报会把正常登录的
 * 用户踢回登录页。
 *
 * 定稿：不做任何主动探测——过期只在**实际播放失败**时由客户端发现（在线曲目整列都播不出来 →
 * 面板切回登录 UI），见 lib/index.js /qq/play 的说明与 lib/client.js 的 onError。
 *
 * `detectVip()` 只用于「VIP 档位」提示（能否取到无损），不承担登录态判定。
 */
