...
// 出网限流：并发上限 + 最小间隔，避免兜底逻辑对第三方接口无限制发起请求
// （耗尽调用配额甚至触发 IP 封禁）。
...
function runNextFetch() {
  if (inFlightFetches >= MAX_CONCURRENT_FETCHES || fetchQueue.length === 0) return
  inFlightFetches++
  const next = fetchQueue.shift()
  setTimeout(next, MIN_FETCH_GAP_MS)
}

function acquireFetchSlot() {
  return new Promise((resolve) => { fetchQueue.push(resolve); runNextFetch() })
}
