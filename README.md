...
- **多标签页统一播放器**：同时开多个 DSH 标签页时，所有标签页共用**同一个播放器**——同一时刻只有一页出声，其余页静音并**同步显示**同一首歌/同一进度/同一播放状态（曲目、歌手、歌词、播放模式、音量、讲书章节全都一致），在任意一页点播放/暂停、上一首/下一首或直接点歌，都会作用到这个唯一播放器上（详见 FAQ）
...
3. **每日定时（Host 自维护）**：在「新闻播报」页签的「⏰ 每日定时」里可视化配置**多定时任务**（如 08:00 / 12:30 / 18:00），每个定时任务可独立指定收集范围与**新闻条数**（1-20，默认 8；选了多个类别时各类别尽量平均分配），并勾选「收集后立即播放」（不勾选 = **静默收集**，只更新简报不出声；熄屏/锁屏时也不会自动出声——该期保持「待播」徽标，解锁后点播即可，见下方 FAQ）；也可勾选「**仅工作日执行**」——按工作日历判断：周一至周五**扣除法定节假日**放假不跑、**周末调休补班视为工作日照常**。节假日数据**按需自动联网获取**（仅在到点判断「仅工作日」定时任务、且日历缺失/过期时才查询 timor.tech，失败自动回退内置表/周一至周五），**无需任何手工维护**。**保存即生效**——定时器由插件在 DSH 主机进程内自维护（无需 agent 创建/同步 DSH 定时任务、无需手动开会话），到点自动执行。也可对 agent 说「每天早上 9 点播报新闻」让其引导配置。可在「⏰ 每日定时」里给执行会话选一个**新闻会话模型**（不选则跟随当前活跃会话）。
...
**播放条固定在输入框上方，太抢注意力 / 想让它挪个地方？**
打开播放面板 →「系统配置」→ 最下方的「播放条位置」，三选一：

| 选项 | 说明 |
| --- | --- |
| 输入框上方 | 默认（与旧版一致），全宽显示在输入卡片上方 |
| 页面底部 | 贴在**窗口底边**、横向对齐会话列的一条细条；插件会给输入区底座补一段等高的下内边距，让输入卡片整体上移，因此这条既压不住输入框、也不遮住对话内容 |
| 悬浮可拖动 | 紧凑三行卡片，**高度固定不跳**：① **歌词独占整行**（第一行，中文歌词基本不用截断；当前没有歌词时留同高占位，所以有词/无词、空闲/播放中都不会变高变矮）② 歌名 + 实时频谱（频谱紧跟歌名/音质徽章）③ 时长 + 播放控制。**整条可直接拖动**，位置记在 Host 上、刷新后仍在；默认贴窗口右下角，「悬浮位置 → 重置位置」可复位 |

悬浮条闲置时会按「沉浸感」变淡，鼠标移上去恢复——不抢注意力但也一眼能找到。另外：改用页面底部或悬浮位置后，播放条不再挂在输入框区域，对话里的**询问弹窗**（它会接管输入区）不会再连播放条一起盖住，弹窗开着也能直接暂停/切歌。

**同时开了好几个 DSH 标签页，会不会两首歌一起响？**
不会。插件把多个标签页收拢成**一个播放器**：同一时刻只有一页出声（由开始播放的那一页担任「出声页」），其余页自动静音并**同步显示**同一个播放器的状态——同一首歌、同一播放进度、同一播放/暂停状态，歌词、播放模式、音量、讲书章节都一致。在任意一页上的操作都会作用到这个唯一播放器：播放/暂停、上一首/下一首、停止、音量、播放模式、讲书换声音/跳章、点歌/点电台/点书/点新闻期次，都会转发到出声页执行。静音的那些页会在播放条歌名后显示 **「此页静音」** 徽标，提示声音来自另一个标签页。出声页被关闭时，其余页会停在最后的位置（不会两页抢播），在任意一页点 ▶ 即可就地接着播（讲书按 Host 上保存的进度续播）。
...
**勾了「收集后立即播放」，为什么熄屏/锁屏时没自动出声？**
熄屏/锁屏时插件**不会自动起播**新闻：页面不可见（后台标签/最小化/锁屏），或页面不再渲染帧（macOS 熄屏/锁屏时浏览器不把页面置为 hidden，但合成器停发帧）即视为「无人观看」——避免对着空房间把整期播完，也避免播放期反复申请/释放防熄屏锁让音频设备进出低功耗态（解锁瞬间的「咚咚」爆音）。该期会保持**「待播」**徽标，解锁后在「新闻播报」页签点一下即可播。若熄屏时本来就在听音乐/讲书（有人在听），定时播报照常插入，播完自动切回原内容。
