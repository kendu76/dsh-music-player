...
- **浏览**：推荐歌单（600+ 热门精选）/ 分类歌单 / 排行榜（TOP500、国潮音乐榜等 57 个，榜单歌曲一次全量加载，如 TOP500=500 首）/ 统一搜索（歌曲 + 歌单）/ 我的歌单（可在歌单详情里移除歌曲）。
